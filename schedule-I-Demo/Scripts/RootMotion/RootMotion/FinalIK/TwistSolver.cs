	...
	public class TwistSolver
	{
		...
		public void Relax()
		{
			if (inititated && !(weight <= 0f))
			{
				...
				Vector3 vector = quaternion * parent.rotation * axisRelativeToParentDefault;
				vector = Quaternion.FromToRotation(transform.position - parent.position, children[0].position - transform.position) * vector;
				Vector3 b = quaternion * children[0].rotation * axisRelativeToChildDefault;
				Vector3 vector2 = Vector3.Slerp(vector, b, parentChildCrossfade);
				vector2 = Quaternion.Inverse(Quaternion.LookRotation(rotation * axis, rotation * twistAxis)) * vector2;
