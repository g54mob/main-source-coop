...
namespace Edgegap.Codice.Utils
{
	public class HttpEncoder
	{
		...
		private static HttpEncoder currentEncoder = defaultEncoder;
		...
		internal static void UrlEncodeChar(char c, Stream result, bool isUnicode)
		{
			if (c > 'ÿ')
			{
				...
				int num2 = ((int)c >> 8) & 0xF;
				result.WriteByte((byte)hexChars[num2]);
				int num3 = ((int)c >> 4) & 0xF;
				result.WriteByte((byte)hexChars[num3]);
				int num4 = c & 0xF;
				result.WriteByte((byte)hexChars[num4]);
			}
			...
			else if (c < '0' || (c < 'A' && c > '9') || (c > 'Z' && c < 'a') || c > 'z')
			{
				...
				int num5 = (int)c >> 4;
				result.WriteByte((byte)hexChars[num5]);
				int num6 = c & 0xF;
				result.WriteByte((byte)hexChars[num6]);
			}
			else
			{
				result.WriteByte((byte)c);
