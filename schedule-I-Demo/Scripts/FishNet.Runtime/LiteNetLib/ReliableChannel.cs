...
namespace LiteNetLib
{
	internal sealed class ReliableChannel : BaseChannel
	{
		...
		protected override bool SendNextPackets()
		{
			...
			{
				NetPacket result;
				while (!OutgoingQueue.IsEmpty && NetUtils.RelativeSequenceNumber(_localSeqence, _localWindowStart) < _windowSize && OutgoingQueue.TryDequeue(out result))
				{
					result.Sequence = (ushort)_localSeqence;
					result.ChannelId = _id;
					_pendingPackets[_localSeqence % _windowSize].Init(result);
