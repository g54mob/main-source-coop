...
namespace LiteNetLib
{
	public class NetManager : IEnumerable<NetPeer>, IEnumerable
	{
		...
		private void CreateEvent(NetEvent.EType type, NetPeer peer = null, IPEndPoint remoteEndPoint = null, SocketError errorCode = SocketError.Success, int latency = 0, DisconnectReason disconnectReason = DisconnectReason.ConnectionFailed, ConnectionRequest connectionRequest = null, DeliveryMethod deliveryMethod = DeliveryMethod.Unreliable, byte channelNumber = 0, NetPacket readerSource = null, object userData = null)
		{
			...
			{
				_netEventsProduceQueue.Enqueue(netEvent);
			}
		}
		...
		public void PollEvents()
		{
			...
			else if (!UnsyncedEvents)
			{
				lock (_netEventsProduceQueue)
				{
					Queue<NetEvent> netEventsProduceQueue = _netEventsProduceQueue;
					Queue<NetEvent> netEventsConsumeQueue = _netEventsConsumeQueue;
					_netEventsConsumeQueue = netEventsProduceQueue;
					_netEventsProduceQueue = netEventsConsumeQueue;
				}
		...
		private void ManualReceive(Socket socket, EndPoint bufferEndPoint)
		{
			try
			{
				...
				while (socket.Available > 0)
				{
					...
					netPacket.Size = socket.ReceiveFrom(netPacket.RawData, 0, NetConstants.MaxPacketSize, SocketFlags.None, ref bufferEndPoint);
		...
		{
			Socket obj = (Socket)state;
			IntPtr handle = obj.Handle;
			byte[] array = new byte[(obj.AddressFamily == AddressFamily.InterNetwork) ? 16 : 28];
			int socketAddressSize = array.Length;
			NetPacket netPacket = PoolGetPacket(NetConstants.MaxPacketSize);
			while (IsActive())
			{
				netPacket.Size = NativeSocket.RecvFrom(handle, netPacket.RawData, NetConstants.MaxPacketSize, array, ref socketAddressSize);
				if (netPacket.Size == 0)
				{
					break;
				}
				if (netPacket.Size == -1)
				{
					...
					if (socketError != SocketError.WouldBlock && socketError != SocketError.TimedOut && ProcessError(new SocketException((int)socketError)))
					{
						break;
					}
					continue;
				}
				NativeAddr key = new NativeAddr(array, socketAddressSize);
				if (!_nativeAddrMap.TryGetValue(key, out var value))
				{
					value = new NativeEndPoint(array);
				}
				OnMessageReceived(netPacket, value);
				netPacket = PoolGetPacket(NetConstants.MaxPacketSize);
			}
		}
		...
		private void ReceiveLogic(object state)
		{
			...
			while (IsActive())
			{
				try
				{
					...
					{
						NetPacket netPacket = PoolGetPacket(NetConstants.MaxPacketSize);
						netPacket.Size = socket.ReceiveFrom(netPacket.RawData, 0, NetConstants.MaxPacketSize, SocketFlags.None, ref remoteEP);
						OnMessageReceived(netPacket, (IPEndPoint)remoteEP);
					}
				}
				catch (SocketException ex)
				{
		...
		public bool Start(IPAddress addressIPv4, IPAddress addressIPv6, int port, bool manualMode)
		{
			...
			if (!BindSocket(_udpSocketv4, new IPEndPoint(flag ? addressIPv6 : addressIPv4, port)))
			{
				return false;
			}
			...
			if (!_manualMode)
			{
				ParameterizedThreadStart start = ReceiveLogic;
				if (UseNativeSockets)
				{
					start = NativeReceiveLogic;
				}
				_threadv4 = new Thread(start)
				{
			...
			if (IPv6Support && IPv6Mode == IPv6Mode.SeparateSocket)
			{
				...
				if (BindSocket(_udpSocketv6, new IPEndPoint(addressIPv6, LocalPort)))
				{
					...
					{
						ParameterizedThreadStart start2 = ReceiveLogic;
						if (UseNativeSockets)
						{
							start2 = NativeReceiveLogic;
						}
						_threadv6 = new Thread(start2)
						{
		...
		private bool BindSocket(Socket socket, IPEndPoint ep)
		{
			...
			if (ep.AddressFamily == AddressFamily.InterNetwork || IPv6Mode == IPv6Mode.DualMode)
			{
				...
				if (IPv6Mode == IPv6Mode.DualMode)
				{
					...
					{
						NetDebug.WriteError($"[B]Bind exception (dualmode setting): {arg}");
					}
				}
