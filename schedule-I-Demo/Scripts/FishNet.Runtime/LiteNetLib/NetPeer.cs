			...
			if (p.IsFragmented)
			{
				...
				for (int i = 0; i < value.ReceivedCount; i++)
				{
					...
					if (num + num2 > netPacket.RawData.Length)
					{
						...
						NetDebug.WriteError("Fragment error pos: {0} >= resultPacketSize: {1} , totalSize: {2}", num + num2, netPacket.RawData.Length, value.TotalSize);
						return;
					}
					if (netPacket2.Size > netPacket2.RawData.Length)
					{
						_holdedFragments.Remove(fragmentId);
						NetDebug.WriteError("Fragment error size: {0} > fragment.RawData.Length: {1}", netPacket2.Size, netPacket2.RawData.Length);
						return;
					}
