...
namespace FishNet.Managing.Scened
{
	public class SceneLookupData : IEquatable<SceneLookupData>
	{
		...
		public static SceneLookupData[] CreateData(Scene[] scenes)
		{
			...
			if (flag)
			{
				NetworkManager.StaticLogWarning("One or more scene information entries contain invalid data and have been skipped.");
			}
			return list.ToArray();
		}
		...
		public static SceneLookupData[] ValidateData(SceneLookupData[] datas)
		{
			...
			foreach (SceneLookupData sceneLookupData in datas)
			{
				if (sceneLookupData.IsValid)
				{
					...
					if (num != -1)
					{
						NetworkManager.StaticLogWarning("Data " + sceneLookupData.ToString() + " matches " + list[num].ToString() + " and has been removed from datas.");
					}
					else
					{
