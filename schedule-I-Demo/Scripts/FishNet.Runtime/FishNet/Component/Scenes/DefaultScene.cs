...
namespace FishNet.Component.Scenes
{
	...
	public class DefaultScene : MonoBehaviour
	{
		...
		{
			if (_enabled && !ApplicationState.IsQuitting() && _networkManager != null && _networkManager.Initialized)
			{
		...
		private void InitializeOnce()
		{
			...
			{
				NetworkManager.StaticLogError("NetworkManager not found on " + base.gameObject.name + " or any parent objects. DefaultScene will not work.");
			}
			else
			{
		...
		{
			if (obj.ConnectionState == LocalConnectionState.Stopped && !_networkManager.IsServer)
			{
				LoadOfflineScene();
			}
		}
