...
namespace Pathfinding
{
	public class PointNode : GraphNode
	{
		...
		public override void AddLooseConnection(GraphNode node, uint cost)
		{
			...
			if (looseConnections != null)
			{
				for (int i = 0; i < looseConnections.Length; i++)
				{
					if (looseConnections[i].node == node)
					{
						looseConnections[i].cost = cost;
						return;
					}
				}
			}
			int num = ((looseConnections != null) ? looseConnections.Length : 0);
			Connection[] array = new Connection[num + 1];
			for (int j = 0; j < num; j++)
			{
				array[j] = looseConnections[j];
			}
			array[num] = new Connection(node, cost);
			looseConnections = array;
			AstarPath.active.hierarchicalGraph.AddDirtyNode(this);
			(base.Graph as PointGraph).RegisterConnectionLength((node.position - position).sqrMagnitudeLong);
		}
		...
		public override void RemoveLooseConnection(GraphNode node)
		{
			...
			for (int i = 0; i < looseConnections.Length; i++)
			{
				if (looseConnections[i].node == node)
				{
					int num = looseConnections.Length;
					Connection[] array = new Connection[num - 1];
					for (int j = 0; j < i; j++)
					{
						array[j] = looseConnections[j];
