...
namespace ScheduleOne.NPCs.Behaviour
{
	public class CheckpointBehaviour : Behaviour
	{
		...
		{
			if (!InstanceFinder.IsServer || player.CrimeData.TimeSinceLastBodySearch < 45f || player.CrimeData.CurrentPursuitLevel != PlayerCrimeData.EPursuitLevel.None || NetworkSingleton<CurfewManager>.Instance.IsCurrentlyActive || Checkpoint.AssignedNPCs.Count == 0)
			{
				return;
			}
			NPC nPC = null;
