...
public class StingrayAI : EnemyAI
{
	...
	public void CreateSlimeRpc(Vector3 slimePos)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			if (__rpc_exec_stage != __RpcExecStage.Execute)
			{
				...
				FastBufferWriter bufferWriter = __beginSendRpc(1072288845u, rpcParams, attributeParams, SendTo.NotMe, RpcDelivery.Reliable);
				bufferWriter.WriteValueSafe(in slimePos);
				__endSendRpc(ref bufferWriter, 1072288845u, rpcParams, attributeParams, SendTo.NotMe, RpcDelivery.Reliable);
			}
			if (__rpc_exec_stage == __RpcExecStage.Execute)
			{
	...
	public void ShowSpitOnPlayerServerRpc(int playerId)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			if (__rpc_exec_stage != __RpcExecStage.Execute && (networkManager.IsClient || networkManager.IsHost))
			{
				...
				FastBufferWriter bufferWriter = __beginSendServerRpc(746352033u, serverRpcParams, RpcDelivery.Reliable);
				BytePacker.WriteValueBitPacked(bufferWriter, playerId);
				__endSendServerRpc(ref bufferWriter, 746352033u, serverRpcParams, RpcDelivery.Reliable);
			}
	...
	public void ShowSpitOnPlayerClientRpc(int playerId)
	{
		...
		if (__rpc_exec_stage != __RpcExecStage.Execute && (networkManager.IsServer || networkManager.IsHost))
		{
			...
			FastBufferWriter bufferWriter = __beginSendClientRpc(3680151288u, clientRpcParams, RpcDelivery.Reliable);
			BytePacker.WriteValueBitPacked(bufferWriter, playerId);
			__endSendClientRpc(ref bufferWriter, 3680151288u, clientRpcParams, RpcDelivery.Reliable);
		}
		if (__rpc_exec_stage == __RpcExecStage.Execute && (networkManager.IsClient || networkManager.IsHost))
		{
			__rpc_exec_stage = __RpcExecStage.Send;
	...
	{
		__registerRpc(1072288845u, __rpc_handler_1072288845, "CreateSlimeRpc");
		__registerRpc(746352033u, __rpc_handler_746352033, "ShowSpitOnPlayerServerRpc");
		__registerRpc(3680151288u, __rpc_handler_3680151288, "ShowSpitOnPlayerClientRpc");
		base.__initializeRpcs();
	}

	private static void __rpc_handler_1072288845(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			((StingrayAI)target).CreateSlimeRpc(value);
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}

	private static void __rpc_handler_746352033(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		NetworkManager networkManager = target.NetworkManager;
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			target.__rpc_exec_stage = __RpcExecStage.Execute;
			((StingrayAI)target).ShowSpitOnPlayerServerRpc(value);
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}

	private static void __rpc_handler_3680151288(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		NetworkManager networkManager = target.NetworkManager;
		if ((object)networkManager != null && networkManager.IsListening)
		{
			ByteUnpacker.ReadValueBitPacked(reader, out int value);
			target.__rpc_exec_stage = __RpcExecStage.Execute;
			((StingrayAI)target).ShowSpitOnPlayerClientRpc(value);
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}
