...
public class SprayPaintItem : GrabbableObject
{
	...
	{
		__registerRpc(853698447u, __rpc_handler_853698447, "SyncKillingWeedRpc");
		__registerRpc(2181546544u, __rpc_handler_2181546544, "SyncKillingCadaverPlantRpc");
		__registerRpc(3342850939u, __rpc_handler_3342850939, "StopKillingCadaverPlantServerRpc");
		__registerRpc(1830928072u, __rpc_handler_1830928072, "StopKillingCadaverPlantClientRpc");
		__registerRpc(3462977352u, __rpc_handler_3462977352, "StopKillingWeedServerRpc");
		__registerRpc(1040528291u, __rpc_handler_1040528291, "StopKillingWeedClientRpc");
		__registerRpc(96399641u, __rpc_handler_96399641, "KillWeedRpc");
		__registerRpc(2604920715u, __rpc_handler_2604920715, "KillCadaverPlantRpc");
		__registerRpc(629055349u, __rpc_handler_629055349, "SprayPaintServerRpc");
		__registerRpc(3280104832u, __rpc_handler_3280104832, "SprayPaintClientRpc");
		base.__initializeRpcs();
	}

	private static void __rpc_handler_853698447(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			((SprayPaintItem)target).SyncKillingWeedRpc(value, value2);
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}

	private static void __rpc_handler_2181546544(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		NetworkManager networkManager = target.NetworkManager;
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			target.__rpc_exec_stage = __RpcExecStage.Execute;
			((SprayPaintItem)target).SyncKillingCadaverPlantRpc(value, value2, value3);
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}

	private static void __rpc_handler_3342850939(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		NetworkManager networkManager = target.NetworkManager;
		if ((object)networkManager != null && networkManager.IsListening)
		{
			target.__rpc_exec_stage = __RpcExecStage.Execute;
			((SprayPaintItem)target).StopKillingCadaverPlantServerRpc();
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}

	private static void __rpc_handler_1830928072(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		NetworkManager networkManager = target.NetworkManager;
		if ((object)networkManager != null && networkManager.IsListening)
		{
			target.__rpc_exec_stage = __RpcExecStage.Execute;
			((SprayPaintItem)target).StopKillingCadaverPlantClientRpc();
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}
	...
	private static void __rpc_handler_96399641(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		NetworkManager networkManager = target.NetworkManager;
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			target.__rpc_exec_stage = __RpcExecStage.Execute;
			((SprayPaintItem)target).KillWeedRpc(value);
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}

	private static void __rpc_handler_2604920715(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		NetworkManager networkManager = target.NetworkManager;
		if ((object)networkManager != null && networkManager.IsListening)
		{
			reader.ReadValueSafe(out Vector3 value);
			target.__rpc_exec_stage = __RpcExecStage.Execute;
			((SprayPaintItem)target).KillCadaverPlantRpc(value);
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}
