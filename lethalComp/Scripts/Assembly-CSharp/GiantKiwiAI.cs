...
public class GiantKiwiAI : EnemyAI, IVisibleThreat
{
	...
	public void DropScrapRpc(NetworkObjectReference item, Vector3 targetFloorPosition, bool droppedInNest, bool onShip, bool insideShipRoom)
	{
		...
		if (__rpc_exec_stage != __RpcExecStage.Execute)
		{
			...
			FastBufferWriter bufferWriter = __beginSendRpc(3498560222u, rpcParams, attributeParams, SendTo.NotMe, RpcDelivery.Reliable);
			bufferWriter.WriteValueSafe(in item, default(FastBufferWriter.ForNetworkSerializable));
			...
			bufferWriter.WriteValueSafe(in onShip, default(FastBufferWriter.ForPrimitives));
			bufferWriter.WriteValueSafe(in insideShipRoom, default(FastBufferWriter.ForPrimitives));
			__endSendRpc(ref bufferWriter, 3498560222u, rpcParams, attributeParams, SendTo.NotMe, RpcDelivery.Reliable);
		}
		if (__rpc_exec_stage == __RpcExecStage.Execute)
		{
			__rpc_exec_stage = __RpcExecStage.Send;
	...
	protected override void __initializeRpcs()
	{
		...
		__registerRpc(3498560222u, __rpc_handler_3498560222, "DropScrapRpc");
	...
	private static void __rpc_handler_3498560222(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		NetworkManager networkManager = target.NetworkManager;
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			target.__rpc_exec_stage = __RpcExecStage.Execute;
			((GiantKiwiAI)target).DropScrapRpc(value, value2, value3, value4, value5);
			target.__rpc_exec_stage = __RpcExecStage.Send;
		}
	}
