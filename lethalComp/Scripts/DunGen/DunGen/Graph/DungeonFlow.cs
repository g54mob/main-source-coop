	...
	public class DungeonFlow : ScriptableObject, ISerializationCallbackReceiver
	{
		...
		public bool CanDoorwaysConnect(ProposedConnection connection)
		{
			foreach (TileConnectionRule customConnectionRule in DoorwayPairFinder.CustomConnectionRules)
			{
				...
				if (customConnectionRule.ConnectionDelegate != null)
				{
					connectionResult = customConnectionRule.ConnectionDelegate(connection);
				}
				else if (customConnectionRule.Delegate != null)
				{
					connectionResult = customConnectionRule.Delegate(connection.PreviousTile.PrefabTile, connection.NextTile.PrefabTile, connection.PreviousDoorway.DoorwayComponent, connection.NextDoorway.DoorwayComponent);
				}
