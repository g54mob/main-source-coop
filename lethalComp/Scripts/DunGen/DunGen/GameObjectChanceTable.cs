...
namespace DunGen
{
	...
	public class GameObjectChanceTable
	{
		...
		public bool HasAnyValidEntries(bool isOnMainPath, float normalizedDepth, GameObject previouslyChosen, bool allowImmediateRepeats, bool allowNullSelection = false)
		{
			...
			foreach (GameObjectChance weight in Weights)
			{
				if (weight.Value != null && weight.GetWeight(isOnMainPath, normalizedDepth) > 0f && (allowImmediateRepeats || previouslyChosen != weight.Value))
				{
					result = true;
					break;
				}
