...
namespace DunGen.Adapters
{
	...
	public class UnityNavMeshAdapter : NavMeshAdapter
	{
		...
		public bool UseAutomaticLinkDistance;
		...
		private void BakeFullDungeon(Dungeon dungeon)
		{
			if (AutoGenerateFullRebakeSurfaces)
			{
				...
				NavMeshSurface[] componentsInChildren = dungeon.gameObject.GetComponentsInChildren<NavMeshSurface>();
		...
		private void AddNavMeshLink(DoorwayConnection connection, NavMeshAgentLinkInfo agentLinkInfo)
		{
			...
			{
				link.startPoint = gameObject.transform.InverseTransformPoint(GetClosestPointOnNavMesh(gameObject.transform.position, gameObject.transform.forward)) + new Vector3(0f, 0f, AutomaticLinkDistanceOffset);
				link.endPoint = gameObject.transform.InverseTransformPoint(GetClosestPointOnNavMesh(gameObject.transform.position, -gameObject.transform.forward)) - new Vector3(0f, 0f, AutomaticLinkDistanceOffset);
			}
			else
			{
		...
		private Vector3 GetClosestPointOnNavMesh(Vector3 point, Vector3? distanceDirection = null)
		{
			...
			for (int i = 2; i < navMeshTriangulation.indices.Length; i += 3)
			{
				Vector3 vector = navMeshTriangulation.vertices[navMeshTriangulation.indices[i - 2]];
				Vector3 vector2 = navMeshTriangulation.vertices[navMeshTriangulation.indices[i - 1]];
				Vector3 vector3 = navMeshTriangulation.vertices[navMeshTriangulation.indices[i]];
				Vector3 closestPointOnEdge = GetClosestPointOnEdge(point, vector, vector2);
				Vector3 closestPointOnEdge2 = GetClosestPointOnEdge(point, vector2, vector3);
				Vector3 closestPointOnEdge3 = GetClosestPointOnEdge(point, vector3, vector);
				float num2 = CalculateDistance(point, closestPointOnEdge);
				float num3 = CalculateDistance(point, closestPointOnEdge2);
				float num4 = CalculateDistance(point, closestPointOnEdge3);
				if (num2 < num)
				{
					num = num2;
				...
				{
					num = num3;
					result = closestPointOnEdge2;
				}
				if (num4 < num)
				{
					num = num4;
					result = closestPointOnEdge3;
				}
			}
			return result;
			float CalculateDistance(Vector3 p0, Vector3 p1)
			{
				if (!distanceDirection.HasValue)
				{
					return (p0 - p1).magnitude;
				}
				if (Vector3.Dot(p1 - p0, distanceDirection.Value) <= 0f)
				{
		...
		{
			Vector3 vector = edgePointB - edgePointA;
			float magnitude = vector.magnitude;
			vector.Normalize();
			float num = Mathf.Clamp(Vector3.Dot(referencePoint - edgePointA, vector), 0f, magnitude);
			return edgePointA + vector * num;
		}
	}
}
