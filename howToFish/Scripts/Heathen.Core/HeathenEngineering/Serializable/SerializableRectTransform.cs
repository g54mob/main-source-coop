...
namespace HeathenEngineering.Serializable
{
	...
	public class SerializableRectTransform : SerializableTransform
	{
		...
		public SerializableRectTransform(Transform transform)
			: base(transform)
		{
			anchorMax = new float2(0.5f, 0.5f);
			anchorMin = new float2(0.5f, 0.5f);
		...
		public SerializableRectTransform(float3 position, quaternion rotation, float3 localScale, float2 anchorMin, float2 anchorMax, float2 pivot)
			: base(position, rotation, localScale)
		{
