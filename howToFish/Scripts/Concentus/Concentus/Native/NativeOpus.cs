...
namespace Concentus.Native
{
	internal class NativeOpus
	{
		...
		{
			if (NativePlatformUtils.PrepareNativeLibrary("opus", logger) == NativeLibraryStatus.Available)
			{
		...
		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern NativeOpusMultistreamDecoder opus_multistream_decoder_create(int Fs, int channels, int streams, int coupled_streams, byte* mapping, out int error);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern int opus_decode(NativeOpusDecoder st, byte* data, int len, short* pcm, int frame_size, int decode_fec);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern int opus_decode_float(NativeOpusDecoder st, byte* data, int len, float* pcm, int frame_size, int decode_fec);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern int opus_multistream_decode(NativeOpusMultistreamDecoder st, byte* data, int len, short* pcm, int frame_size, int decode_fec);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern int opus_multistream_decode_float(NativeOpusMultistreamDecoder st, byte* data, int len, float* pcm, int frame_size, int decode_fec);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal static extern NativeOpusEncoder opus_encoder_create(int Fs, int channels, int application, out int error);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern NativeOpusMultistreamEncoder opus_multistream_surround_encoder_create(int Fs, int channels, int mapping_family, out int streams, out int coupled_streams, byte* mapping, int application, out int error);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern int opus_encode(NativeOpusEncoder st, short* pcm, int frame_size, byte* data, int max_data_bytes);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern int opus_encode_float(NativeOpusEncoder st, float* pcm, int frame_size, byte* data, int max_data_bytes);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern int opus_multistream_encode(NativeOpusMultistreamEncoder st, short* pcm, int frame_size, byte* data, int max_data_bytes);

		[DllImport("opus", CallingConvention = CallingConvention.Cdecl)]
		internal unsafe static extern int opus_multistream_encode_float(NativeOpusMultistreamEncoder st, float* pcm, int frame_size, byte* data, int max_data_bytes);
