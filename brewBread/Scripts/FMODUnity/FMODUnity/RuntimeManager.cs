...
namespace FMODUnity
{
	...
	public class RuntimeManager : MonoBehaviour
	{
		...
		private static RESULT ERROR_CALLBACK(IntPtr system, FMOD.SYSTEM_CALLBACK_TYPE type, IntPtr commanddata1, IntPtr commanddata2, IntPtr userdata)
		{
			...
			if (eRRORCALLBACK_INFO.instancetype == ERRORCALLBACK_INSTANCETYPE.STUDIO_EVENTINSTANCE && eRRORCALLBACK_INFO.functionname == "EventInstance::set3DAttributes" && eRRORCALLBACK_INFO.result == RESULT.ERR_INVALID_HANDLE)
			{
				return RESULT.OK;
			}
		...
		private void Update()
		{
			...
			for (int i = 0; i < attachedInstances.Count; i++)
			{
				...
				else if ((bool)attachedInstances[i].rigidBody2D)
				{
		...
		{
			if (HaveMasterBanksLoaded && StudioSystem.getBus("bus:/", out var bus) == RESULT.OK)
			{
