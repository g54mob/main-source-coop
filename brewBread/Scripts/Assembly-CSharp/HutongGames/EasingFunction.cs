...
namespace HutongGames
{
	public class EasingFunction
	{
		...
		public static float Punch(float start, float end, float value)
		{
			...
			return Mathf.Pow(2f, -10f * value) * Mathf.Sin(value * ((float)Math.PI * 2f) / 0.15f);
		}

		public static float PunchD(float start, float end, float value)
		{
			return (0f - (10f * Mathf.Log(2f) * 0.15f * Mathf.Sin((float)Math.PI * 2f * value / 0.15f) - (float)Math.PI * 2f * Mathf.Cos((float)Math.PI * 2f * value / 0.15f))) / Mathf.Pow(0.3f, 10f * value);
		}
