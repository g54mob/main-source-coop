...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_Accelerometer : CustomCalculation
	{
		...
		internal bool HVXDdakdehEYoKOjqYfCCUkUvAyI()
		{
			...
			if (base.DataCount >= 1)
			{
				...
				case CalculationType.Pitch:
					_result = aXaKYSEzeNMeYmPzHBoIkQNYokukA();
					flag = true;
					break;
				case CalculationType.Roll:
					_result = OmaAKockFIiVajHrJLmmAAMkARqec();
					flag = true;
					break;
				}
			}
		...
		private float aXaKYSEzeNMeYmPzHBoIkQNYokukA()
		{
			...
			return _outputType switch
			{
				...
				OutputType.Axis => oiXdProDsERvJdkZUAPcWxlfEYQW(num2), 
				_ => 0f, 
			};
		}

		private float OmaAKockFIiVajHrJLmmAAMkARqec()
		{
			Vector3 vector = default(Vector3);
			int num = MathTools.Min(base.DataCount, 3);
			for (int i = 0; i < num; i++)
		...
		private float oiXdProDsERvJdkZUAPcWxlfEYQW(float P_0)
		{
			if (P_0 == 0f)
			{
				return 0f;
