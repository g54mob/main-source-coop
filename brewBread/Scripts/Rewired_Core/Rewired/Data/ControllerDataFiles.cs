...
namespace Rewired.Data
{
	public sealed class ControllerDataFiles : ScriptableObject
	{
		...
		[NonSerialized]
		private bool kIYovWzhdTIKVHftiOVFxvPLeXrm;
		...
		internal ControllerTemplateElementIdentifier EUZRwgQIgYtQoFlxWBOXLFnVfPwl(Guid P_0, int P_1)
		{
			if (P_1 < 0)
			{
				return null;
			...
			foreach (Guid templateGuid in hardwareJoystickMap.TemplateGuids)
			{
				...
				{
					ControllerTemplateElementIdentifier controllerTemplateElementIdentifier = joystickTemplate.TQKQOcjTcFmNzxDgybZxLvFfTrkC(P_0, P_1);
		...
		internal HardwareJoystickMap_InputManager igyTzPwKAtUKQePinKyePYFeMWpS(Guid P_0, InputSource P_1)
		{
			hDWcPcAMSTBgWuXOoABfbCNmQJEs();
			...
			if (hardwareJoystickMap != null)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = iMpeSJkzGihLBfkSFVWvavOWGDtr(hardwareJoystickMap, bridgedController, true, out inputPlatform, out num, out platform);
		...
		internal HardwareJoystickMap_InputManager QybEKscbrtSTeOWvpcJxVwbuuRGnA(BridgedControllerHWInfo P_0)
		{
			if (P_0 == null)
			{
				return null;
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = xRkffNJUDhwycCKhxflaknQToJlEA(P_0);
				if (hardwareJoystickMap_InputManager != null)
				{
					return hardwareJoystickMap_InputManager;
				}
			...
			for (int i = 0; i < hardwareJoystickMaps.Length; i++)
			{
				...
				int num;
				HardwareJoystickMap.Platform platform;
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager2 = iMpeSJkzGihLBfkSFVWvavOWGDtr(hardwareJoystickMaps[i], P_0, true, out inputPlatform, out num, out platform);
			...
			for (int j = 0; j < hardwareJoystickMaps.Length; j++)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager3 = iMpeSJkzGihLBfkSFVWvavOWGDtr(hardwareJoystickMaps[j], P_0, false, out inputPlatform2, out num2, out platform2);
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager4 = DOTARxVKUszxeQgDyGmMvOlDLZQG(P_0, "[UNITY PRECONFIGURED JOYSTICK]");
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager5 = xRkffNJUDhwycCKhxflaknQToJlEA(P_0);
		...
		private HardwareJoystickMap_InputManager iMpeSJkzGihLBfkSFVWvavOWGDtr(HardwareJoystickMap P_0, BridgedControllerHWInfo P_1, bool P_2, out InputPlatform P_3, out int P_4, out HardwareJoystickMap.Platform P_5)
		{
		...
		private HardwareJoystickMap_InputManager DOTARxVKUszxeQgDyGmMvOlDLZQG(BridgedControllerHWInfo P_0, string P_1)
		{
		...
		private HardwareJoystickMap_InputManager xRkffNJUDhwycCKhxflaknQToJlEA(BridgedControllerHWInfo P_0)
		{
			HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = DOTARxVKUszxeQgDyGmMvOlDLZQG(P_0, "[STANDARDIZED GAMEPAD]");
			if (hardwareJoystickMap_InputManager == null)
			{
				return null;
			}
		...
		private void hDWcPcAMSTBgWuXOoABfbCNmQJEs()
		{
			if (!kIYovWzhdTIKVHftiOVFxvPLeXrm)
			{
				...
				{
					kIYovWzhdTIKVHftiOVFxvPLeXrm = true;
				}
			}
		}
	}
