...
namespace Rewired.Data
{
	...
	public sealed class Player_Editor
	{
		...
		public sealed class Mapping
		{
			...
			internal skUrVfnSYVIqLSMOQKrWFMoXjfWy leCRxbzLXHstujZFfxpfxRAhcfRo()
			{
				return new skUrVfnSYVIqLSMOQKrWFMoXjfWy(_categoryId, _layoutId, _enabled);
			}
		}
		...
		public sealed class ControllerMapLayoutManagerSettings : IDeepCloneable
		{
			...
			internal ControllerMapLayoutManager.iZBHTLMJEscsCxujGlXwPdwYSOZv yMMoRTmlFkeyjRZhjJmIJpImkMHJ()
			{
				return new ControllerMapLayoutManager.iZBHTLMJEscsCxujGlXwPdwYSOZv(_enabled, _loadFromUserDataStore, pBFUgMuZGVbXOxmTemAPDZAivczt());
			}

			private vNnAsbNOxQRuVRxCcHhnyysOuoGy[] pBFUgMuZGVbXOxmTemAPDZAivczt()
			{
				List<vNnAsbNOxQRuVRxCcHhnyysOuoGy> list = new List<vNnAsbNOxQRuVRxCcHhnyysOuoGy>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
				{
					if (_ruleSets[i] != null)
					{
						list.Add(_ruleSets[i].OxIfdrHgBXrLvKJsKxayQFwmyaqG());
					}
				}
				return list.ToArray();
			}
		...
		public sealed class ControllerMapEnablerSettings : IDeepCloneable
		{
			...
			internal ControllerMapEnabler.oejthPnlmICeXCzkndFXvfozHuLjA yMMoRTmlFkeyjRZhjJmIJpImkMHJ()
			{
				return new ControllerMapEnabler.oejthPnlmICeXCzkndFXvfozHuLjA(_enabled, pBFUgMuZGVbXOxmTemAPDZAivczt());
			}
		...
		public sealed class RuleSetMapping : IDeepCloneable
		{
			...
			internal vNnAsbNOxQRuVRxCcHhnyysOuoGy OxIfdrHgBXrLvKJsKxayQFwmyaqG()
			{
				return new vNnAsbNOxQRuVRxCcHhnyysOuoGy(_id, _enabled);
			}
		...
		internal ypwUncqYrrgWARbofZIocbdoyJzm bPCRfTsZBWAqvvaTkOFDsaqvDzII()
		{
			skUrVfnSYVIqLSMOQKrWFMoXjfWy[] array = null;
			if (_defaultJoystickMaps != null)
			{
				array = new skUrVfnSYVIqLSMOQKrWFMoXjfWy[_defaultJoystickMaps.Count];
				for (int i = 0; i < _defaultJoystickMaps.Count; i++)
				{
					array[i] = _defaultJoystickMaps[i].leCRxbzLXHstujZFfxpfxRAhcfRo();
				}
			}
			skUrVfnSYVIqLSMOQKrWFMoXjfWy[] array2 = null;
			if (_defaultKeyboardMaps != null)
			{
				array2 = new skUrVfnSYVIqLSMOQKrWFMoXjfWy[_defaultKeyboardMaps.Count];
				for (int j = 0; j < _defaultKeyboardMaps.Count; j++)
				{
					array2[j] = _defaultKeyboardMaps[j].leCRxbzLXHstujZFfxpfxRAhcfRo();
				}
			}
			skUrVfnSYVIqLSMOQKrWFMoXjfWy[] array3 = null;
			if (_defaultMouseMaps != null)
			{
				array3 = new skUrVfnSYVIqLSMOQKrWFMoXjfWy[_defaultMouseMaps.Count];
				for (int k = 0; k < _defaultMouseMaps.Count; k++)
				{
					array3[k] = _defaultMouseMaps[k].leCRxbzLXHstujZFfxpfxRAhcfRo();
				}
			}
			skUrVfnSYVIqLSMOQKrWFMoXjfWy[] array4 = null;
			if (_defaultCustomControllerMaps != null)
			{
				array4 = new skUrVfnSYVIqLSMOQKrWFMoXjfWy[_defaultCustomControllerMaps.Count];
				for (int l = 0; l < _defaultCustomControllerMaps.Count; l++)
				{
					array4[l] = _defaultCustomControllerMaps[l].leCRxbzLXHstujZFfxpfxRAhcfRo();
				}
			}
			return new ypwUncqYrrgWARbofZIocbdoyJzm(array, array2, array3, array4);
		}
	}
}
