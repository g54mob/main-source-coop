...
namespace Rewired.Utils
{
	...
	{
		private static string urTiAGpyTdYIpEfryRFChszmVFAG;
		...
		static StringTools()
		{
			urTiAGpyTdYIpEfryRFChszmVFAG = Regex.Escape(new string(Path.GetInvalidFileNameChars()));
		}

		public static string CleanUpFileName(string name)
		{
			name = name.Trim();
			string pattern = "[ ~`,:;'\\.\\$\\^\\{\\}\\[\\]\\(\\|\\)\\*\\+\\?\\\\" + urTiAGpyTdYIpEfryRFChszmVFAG + "]";
