...
namespace Rewired.Utils
{
	...
	{
		private static byte[] gSvneBQmqcPdRsiJxIvzzXwlyHsv;
		...
		public static bool FillMemory(IntPtr buffer, int startIndex, int length, byte value, bool throwOnError = true)
		{
			...
			if (value != 0)
			{
				if (gSvneBQmqcPdRsiJxIvzzXwlyHsv == null)
				{
					gSvneBQmqcPdRsiJxIvzzXwlyHsv = new byte[8];
				}
				bool flag = false;
				if (num >= 8)
				{
					long val;
					lock (gSvneBQmqcPdRsiJxIvzzXwlyHsv)
					{
						for (int i = 0; i < 8; i++)
						{
							gSvneBQmqcPdRsiJxIvzzXwlyHsv[i] = value;
						}
						flag = true;
						val = BitConverter.ToInt64(gSvneBQmqcPdRsiJxIvzzXwlyHsv, 0);
					}
					int num2 = length / 8 * 8;
				...
				if (num >= 4)
				{
					...
					lock (gSvneBQmqcPdRsiJxIvzzXwlyHsv)
					{
						if (!flag)
						{
							for (int k = 0; k < 4; k++)
							{
								gSvneBQmqcPdRsiJxIvzzXwlyHsv[k] = value;
							}
							flag = true;
						}
						val2 = BitConverter.ToInt32(gSvneBQmqcPdRsiJxIvzzXwlyHsv, 0);
					}
					int num3 = length / 4 * 4;
				...
				if (num >= 2)
				{
					...
					lock (gSvneBQmqcPdRsiJxIvzzXwlyHsv)
					{
						if (!flag)
						{
							for (int m = 0; m < 2; m++)
							{
								gSvneBQmqcPdRsiJxIvzzXwlyHsv[m] = value;
							}
							flag = true;
						}
						val3 = BitConverter.ToInt16(gSvneBQmqcPdRsiJxIvzzXwlyHsv, 0);
					}
					int num4 = length / 2 * 2;
