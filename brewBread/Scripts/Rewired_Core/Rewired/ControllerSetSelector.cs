...
namespace Rewired
{
	...
	public sealed class ControllerSetSelector : ISerializationCallbackReceiver, IDeepCloneable
	{
		...
		[NonSerialized]
		private Guid ogxUreeXNLqmoemRiQVPimdeNdIl;

		internal bool IRxuBlsWPAUhvrFlMvUAiDQPeqYAA => _type != Type.All;
		...
		public Guid hardwareTypeGuid
		{
			get
			{
				...
				return ogxUreeXNLqmoemRiQVPimdeNdIl;
			}
			set
			{
				...
				ogxUreeXNLqmoemRiQVPimdeNdIl = value;
		...
		public ControllerSetSelector(ControllerSetSelector P_0)
		{
			...
			ogxUreeXNLqmoemRiQVPimdeNdIl = P_0.ogxUreeXNLqmoemRiQVPimdeNdIl;
		}
		...
		internal ControllerSetSelector(Type P_0, ControllerType P_1, string P_2, string P_3, int P_4)
		{
			...
			_controllerType = P_1;
			_guid = P_2;
			ogxUreeXNLqmoemRiQVPimdeNdIl = StringTools.ToGuid(P_2);
		...
		public bool Matches(Controller controller)
		{
			...
			case Type.HardwareType:
				if (ogxUreeXNLqmoemRiQVPimdeNdIl != Guid.Empty)
				{
					return ogxUreeXNLqmoemRiQVPimdeNdIl == controller.hardwareTypeGuid;
				}
			...
			case Type.ControllerTemplateType:
				return controller.ImplementsTemplate(ogxUreeXNLqmoemRiQVPimdeNdIl);
			case Type.PersistentControllerInstance:
				return controller.deviceInstanceGuid == ogxUreeXNLqmoemRiQVPimdeNdIl;
		...
		private void xFNIJTPQwdXXTZgChPlhlVhgJESb()
		{
			_guid = string.Empty;
			ogxUreeXNLqmoemRiQVPimdeNdIl = Guid.Empty;
		}

		void ISerializationCallbackReceiver.OnAfterDeserialize()
		{
			ogxUreeXNLqmoemRiQVPimdeNdIl = StringTools.ToGuid(_guid);
		}
