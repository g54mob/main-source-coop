...
internal sealed class CinemachineMixer : PlayableBehaviour
{
	...
	private float mLastOverrideTime;
	...
	private float GetDeltaTime(float deltaTime)
	{
		if (!mPlaying)
		{
			...
			{
				mLastOverrideTime = -1f;
			}
			if (!Application.isPlaying)
			{
				deltaTime = Time.unscaledDeltaTime;
				float realtimeSinceStartup = Time.realtimeSinceStartup;
				if (mLastOverrideTime < 0f || realtimeSinceStartup - mLastOverrideTime > Time.maximumDeltaTime * 5f)
				{
					deltaTime = -1f;
				}
				mLastOverrideTime = realtimeSinceStartup;
			}
		}
		return deltaTime;
	}
