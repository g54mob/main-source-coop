	...
	public class CinemachineComposer : CinemachineComponentBase
	{
		...
		public override void MutateCameraState(ref CameraState curState, float deltaTime)
		{
			...
			if (deltaTime < 0f || !base.VirtualCamera.PreviousStateIsValid)
			{
				...
				RotateToScreenBounds(ref curState, screenRect, curState.ReferenceLookAt, ref rigOrientation, mCache.mFov, mCache.mFovH, -1f);
			}
			else
			{
				Vector3 vector2 = m_LookAtPrevFrame - (m_CameraPosPrevFrame + curState.PositionDampingBypass);
				...
				{
					rigOrientation = Quaternion.LookRotation(vector2, curState.ReferenceUp);
					rigOrientation = rigOrientation.ApplyCameraRotation(-m_ScreenOffsetPrevFrame, curState.ReferenceUp);
				}
				RotateToScreenBounds(ref curState, mCache.mFovHardGuideRect, TrackedPoint, ref rigOrientation, mCache.mFov, mCache.mFovH, -1f);
				RotateToScreenBounds(ref curState, mCache.mFovSoftGuideRect, TrackedPoint, ref rigOrientation, mCache.mFov, mCache.mFovH, deltaTime);
			}
			if (!(TrackedPoint - curState.ReferenceLookAt).AlmostZero())
			{
				RotateToScreenBounds(ref curState, mCache.mFovHardGuideRect, curState.ReferenceLookAt, ref rigOrientation, mCache.mFov, mCache.mFovH, -1f);
			}
