...
namespace Photon.Voice
{
	public class WebRTCAudioProcessor : WebRTCAudioLib, IProcessor<short>, IDisposable
	{
		...
		private void InitReverseStream()
		{
			lock (this)
			{
				if (!aecInited && !disposed)
				{
					...
					reverseBufferFactory = new FactoryPrimitiveArrayPool<short>(50, "WebRTCAudioProcessor Reverse Buffers", num);
		...
		public short[] Process(short[] buf)
		{
			...
			{
				logger.LogError("[PV] WebRTCAudioProcessor Process: frame size expected: {0}, passed: {1}", inFrameSize, buf.Length);
				return buf;
			}
			bool flag = false;
