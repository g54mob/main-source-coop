	...
	public class MicWrapper : IAudioReader<float>, IDataReader<float>, IDisposable, IAudioDesc
	{
		...
		public bool Read(float[] buffer)
		{
			...
			if (num3 < num)
			{
				...
				if (Singleton<RecorderAudioListener>.Instance != null)
				{
					Singleton<RecorderAudioListener>.Instance.SendMic(buffer, SamplingRate, Channels);
				}
				if (Singleton<MicrophoneRelay>.Instance != null)
				{
					Singleton<MicrophoneRelay>.Instance.SendMic(buffer);
				}
				readAbsPos = num3;
				return true;
			}
