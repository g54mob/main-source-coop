...
namespace ExitGames.Client.Photon
{
	public class Protocol18 : IProtocol
	{
		...
		private GpType GetCodeOfType(Type type)
		{
			...
			if ((object)type == typeof(StructWrapper<>))
			{
			...
			if (type.IsArray)
			{
				...
				if ((object)elementType == typeof(object) || (object)elementType == typeof(StructWrapper))
				{
					return GpType.ObjectArray;
				}
				if ((object)elementType == typeof(Hashtable))
				{
					return GpType.HashtableArray;
				}
				if (elementType.IsGenericType && (object)typeof(Dictionary<, >) == elementType.GetGenericTypeDefinition())
				{
			...
			if ((object)type == typeof(List<object>))
			{
				return GpType.ObjectArray;
			}
			if (type.IsGenericType && (object)typeof(Dictionary<, >) == type.GetGenericTypeDefinition())
			{
		...
		private void WriteDictionaryHeader(StreamBuffer stream, Type type, out GpType keyWriteType, out GpType valueWriteType)
		{
			...
			if ((object)genericArguments[0] == typeof(object))
			{
				stream.WriteByte(0);
			...
			{
				if (!genericArguments[0].IsPrimitive && (object)genericArguments[0] != typeof(string))
				{
			...
			if ((object)genericArguments[1] == typeof(object))
			{
				stream.WriteByte(0);
		...
		private bool WriteArrayType(StreamBuffer stream, Type type, out GpType writeType)
		{
			...
			if (elementType.IsArray)
			{
				while ((object)elementType != null && elementType.IsArray)
				{
					stream.WriteByte(64);
					elementType = elementType.GetElementType();
				}
			...
			if ((object)elementType == typeof(object))
			{
				stream.WriteByte(23);
