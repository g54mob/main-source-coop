...
namespace MonoMod.Utils
{
	public sealed class MMReflectionImporter : IReflectionImporter
	{
		...
		private TypeReference _ImportReference(Type type, IGenericParameterProvider context, GenericImportKind importKind = GenericImportKind.Open)
		{
			...
			if (type.HasElementType)
			{
				...
				if (type.IsArray)
				{
					...
					if ((object)type != type.GetElementType().MakeArrayType())
					{
		...
		public FieldReference ImportReference(FieldInfo field, IGenericParameterProvider context)
		{
			...
			TypeReference typeReference = (((object)declaringType != null) ? ImportReference(declaringType, context) : ImportModuleType(field.Module, context));
			FieldInfo key = field;
		...
		private MethodReference _ImportReference(MethodBase method, IGenericParameterProvider context, GenericImportKind importKind)
		{
			...
			value = new MethodReference(method.Name, _ImportReference(typeof(void), context), ((object)declaringType != null) ? _ImportReference(declaringType, context, GenericImportKind.Definition) : ImportModuleType(method.Module, context));
