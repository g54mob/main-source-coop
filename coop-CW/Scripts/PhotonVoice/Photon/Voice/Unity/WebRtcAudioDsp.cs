...
namespace Photon.Voice.Unity
{
	...
	public class WebRtcAudioDsp : VoiceComponent
	{
		...
		protected override void Awake()
		{
			...
			base.Logger.LogWarning("WebRtcAudioDsp is not supported on this platform {0}. The component will be disabled.", Application.platform);
		}
		...
		public void AdjustVoiceInfo(ref VoiceInfo voiceInfo, ref AudioSampleType st)
		{
			if (IsSupported && base.enabled)
			{
				...
				{
				case 12000:
					base.Logger.LogWarning("Sampling rate requested (12kHz) is not supported by WebRtcAudioDsp, switching to the closest supported value: 16kHz.");
					voiceInfo.SamplingRate = 16000;
					break;
				case 24000:
					base.Logger.LogWarning("Sampling rate requested (24kHz) is not supported by WebRtcAudioDsp, switching to the closest supported value: 48kHz.");
					voiceInfo.SamplingRate = 48000;
					break;
				}
				if (voiceInfo.FrameDurationUs < 10000)
				{
					base.Logger.LogWarning("Frame duration requested ({0}ms) is not supported by WebRtcAudioDsp (it needs to be N x 10ms), switching to the closest supported value: 10ms.", voiceInfo.FrameDurationUs / 1000);
		...
		private void OnAudioOutFrameFloat(float[] data, int outChannels)
		{
			...
			{
				base.Logger.LogWarning("OnAudioOutFrame channel count {0} != initialized {1}.", outChannels, reverseChannels);
			}
			else
			{
