...
namespace Dissonance.Networking.Client
{
	internal class PeerVoiceReceiver
	{
		...
		private static readonly Log Log = Logs.Create(LogCategory.Network, "PeerVoiceReceiver");
		...
		private RemoteChannel? IsChannelToLocalPlayer(ChannelBitField channel, ushort recipient)
		{
			...
			if (channel.Type == ChannelType.Room)
			{
				...
				if (ReceiveAllVoicePackets)
				{
					return new RemoteChannel(text ?? "Unknown Room Name", ChannelType.Room, options);
				}
				if (text == null)
				{
					return null;
