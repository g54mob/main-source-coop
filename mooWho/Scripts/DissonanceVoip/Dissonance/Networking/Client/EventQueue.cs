...
{
	internal class EventQueue : IVoiceEventQueue
	{
		...
		private static readonly Log Log = Logs.Create(LogCategory.Network, "EventQueue");
		...
		{
			_byteArrayPool = byteArrayPool ?? throw new ArgumentNullException("byteArrayPool");
			_channelsListPool = channelsListPool ?? throw new ArgumentNullException("channelsListPool");
		}
		...
		{
			this.OnEnqueuePlayerLeft?.Invoke(playerName);
			using ReadonlyLockedValue<List<NetworkEvent>>.Unlocker unlocker = _queuedEvents.Lock();
