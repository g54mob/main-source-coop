...
namespace Dissonance.Networking
{
	public abstract class BaseClient<TServer, TClient, TPeer> : IClient<TPeer> where TServer : BaseServer<TServer, TClient, TPeer> where TClient : BaseClient<TServer, TClient, TPeer> where TPeer : struct, IEquatable<TPeer>
	{
		...
		[NotNull]
		internal TrafficCounter RecvRemoveClient { get; } = new TrafficCounter();

		[NotNull]
		internal TrafficCounter RecvVoiceData { get; } = new TrafficCounter();

		[NotNull]
		internal TrafficCounter RecvTextData { get; } = new TrafficCounter();

		[NotNull]
		internal TrafficCounter RecvHandshakeResponse { get; } = new TrafficCounter();

		[NotNull]
		internal TrafficCounter RecvHandshakeP2P { get; } = new TrafficCounter();

		[NotNull]
		internal TrafficCounter RecvClientState { get; } = new TrafficCounter();

		[NotNull]
		internal TrafficCounter RecvDeltaState { get; } = new TrafficCounter();

		[NotNull]
		internal TrafficCounter SentServerTraffic { get; } = new TrafficCounter();
		...
		{
			Log.CheckMainThreadAndThrow(null, "NetworkReceivedPacket");
			if (_disconnected)
			{
		...
		{
			Log.CheckMainThreadAndThrow(null, "ReceiveHandshakeP2P");
			if (!IsConnected)
			{
