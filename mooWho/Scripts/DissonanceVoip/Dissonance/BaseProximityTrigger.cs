...
namespace Dissonance
{
	public abstract class BaseProximityTrigger<THandle> : BaseCommsTrigger
	{
		...
		{
			CloseChannels();
			List<VoiceProximityBroadcastTrigger> list = (from a in Object.FindObjectsByType<VoiceProximityBroadcastTrigger>(FindObjectsInactive.Include, FindObjectsSortMode.InstanceID)
				where a.RoomName == RoomName
				select a).ToList();
			List<VoiceProximityReceiptTrigger> list2 = (from a in Object.FindObjectsByType<VoiceProximityReceiptTrigger>(FindObjectsInactive.Include, FindObjectsSortMode.InstanceID)
				where a.RoomName == RoomName
				select a).ToList();
