...
namespace Dissonance.Audio.Capture
{
	internal class WebRtcPreprocessingPipeline : BasePreprocessingPipeline
	{
			...
			public void Process(AudioPluginDissonanceNative.SampleRates inputSampleRate, float[] input, float[] output)
			{
				if (Enabled)
				{
					using (LockedValue<IntPtr>.Unlocker unlocker = _handle.Lock())
					{
						...
						float num = Mathf.Lerp(0f, -80f, _removalAmount);
						float min_band_gain = (float)Math.Pow(10.0, num / 20f);
						AudioPluginDissonanceNative.Dissonance_SetMinBandGain(unlocker.Value, min_band_gain);
						if (!AudioPluginDissonanceNative.Dissonance_RnnoiseProcessFrame(unlocker.Value, input.Length, (int)inputSampleRate, input, output))
						{
		...
		private static readonly Log Log = Logs.Create(LogCategory.Recording, "WebRtcPreprocessingPipeline");
