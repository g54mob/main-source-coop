...
namespace Dissonance.Audio.Capture
{
	...
	{
		private static readonly Log Log = Logs.Create(LogCategory.Recording, "BasePreprocessingPipeline");
		...
		void IMicrophoneSubscriber.ReceiveMicrophoneData(ArraySegment<float> data, [NotNull] WaveFormat format)
		{
			...
			lock (_inputWriteLock)
			{
				...
				Metrics.Sample(_metricInputBufferLostSamples, num2);
			}
			_threadEvent.Set();
		}
				...
				while (_runThread)
				{
					...
					if (num > 0)
					{
						...
						while (num >= _intermediateFrame.Length)
						{
							...
							num3 += _intermediateFrame.Length;
						}
						Metrics.Sample(_metricInjectedCatchupSamples, num3);
						if (num > 0)
						{
					...
					int num4 = tickCount2 - tickCount;
					if (tickCount2 > tickCount && num4 > 50 + num2 * 5)
					{
						Log.Warn("Preprocessor running slow! Iteration took:{0}ms for {1} frames", num4, num2);
					}
					if (tickCount2 > tickCount)
					{
						Metrics.Sample(_metricPreprocessorTime, num4);
					}
				}
			}
			catch (Exception ex)
