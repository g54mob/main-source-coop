...
namespace Dissonance.Audio.Playback
{
	internal class BufferedDecoder : IFrameSource, IRemoteChannelProvider
	{
		...
		{
			_decoder = decoder ?? throw new ArgumentNullException("decoder");
			_waveFormat = waveFormat ?? throw new ArgumentNullException("waveFormat");
			_recycleFrame = recycleFrame ?? throw new ArgumentNullException("recycleFrame");
		...
		public bool Read(ArraySegment<float> frame)
		{
			...
			EncodedBuffer input = new EncodedBuffer(frame2?.EncodedAudioFrame, lostPacket || !frame2.HasValue);
		...
		public void Reset()
		{
			...
			using (LockedValue<PlaybackOptions>.Unlocker unlocker = _options.Lock())
			{
				unlocker.Value = new PlaybackOptions(isPositional: false, 0f, ChannelPriority.Default);
			}
		...
		public void Push(VoicePacket frame)
		{
			if (!_receivedFirstPacket)
			{
				...
				using LockedValue<PlaybackOptions>.Unlocker unlocker = _options.Lock();
				unlocker.Value = frame.PlaybackOptions;
			}
			_buffer.Push(frame);
