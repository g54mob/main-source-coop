...
namespace Dissonance
{
	public class Log
	{
		...
		public static void CheckMainThreadAndThrow([CanBeNull] string likelyCause = null, [CallerMemberName] string method = "")
		{
			...
			string text = "Error: Method `" + method + "` can only be called from the main thread";
			if (likelyCause != null)
			{
				text = text + ". This is likely caused by \"" + likelyCause + "\"";
			}
			throw new DissonanceException(text);
		}
