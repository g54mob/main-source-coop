...
namespace Mirror
{
	...
	public class NetworkTransformReliable : NetworkTransformBase
	{
		...
		{
			base.ResetState();
			Compression.ScaleToLong(GetPosition(), positionPrecision, out lastSerializedPosition);
			lastDeserializedPosition = lastSerializedPosition;
			Compression.ScaleToLong(GetScale(), scalePrecision, out lastSerializedScale);
