...
public class Outline : MonoBehaviour
{
	...
	[Tooltip("Outline sadece bu objenin altındaki renderer'lara uygulanır. Boşsa tüm child'lar.")]
	[SerializeField]
	private GameObject outlineGroup;
	...
	private void Awake()
	{
		...
		{
			renderers = outlineGroup.GetComponentsInChildren<Renderer>();
		}
		else
		{
	...
	public void RebuildRenderers()
	{
		...
		if (base.enabled)
		{
			...
			foreach (Renderer renderer2 in array)
			{
				if (!(renderer2 == null))
				{
					List<Material> list2 = renderer2.sharedMaterials.ToList();
					if (!list2.Contains(outlineMaskMaterial))
					{
						list2.Add(outlineMaskMaterial);
					}
					if (!list2.Contains(outlineFillMaterial))
					{
						list2.Add(outlineFillMaterial);
					}
					renderer2.materials = list2.ToArray();
				}
			}
		}
		needsUpdate = true;
