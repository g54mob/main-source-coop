...
namespace Mirror.Examples.MultipleAdditiveScenes
{
	internal static class Spawner
	{
			...
			private void MoveNext()
			{
				...
				try
				{
					...
					if (num != 0)
					{
						...
						awaiter = DelayedSpawn(reward.scene).GetAwaiter();
		...
		internal static void SpawnReward(Scene scene)
		{
			if (NetworkServer.active)
			{
				GameObject gameObject = Get(new Vector3(UnityEngine.Random.Range(-19, 20), 1f, UnityEngine.Random.Range(-19, 20)), Quaternion.identity);
