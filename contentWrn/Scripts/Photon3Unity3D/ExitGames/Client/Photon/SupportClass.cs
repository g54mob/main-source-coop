...
namespace ExitGames.Client.Photon
{
	public class SupportClass
	{
		...
		public static List<MethodInfo> GetMethods(Type type, Type attribute)
		{
			...
			{
				if ((object)attribute == null || methodInfo.IsDefined(attribute, inherit: false))
				{
					list.Add(methodInfo);
				}
			}
		...
		public static string DictionaryToString(IDictionary dictionary, bool includeTypes = true)
		{
			...
			foreach (object key in dictionary.Keys)
			{
				...
				if ((object)type == typeof(IDictionary) || (object)type == typeof(Hashtable))
				{
					text = DictionaryToString((IDictionary)dictionary[key]);
				}
				else if ((object)type == typeof(NonAllocDictionary<byte, object>))
				{
