...
namespace MonoMod.Utils
{
	internal static class _DMDEmit
	{
		...
		private static MethodBuilder _CreateMethodProxy(MethodBuilder context, MethodInfo target)
		{
			...
			if ((object)target.ReturnType == typeof(void))
			{
		...
		public static void Generate(DynamicMethodDefinition dmd, MethodBase _mb, ILGenerator il)
		{
			...
			if ((object)mb != null)
			{
				foreach (ParameterDefinition parameter2 in definition.Parameters)
				{
			...
			LocalBuilder[] array = definition.Body.Variables.Select(delegate(VariableDefinition var)
			{
				...
				if ((object)mb != null && defInfo != null && defInfo.TryGetName(var, out var name))
				{
			...
			Dictionary<Document, ISymbolDocumentWriter> dictionary = (((object)mb == null) ? null : new Dictionary<Document, ISymbolDocumentWriter>());
			...
			foreach (Instruction instruction2 in definition.Body.Instructions)
			{
				...
				if ((object)mb != null && sequencePoint != null)
				{
				...
				else
				{
					...
					else if (obj is MemberReference memberReference)
					{
						...
						obj = memberInfo;
						if ((object)mb != null && (object)memberInfo != null)
						{
							...
							if ((object)assembly != null && !hashSet.Contains(assembly))
							{
					...
					if ((object)mb != null && obj is MethodBase { DeclaringType: null } methodBase)
					{
		...
		internal static void _EmitCallSite(DynamicMethod dm, ILGenerator il, System.Reflection.Emit.OpCode opcode, CallSite csite)
		{
			...
			if ((object)_ILGen_emit_int != null)
			{
			...
			void AddOneArgTypeHelperWorker(Type clsArgument, bool lastWasGenericInst)
			{
				...
				else
				{
					...
					{
						if ((object)clsArgument == CorElementTypes[k])
						{
							b = (byte)k;
							break;
						}
					...
					if (b == 0)
					{
						b = (byte)(((object)clsArgument == typeof(object)) ? 28 : ((!clsArgument.IsValueType) ? 18 : 17));
					}
