	...
	public static class AudioUtil
	{
		...
		public abstract class GeneratorPusher<T> : IAudioPusher<T>, IAudioDesc, IDisposable
		{
			...
			public void SetCallback(Action<T[]> callback, ObjectFactory<T[], int> bufferFactory)
			{
				...
				timer = new Timer(1000.0 * (double)bufSamples / (double)SamplingRate);
			...
			{
				T[] array = bufferFactory.New(bufSamples * Channels);
				timeSamples += Gen(array, timeSamples);
				callback(array);
			}
		...
		public class ToneAudioReader<T> : GeneratorReader<T>
		{
			...
			{
				k = Math.PI * 2.0 * frequency / (double)base.SamplingRate;
			}

			protected override int Gen(T[] buf, long timeSamples)
			{
				return ToneToBuf(buf, timeSamples, base.Channels, 0.2, k);
			}
		}
		...
		public class ToneAudioPusher<T> : GeneratorPusher<T>
		{
			...
			{
				k = Math.PI * 2.0 * (double)frequency / (double)base.SamplingRate;
			}
		...
		public class WaveformAudioReader<T> : GeneratorReader<T>
		{
			...
			protected override int Gen(T[] buf, long timeSamples)
			{
				...
				return WaveformToBuf(buf, waveform, timeSamples * base.Channels) / base.Channels;
			}
		}
		...
		{
			int num = length / channels;
			if (buf is float[])
			{
				float[] array = buf as float[];
				for (int i = 0; i < num; i++)
				{
					double num2 = (double)timeSamples++ * k;
					float num3 = (float)(Math.Sin(num2 + Math.Sin(num2) * phaseMod) * amp);
			...
			else if (buf is short[])
			{
				...
				for (int l = 0; l < num; l++)
				{
					double num4 = (double)timeSamples++ * k;
					short num5 = (short)(Math.Sin(num4 + Math.Sin(num4) * phaseMod) * (amp * 32767.0));
		...
		{
			int num = (int)(timePos % waveform.Length);
			if (num + buf.Length <= waveform.Length)
			{
				Array.Copy(waveform, num, buf, 0, buf.Length);
			}
			else
			{
				int num2 = 0;
				while (waveform.Length - num <= buf.Length - num2)
				{
					Array.Copy(waveform, num, buf, num2, waveform.Length - num);
					num2 += waveform.Length - num;
					num = 0;
				}
				Array.Copy(waveform, num, buf, num2, buf.Length - num2);
			}
			return buf.Length;
		}
