...
namespace MonoMod.RuntimeDetour.Platforms
{
	public class DetourNativeWindowsPlatform : IDetourNativePlatform
	{
		...
		private unsafe Exception LogAllSections(int error, string from, IntPtr src, uint size)
		{
			...
			if (MonoMod.MMDbgLog.Writer == null)
			{
				return ex;
			}
			MonoMod.MMDbgLog.Log($"{from} failed for 0x{(long)src:X16} + {size} - logging all memory sections");
			MonoMod.MMDbgLog.Log("reason: " + ex.Message);
			try
			{
				...
				while (VirtualQueryEx(currentProcess, intPtr, out lpBuffer, sizeof(MEMORY_BASIC_INFORMATION)) != 0)
				{
					...
					MonoMod.MMDbgLog.Log(string.Format("{0} #{1}", flag ? "*" : "-", num++));
					MonoMod.MMDbgLog.Log($"addr: 0x{(long)lpBuffer.BaseAddress:X16}");
					MonoMod.MMDbgLog.Log($"size: 0x{(long)lpBuffer.RegionSize:X16}");
					MonoMod.MMDbgLog.Log($"aaddr: 0x{(long)lpBuffer.AllocationBase:X16}");
					MonoMod.MMDbgLog.Log($"state: {lpBuffer.State}");
					MonoMod.MMDbgLog.Log($"type: {lpBuffer.Type}");
					MonoMod.MMDbgLog.Log($"protect: {lpBuffer.Protect}");
					MonoMod.MMDbgLog.Log($"aprotect: {lpBuffer.AllocationProtect}");
					try
					{
