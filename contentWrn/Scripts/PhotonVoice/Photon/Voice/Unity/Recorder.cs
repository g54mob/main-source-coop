	...
	public class Recorder : VoiceComponent
	{
		...
		public object UserData
		{
			...
			set
			{
				...
				{
					userData = value;
					base.Logger.LogInfo("Recorder.UserData changed");
					RestartRecording();
				}
			}
		}
		...
		public Func<IAudioDesc> InputFactory
		{
			...
			set
			{
				...
				{
					inputFactory = value;
					base.Logger.LogInfo("Recorder.InputFactory changed");
		...
		public InputSourceType SourceType
		{
			...
			set
			{
				...
				{
					sourceType = value;
					base.Logger.LogInfo("Recorder.Source changed");
					RestartRecording();
				}
			}
		}
		...
		public MicType MicrophoneType
		{
			...
			set
			{
				if (microphoneType != value)
				{
					...
					base.Logger.LogInfo("Recorder.MicrophoneType changed");
		...
		public AudioClip AudioClip
		{
			...
			set
			{
				...
				{
					audioClip = value;
					base.Logger.LogInfo("Recorder.AudioClip change");
		...
		public SamplingRate SamplingRate
		{
			...
			set
			{
				...
				{
					samplingRate = value;
					base.Logger.LogInfo("Recorder.SamplingRate changed");
					RestartRecording();
				}
			}
		}
		...
		public OpusCodec.FrameDuration FrameDuration
		{
			...
			set
			{
				...
				{
					frameDuration = value;
					base.Logger.LogInfo("Recorder.FrameDuration changed");
					RestartRecording();
				}
			}
		}
		...
		public int Bitrate
		{
			...
			set
			{
				if (bitrate != value)
				{
					...
					{
						bitrate = value;
						base.Logger.LogInfo("Recorder.Bitrate changed");
						RestartRecording();
					}
				}
			}
		...
		public bool UseOnAudioFilterRead
		{
			...
			set
			{
				if (useOnAudioFilterRead != value)
				{
					...
					base.Logger.LogInfo("Recorder.UseOnAudioFilterRead changed");
		...
		public DeviceInfo MicrophoneDevice
		{
			...
			set
			{
				if (microphoneDevice != value)
				{
					...
					base.Logger.LogInfo("Recorder.MicrophoneDevice changed");
					if (SourceType == InputSourceType.Microphone)
					{
						RestartRecording();
					}
		...
		internal bool Init(VoiceConnection connection)
		{
			...
			{
				base.Logger.LogWarning("Recorder is disabled.");
				return false;
			}
		...
		private void StartRecording()
		{
			...
			if (voiceConnection == null)
			{
				base.Logger.LogInfo("Recording can't be started if Recorder is not initialized.");
				return;
			}
			base.Logger.LogInfo("Starting recording");
			if (inputSource != null)
			{
				inputSource.Dispose();
				inputSource = null;
		...
		{
			base.Logger.LogInfo("Stopping recording");
			if (voice != LocalVoiceAudioDummy.Dummy)
			{
				voice.RemoveSelf();
		...
		public bool SetIosAudioSessionParameters(AudioSessionCategory category, AudioSessionMode mode, AudioSessionCategoryOption[] options)
		{
			...
			if (audioSessionParameters.Category != category || audioSessionParameters.Mode != mode || audioSessionParameters.CategoryOptionsToInt() != num)
			{
				...
				base.Logger.LogInfo("Recorder.iOSAudioSessionParameters changed to {0}", audioSessionParameters);
		...
		public bool SetAndroidNativeMicrophoneSettings(bool aec = false, bool agc = false, bool ns = false)
		{
			if (androidMicrophoneSettings.EnableAEC != aec || androidMicrophoneSettings.EnableAGC != agc || androidMicrophoneSettings.EnableNS != ns)
			{
				...
				base.Logger.LogInfo("Recorder.nativeAndroidMicrophoneSettings changed to aec = {0}, agc = {1}, ns = {2}", aec, agc, ns);
				if (SourceType == InputSourceType.Microphone && MicrophoneType == MicType.Photon)
				{
					RestartRecording();
				}
		...
		private LocalVoice CreateLocalVoiceAudioAndSource()
		{
			...
			case InputSourceType.Factory:
				...
				{
					base.Logger.LogWarning("Recorder.Source is Factory but Recorder.InputFactory is not set. Setting it to ToneAudioReader.");
			...
			default:
				...
				base.Logger.LogInfo("Setting recorder's source to Photon microphone device={0}", deviceInfo);
				...
				case RuntimePlatform.MetroPlayerARM:
					base.Logger.LogInfo("Setting recorder's source to UWP.AudioInPusher");
					break;
				case RuntimePlatform.IPhonePlayer:
					otherParams = audioSessionParameters;
					base.Logger.LogInfo("Setting recorder's source to IOS.AudioInPusher with session {0}", audioSessionParameters);
					break;
				case RuntimePlatform.OSXEditor:
				case RuntimePlatform.OSXPlayer:
					base.Logger.LogInfo("Setting recorder's source to MacOS.AudioInPusher");
					break;
				case RuntimePlatform.Switch:
					base.Logger.LogInfo("Setting recorder's source to Switch.AudioInPusher");
					break;
				case RuntimePlatform.Android:
					otherParams = androidMicrophoneSettings;
					base.Logger.LogInfo("Setting recorder's source to UnityAndroidAudioInAEC");
					break;
				case RuntimePlatform.WebGLPlayer:
					base.Logger.LogInfo("Setting recorder's source to Unity.WebAudioMicIn");
					break;
				default:
					base.Logger.LogError("Photon microphone type is not supported for the current platform {0}", Application.platform);
					break;
				}
				...
				base.Logger.LogInfo("Setting recorder's source to Unity microphone device {0}", deviceInfo2);
		...
		protected void Update()
		{
			if (!(voiceConnection == null))
			{
				...
				{
					base.Logger.LogInfo("Restarting recording");
					StartRecording();
				}
			}
		}
		...
		private void OnDestroy()
		{
			if (!(voiceConnection == null))
			{
				base.Logger.LogInfo("Recorder is about to be destroyed, removing local voice.");
				StopRecording();
		...
		{
			base.Logger.LogInfo("App paused?= {0}, isPausedOrInBackground = {1}, StopRecordingWhenPaused = {2}, RecordingEnabled = {3}", paused, isPausedOrInBackground, StopRecordingWhenPaused, RecordingEnabled);
