...
namespace HarmonyLib
{
	public static class AccessTools
	{
		...
		private static FieldRef<T, F> FieldRefAccessInternal<T, F>(FieldInfo fieldInfo, bool needCastclass)
		{
			...
			return (FieldRef<T, F>)Extensions.CreateDelegate(dynamicMethodDefinition.Generate(), typeof(FieldRef<T, F>));
		}
		...
		private static StructFieldRef<T, F> StructFieldRefAccessInternal<T, F>(FieldInfo fieldInfo) where T : struct
		{
			...
			iLGenerator.Emit(System.Reflection.Emit.OpCodes.Ldflda, fieldInfo);
			iLGenerator.Emit(System.Reflection.Emit.OpCodes.Ret);
			return (StructFieldRef<T, F>)Extensions.CreateDelegate(dynamicMethodDefinition.Generate(), typeof(StructFieldRef<T, F>));
		}
		...
		private static FieldRef<F> StaticFieldRefAccessInternal<F>(FieldInfo fieldInfo)
		{
			...
			iLGenerator.Emit(System.Reflection.Emit.OpCodes.Ldsflda, fieldInfo);
			iLGenerator.Emit(System.Reflection.Emit.OpCodes.Ret);
			return (FieldRef<F>)Extensions.CreateDelegate(dynamicMethodDefinition.Generate(), typeof(FieldRef<F>));
		}
		...
		public static DelegateType MethodDelegate<DelegateType>(MethodInfo method, object instance = null, bool virtualCall = true) where DelegateType : Delegate
		{
			...
			if (instance == null)
			{
				...
				iLGenerator.Emit(System.Reflection.Emit.OpCodes.Ret);
				return (DelegateType)Extensions.CreateDelegate(dynamicMethodDefinition.Generate(), typeFromHandle);
			}
			if (virtualCall)
			{
