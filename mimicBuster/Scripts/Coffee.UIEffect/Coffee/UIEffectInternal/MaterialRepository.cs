...
namespace Coffee.UIEffectInternal
{
	internal static class MaterialRepository
	{
		...
		{
			s_Repository.Get(hash, ref material, (string x) => new Material(onShaderFind(x))
			{
		...
		{
			s_Repository.Get(hash, ref material, ((string shaderName, string[] keywords) x) => new Material(onShaderFind(x.shaderName))
			{
				hideFlags = (HideFlags.DontSave | HideFlags.NotEditable),
