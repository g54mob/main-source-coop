...
{
	public class NetworkEventsSynchronizerSpawnSystem : IInitializable, IDisposable
	{
		private readonly NetworkRunnerEventBus _eventBus;

		private readonly EventSynchronizerConfiguration _modelsSynchronizerConfiguration;

		private readonly Dictionary<PlayerRef, NetworkObject> _spawnedObjects = new Dictionary<PlayerRef, NetworkObject>();

		public NetworkEventsSynchronizerSpawnSystem(NetworkRunnerEventBus eventBus, EventSynchronizerConfiguration modelsSynchronizerConfiguration)
		{
			_eventBus = eventBus;
		...
		private void OnPlayerJoined(OnPlayerJoinedEvent playerJoinedEvent)
		{
			...
			{
				NetworkObject networkObject = playerJoinedEvent.Runner.Spawn(_modelsSynchronizerConfiguration.NetworkEventSynchronizer, Vector3.zero, Quaternion.identity, playerJoinedEvent.Player);
