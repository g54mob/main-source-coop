...
[AddComponentMenu("Effects/Planar Reflection Volume")]
public class PlanarReflectionVolume : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		if (main != null)
		{
			DoPlanarReflections(default(ScriptableRenderContext), main);
		}
	}
	...
	private void UpdateTargetMaterial()
	{
		...
		{
			Renderer component = reflectionTarget.GetComponent<Renderer>();
			if (component != null)
			{
				_targetMaterial = component.sharedMaterial;
			}
		}
	}
	...
	{
		_volumeBounds = new Bounds(base.transform.position, volumeSize);
	}
	...
	private float GetBlendFactor(Camera camera)
	{
		...
		Vector3 vector2 = volumeSize * 0.5f;
		float a = Mathf.Max(0f, Mathf.Abs(vector.x) - vector2.x);
		float a2 = Mathf.Max(0f, Mathf.Abs(vector.y) - vector2.y);
		float b = Mathf.Max(0f, Mathf.Abs(vector.z) - vector2.z);
		float num = Mathf.Max(a, Mathf.Max(a2, b));
		if (num <= 0f)
		{
			return 0f;
		}
		return Mathf.Clamp01(num / blendDistance);
	}

	private bool IsCameraInRange(Camera camera)
	{
		Vector3 vector = base.transform.InverseTransformPoint(camera.transform.position);
		Vector3 vector2 = volumeSize * 0.5f + new Vector3(blendDistance, blendDistance, blendDistance);
	...
	private void UpdateReflectionCamera(Camera realCamera)
	{
		...
		else if (_reflectionCamera.gameObject.hideFlags != (HideFlags)(hideReflectionCamera ? 61 : 52))
		{
	...
	{
		if (camera.cameraType == CameraType.Reflection || camera.cameraType == CameraType.Preview || !reflectionTarget)
		{
			return;
		}
		UpdateTargetMaterial();
		...
		if (!(blendFactor >= 1f))
		{
			...
			PlanarReflectionVolume.BeginPlanarReflections?.Invoke(context, _reflectionCamera);
	...
	private void OnDrawGizmos()
	{
		...
		if (blendDistance > 0f)
		{
			...
			Vector3 size = volumeSize + new Vector3(blendDistance * 2f, blendDistance * 2f, blendDistance * 2f);
