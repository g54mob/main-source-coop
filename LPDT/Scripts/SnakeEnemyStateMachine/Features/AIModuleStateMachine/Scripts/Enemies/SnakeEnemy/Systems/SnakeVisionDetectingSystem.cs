...
namespace Features.AIModuleStateMachine.Scripts.Enemies.SnakeEnemy.Systems
{
	...
	public class SnakeVisionDetectingSystem : MonoSystem
	{
		[SerializeField]
		private EnemyVisibilityTargetDetector _targetDetector;

		[SerializeField]
		private float _timeToDetect = 0.7f;
		...
		private SnakeEnemyContext _snakeEnemyContext;
		...
		[Inject]
		private void InjectDependencies(IDetectionContext detectionContext, SpawnedPlayersModel spawnedPlayersModel, PlayersGatesModelSynchronizedModel playersGatesModel, SnakeEnemyContext snakeEnemyContext)
		{
			_detectionContext = detectionContext;
			_spawnedPlayersModel = spawnedPlayersModel;
			_playersGatesModel = playersGatesModel;
			_snakeEnemyContext = snakeEnemyContext;
		}
		...
		private void Update()
		{
			...
			foreach (KeyValuePair<PlayerRef, PlayerDataHolder> player2 in _spawnedPlayersModel.Players)
			{
				...
				{
					bool isVisibleNow = IsInsideGate(key) && !IsPlayerInSafeZone(value) && _targetDetector.DetectTargets.Contains(key);
		...
		private bool IsPlayerInSafeZone(PlayerDataHolder playerData)
		{
			if (_snakeEnemyContext != null)
			{
				return _snakeEnemyContext.IsPlayerDataInSafeZone(playerData);
			}
			return false;
		}

		private float GetTimer(Dictionary<PlayerRef, float> timers, PlayerRef player)
		{
