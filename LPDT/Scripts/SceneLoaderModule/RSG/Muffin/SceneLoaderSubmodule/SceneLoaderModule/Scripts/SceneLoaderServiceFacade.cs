...
namespace RSG.Muffin.SceneLoaderSubmodule.SceneLoaderModule.Scripts
{
	public class SceneLoaderServiceFacade : ISceneLoaderService
	{
		...
		internal SceneLoaderServiceFacade(BuildInSceneLoaderService buildInSceneLoaderService, AddressablesSceneLoaderService addressablesSceneLoaderService, SceneLoaderConfig sceneLoaderConfiguration)
		{
		...
		{
			List<string> scenesNotInAddressables = scenesToUnload.Except(_sceneLoaderConfiguration.SceneAddressablesInGroup).ToList();
			if (!scenesNotInAddressables.Any())
			{
