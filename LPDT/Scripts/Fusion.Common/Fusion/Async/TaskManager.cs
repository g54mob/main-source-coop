...
namespace Fusion.Async
{
	public static class TaskManager
	{
		...
		{
			Assert.Check(action != null, "action != null");
			Assert.Check(cancellationToken != default(CancellationToken), "cancellationToken != default");
			return TaskFactory.StartNew((Func<Task>)async delegate
			{
				try
				{
		...
		public static Task ContinueWhenAll(Task[] precedingTasks, Func<CancellationToken, Task> action, CancellationToken cancellationToken)
		{
			Assert.Check(action != null, "action != null");
			Assert.Check(cancellationToken != default(CancellationToken), "cancellationToken != default");
			Assert.Check(precedingTasks != null, "precedingTasks != null");
			Assert.Check(precedingTasks.Length != 0, "precedingTasks.Length > 0");
