...
namespace Photon.Voice.Unity.Demos.DemoVoiceUI
{
	public static class PhotonDemoExtensions
	{
		...
		{
			return player.SetCustomProperties(new PhotonHashtable(1) { { "mu", true } });
		}

		public static bool Unmute(this Player player)
		{
			return player.SetCustomProperties(new PhotonHashtable(1) { { "mu", false } });
		}
		...
		{
			return player.SetCustomProperties(new PhotonHashtable(1) { { "pv", value } });
		}

		public static bool SetWebRTCVAD(this Player player, bool value)
		{
			return player.SetCustomProperties(new PhotonHashtable(1) { { "wv", value } });
		}

		public static bool SetAEC(this Player player, bool value)
		{
			return player.SetCustomProperties(new PhotonHashtable(1) { { "ec", value } });
		}

		public static bool SetAGC(this Player player, bool agcEnabled, int gain, int level)
		{
			return player.SetCustomProperties(new PhotonHashtable(1) { 
			{
				"gc",
		...
		{
			return player.SetCustomProperties(new PhotonHashtable(1) { { "m", type } });
		}
