...
namespace Features.AIModuleStateMachine.Scripts.MimicEnemy
{
	...
	public class MimicEyeRotator : NetworkBehaviour
	{
		...
		private unsafe int EyesHorizontalQuantized
		{
			get
			{
				if (Ptr == null)
				{
					throw new InvalidOperationException("Error when accessing MimicEyeRotator.EyesHorizontalQuantized. Networked properties can only be accessed when Spawned() has been called.");
				}
		...
		private unsafe int EyesVerticalQuantized
		{
			get
			{
				if (Ptr == null)
				{
					throw new InvalidOperationException("Error when accessing MimicEyeRotator.EyesVerticalQuantized. Networked properties can only be accessed when Spawned() has been called.");
				}
				return Ptr[1];
			}
			set
