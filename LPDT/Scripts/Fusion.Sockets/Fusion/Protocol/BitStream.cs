...
namespace Fusion.Protocol
{
	public class BitStream
	{
		...
		public int RoundToByte()
		{
			...
			Assert.Check(_ptr % 8 == 0, "(_ptr % 8) == 0");
			return _ptr / 8;
		}
		...
		{
			Assert.Check(array.Length <= _data.Length, "array.Length <= _data.Length");
		...
		{
			Assert.Check(byteSize <= _data.Length, "byteSize <= _data.Length");
		...
		public void Serialize(ref byte[] value, int fixedSize)
		{
			if (_write)
			{
				...
				{
					Assert.Check(value.Length == fixedSize, "value.Length == fixedSize");
		...
		public void Serialize(ref byte[] array, ref int length, int fixedSize)
		{
			...
			if (_write)
			{
				...
				{
					Assert.Check(length == fixedSize, "length == fixedSize");
					Assert.Check(array.Length <= fixedSize, "array.Length <= fixedSize");
