...
namespace Fusion.Sockets
{
	...
	public struct NetAddress : IEquatable<NetAddress>
	{
		...
		internal static class SubnetMask
		{
			...
			public static bool IsSameSubNet(NetAddress addressA, NetAddress addressB, NetAddress[] subnetMasks = null)
			{
				...
				if (!addressA.IsIPv6 && !addressB.IsIPv6)
				{
					...
					NetAddress[] array = subnetMasks;
					foreach (NetAddress subnetMask in array)
					{
		...
		public readonly int ActorId => _actorId - 1;

		public readonly bool IsRelayAddr => ActorId >= 0;

		public readonly bool IsIPv6 => !IsRelayAddr && (NativeAddress._address0 != 0L || (NativeAddress._address1 & 0xFFFF0000u) != 4294901760u);

		public readonly bool IsIPv4 => !IsRelayAddr && (NativeAddress._address1 & 0xFFFF0000u) == 4294901760u;

		public readonly bool IsValid => !Equals(AnyIPv4Addr) && !Equals(AnyIPv6Addr);
		...
		public static bool IsPrivateIp(NetAddress netAddress)
		{
			if (!netAddress.IsValid)
			{
				return false;
			}
			return SubnetMask.IsSameSubNet(CreateFromIpPort("10.0.0.0", 0), netAddress, new NetAddress[1] { CreateFromIpPort("255.0.0.0", 0) }) || SubnetMask.IsSameSubNet(CreateFromIpPort("172.16.0.0", 0), netAddress, new NetAddress[1] { CreateFromIpPort("255.240.0.0", 0) }) || SubnetMask.IsSameSubNet(CreateFromIpPort("192.168.0.0", 0), netAddress, new NetAddress[1] { CreateFromIpPort("255.255.0.0", 0) });
		}
