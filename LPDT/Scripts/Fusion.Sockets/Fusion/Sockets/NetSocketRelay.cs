...
namespace Fusion.Sockets
{
	internal class NetSocketRelay : INetSocket
	{
		...
		{
			Assert.Check(socket.Handle == _handle, "socket.Handle == _handle");
			return LocalAddress;
		}
		...
		{
			Assert.Check(_communicator != null, "_communicator != null");
			Assert.Check(_handle != 0, "_handle       != 0");
			Assert.Check(_handle == socket.Handle, "_handle       == socket.Handle");
