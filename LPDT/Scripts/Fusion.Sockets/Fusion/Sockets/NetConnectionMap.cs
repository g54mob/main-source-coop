...
namespace Fusion.Sockets
{
	public struct NetConnectionMap
	{
		...
		public struct Iterator
		{
			...
			public unsafe readonly NetConnection* Current => IsValid ? (_map->Connections + _index) : null;

			public readonly bool IsValid => _index >= 0 && _index < _count;
		...
		public readonly int Count => (int)(UsedCount - FreeCount);

		public readonly int CountUsed => (int)UsedCount;

		public unsafe readonly NetConnection* ConnectionsBuffer => Connections;

		public readonly bool Full => UsedCount == CapacityAllocated;
		...
		public unsafe static void Dispose(ref NetConnectionMap* map, INetPeerGroupCallbacks callbacks)
		{
			if (map != null)
			{
				for (int i = 0; i < (int)map->CapacityUsable; i++)
				{
					...
					FusionUnsafe.Free(ref ptr->ConnectionToken);
					ptr->ConnectionTokenLength = 0;
					FusionUnsafe.Free(ref ptr->UniqueId);
					ptr->UniqueIdHash = 0L;
				}
				FusionUnsafe.Free(ref map);
			}
		}

		public unsafe static NetConnectionMap* Allocate(int capacity, short groupIndex, in NetConfig* config)
		{
			Assert.Check(capacity >= 0, "capacity >= 0");
			int nextPrime = Primes.GetNextPrime(capacity);
			int num = FusionUnsafe.MakeAligned(sizeof(NetConnectionMap));
			int num2 = FusionUnsafe.MakeAligned(sizeof(NetConnection*) * nextPrime);
			int num3 = FusionUnsafe.MakeAligned(sizeof(NetConnection) * nextPrime);
			int num4 = FusionUnsafe.MakeAligned(sizeof(UniqueIdMapping) * nextPrime);
			byte* ptr = (byte*)FusionUnsafe.AllocAndClear(num + num2 + num3 + num4, 8, "Fusion\\Fusion.Sockets\\NetConnectionMap.cs", 137);
		...
		public unsafe NetConnection* Remap(NetAddress oldAddress, NetAddress newAddress)
		{
			...
			while (ptr != null)
			{
				...
				{
					Assert.Check(ptr->MapState == EntryState.Used, "bucketHead->MapState == EntryState.Used");
					if (ptr2 == null)
					{
		...
		{
			Assert.Check(Find(address) == null, "Find(address)           == null");
			Assert.Check(!address.Equals(default(NetAddress)), "address.Equals(default) == false");
			...
			if (FreeHead != null)
			{
				Assert.Check(FreeCount != 0, "FreeCount > 0");
				ptr2 = FreeHead;
				FreeHead = ptr2->MapNext;
				FreeCount--;
				Assert.Check(ptr2->MapState == EntryState.Free, "entry->MapState == EntryState.Free");
			}
			else
			{
				...
				Assert.Check(ptr2->MapState == EntryState.None, "entry->MapState == EntryState.None");
				Assert.Check(ptr2->MapNext == null, "entry->MapNext  == null");
			}
			Assert.Check(ptr2 == Connections + ptr2->LocalId.GroupIndex, "entry == (Connections + entry->LocalId.GroupIndex)");
			...
			{
				ptr2->UniqueId = (byte*)FusionUnsafe.AllocAndClear(uniqueId.Length, 8, "Fusion\\Fusion.Sockets\\NetConnectionMap.cs", 372);
			}
			ptr2->UniqueIdHash = num;
			fixed (byte* source = uniqueId)
			{
				FusionUnsafe.Copy(ptr2->UniqueId, source, 8);
			}
		...
		{
			Assert.Check(Group == id.Group, "Group == id.Group");
		...
		private unsafe void StoreUniqueId(long value, short groupIndex)
		{
			...
			FusionUnsafe.Move(UniqueIdHashes + num + 1, UniqueIdHashes + num, (int)(IdsCount - num) * sizeof(UniqueIdMapping));
		...
		private unsafe bool RemoveUniqueId(long value)
		{
			...
			FusionUnsafe.Move(UniqueIdHashes + num, UniqueIdHashes + num + 1, (int)(IdsCount - num - 1) * sizeof(UniqueIdMapping));
