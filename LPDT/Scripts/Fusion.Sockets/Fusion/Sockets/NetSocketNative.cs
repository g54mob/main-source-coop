...
namespace Fusion.Sockets
{
	internal class NetSocketNative : INetSocket
	{
		...
		private unsafe byte* _encryptionBuffer;

		private const int EncryptionBufferLength = 2048;

		private readonly NetSocketFlags _flags;

		public bool SupportsMultiThreading => true;

		public unsafe NetSocketNative(NetSocketFlags flags = NetSocketFlags.None)
		{
		...
		public unsafe int Receive(NetSocket socket, NetAddress* address, byte* buffer, int bufferLength)
		{
			...
			using (HostProfiler.Markers.EncryptionSocketReceive())
			{
				if (!HandleEncryptionIngoing(address, ref buffer, bufferLength, ref received))
				{
					return -1;
				}
			}
		...
		{
			using (HostProfiler.Markers.EncryptionSocketSend())
			{
				if (!HandleEncryptionOutgoing(address, ref buffer, ref bufferLength))
				{
					return -1;
				}
			}
		...
		public unsafe void SetupEncryption(byte[] key, byte[] encryptedKey)
		{
			...
			_encryptionBuffer = FusionUnsafe.AllocAndClearArray<byte>(2048, 8, "Fusion\\Fusion.Sockets\\Socket\\NetSocketNative.cs", 175);
		...
		private unsafe bool HandleEncryptionOutgoing(NetAddress* address, ref byte* buffer, ref int bufferLength)
		{
			if (_encryptionManager != null && bufferLength > 1)
			{
				...
				FusionUnsafe.Clear(_encryptionBuffer, 2048);
				FusionUnsafe.Copy(_encryptionBuffer, buffer, Math.Min(bufferLength, 2048));
				...
				if (!_encryptionManager.HasEncryptionForHandle(netAddress))
				{
					...
					num = num2 + 1;
					FusionUnsafe.Move(buffer + num, buffer, bufferLength);
					*buffer = (byte)num2;
					FusionUnsafe.Copy(buffer + 1, _encryptionToken.KeyEncrypted);
			...
			if (_encryptionManager != null && received > 1)
			{
				...
				if (!_encryptionManager.HasEncryptionForHandle(netAddress))
				{
					...
					else
					{
						...
						FusionUnsafe.Copy(array, new ReadOnlySpan<byte>(ptr, length));
						_encryptionManager.RegisterEncryptionKey(netAddress, array);
						received -= num;
						FusionUnsafe.Move(buffer, buffer + num, received);
					}
				}
		...
		private unsafe void ResetEncryption()
		{
			...
			_remoteEncryptionHandler = default(NetAddress);
			FusionUnsafe.Free(ref _encryptionBuffer);
		}
