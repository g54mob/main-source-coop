	...
	public class NetworkSceneManagerDefault : Behaviour, INetworkSceneManager
	{
		...
		public virtual void Initialize(NetworkRunner runner)
		{
			...
			if (runner.SceneInfo.SceneCount == 0)
			{
				...
				{
					Log.Error("PeerModes.Multiple requires a scene to be set in StartGameArgs.Scene. Set a scene or disable Multiple Peer mode in the NetworkProjectConfig.");
				}
				else
				{
					Log.Warn("NetworkRunner started with no scene in StartGameArgs.Scene. No network scene will be loaded and no scene NetworkObjects will be spawned.");
				}
			}
			if (IsMultiplePeer)
			{
		...
		protected virtual IEnumerator LoadSceneCoroutine(SceneRef sceneRef, NetworkLoadSceneParameters sceneParams)
		{
			...
			using (MakeLoadingScope())
			{
				...
				if (!scene.IsValid())
				{
					...
					if (sceneRef.IsIndex)
					{
						num = sceneRef.AsIndex;
					}
					else
					{
						for (int j = 0; j < SceneManager.sceneCountInBuildSettings; j++)
						{
							string scenePathByBuildIndex = SceneUtility.GetScenePathByBuildIndex(j);
							if (sceneRef.IsPath(scenePathByBuildIndex))
							{
								num = j;
								break;
							}
					...
					if (num >= 0)
					{
						AsyncOperation op = SceneManager.LoadSceneAsync(num, new LoadSceneParameters(loadSceneMode, localPhysicsMode));
						if (op == null)
						{
							throw new InvalidOperationException($"Scene not found: {num}");
						}
		...
		protected virtual IEnumerator OnSceneLoaded(SceneRef sceneRef, Scene scene, NetworkLoadSceneParameters sceneParams)
		{
			...
			if (IsMultiplePeer)
			{
				...
				multiPeerSceneRoot.SceneHandle = scene.GetRawHandle();
