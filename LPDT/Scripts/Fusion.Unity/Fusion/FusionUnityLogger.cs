...
namespace Fusion
{
	public class FusionUnityLogger : FusionUnityLoggerBase
	{
		...
		public bool LogActiveRunnerLocalPlayer;
			...
			try
			{
				...
				if (LogActiveRunnerTick || LogActiveRunnerLocalPlayer)
				{
					...
					while (instancesEnumerator.MoveNext())
					{
						...
						{
							PlayerRef localPlayer = current.LocalPlayer;
							if (localPlayer.IsRealPlayer)
							{
								threadSafeStringBuilder.Append("[P").Append(localPlayer.PlayerId).Append("]");
							}
							else
							{
								threadSafeStringBuilder.Append("[P-]");
							}
						}
						break;
					}
