...
namespace Obi
{
	public struct ObiPathFrame
	{
		...
		public static ObiPathFrame Identity => new ObiPathFrame(Vector3.zero, Vector3.forward, Vector3.up, Vector3.right, Color.white, 0f);
		...
		{
			Quaternion quaternion = Quaternion.FromToRotation(tangent, frame.tangent);
			Quaternion quaternion2 = Quaternion.AngleAxis(twist, frame.tangent) * quaternion;
		...
		public Matrix4x4 ToMatrix(Axis mainAxis)
		{
			...
			{
				[(int)mainAxis % 3 * 4] = tangent[0],
				[num + 1] = tangent[1],
				[num + 2] = tangent[2],
				[(int)(mainAxis + 1) % 3 * 4] = binormal[0],
				[num2 + 1] = binormal[1],
				[num2 + 2] = binormal[2],
				[(int)(mainAxis + 2) % 3 * 4] = normal[0],
				[num3 + 1] = normal[1],
				[num3 + 2] = normal[2]
			};
		}
