...
namespace Obi
{
	public class MeshDataBatch
	{
		...
		public int Count => meshData.count;
		...
		{
			if (!meshToIndex.TryGetValue(meshProvider.sourceMesh, out var value))
			{
				value = meshData.count;
				meshToIndex[meshProvider.sourceMesh] = value;
		...
		{
			int firstVertex = meshData[meshIndex].firstVertex;
			int vertexCount = meshData[meshIndex].vertexCount;
			return restPositions.AsNativeArray<Vector3>().Slice(firstVertex, vertexCount);
		}
		...
		public NativeSlice<Vector3> GetNormals(int meshIndex)
		{
			int firstVertex = meshData[meshIndex].firstVertex;
			int vertexCount = meshData[meshIndex].vertexCount;
			return restNormals.AsNativeArray<Vector3>().Slice(firstVertex, vertexCount);
		}
		...
		public NativeSlice<Vector4> GetTangents(int meshIndex)
		{
			int firstVertex = meshData[meshIndex].firstVertex;
			int vertexCount = meshData[meshIndex].vertexCount;
			return restTangents.AsNativeArray<Vector4>().Slice(firstVertex, vertexCount);
		}
		...
		public NativeSlice<Color> GetColors(int meshIndex)
		{
			int firstVertex = meshData[meshIndex].firstVertex;
			int vertexCount = meshData[meshIndex].vertexCount;
			return restColors.AsNativeArray<Color>().Slice(firstVertex, vertexCount);
		}
		...
		public NativeSlice<Vector2> GetUV(int meshIndex)
		{
			int firstVertex = meshData[meshIndex].firstVertex;
			int vertexCount = meshData[meshIndex].vertexCount;
			return uv.AsNativeArray<Vector2>().Slice(firstVertex, vertexCount);
		}
		...
		public NativeSlice<Vector2> GetUV2(int meshIndex)
		{
			int firstVertex = meshData[meshIndex].firstVertex;
			int vertexCount = meshData[meshIndex].vertexCount;
			return uv2.AsNativeArray<Vector2>().Slice(firstVertex, vertexCount);
		}
		...
		public NativeSlice<Vector2> GetUV3(int meshIndex)
		{
			int firstVertex = meshData[meshIndex].firstVertex;
			int vertexCount = meshData[meshIndex].vertexCount;
			return uv3.AsNativeArray<Vector2>().Slice(firstVertex, vertexCount);
		}
		...
		public NativeSlice<Vector2> GetUV4(int meshIndex)
		{
			int firstVertex = meshData[meshIndex].firstVertex;
			int vertexCount = meshData[meshIndex].vertexCount;
			return uv4.AsNativeArray<Vector2>().Slice(firstVertex, vertexCount);
		}

		public NativeSlice<int> GetTriangles(int meshIndex)
		{
			int firstTriangle = meshData[meshIndex].firstTriangle;
			int triangleCount = meshData[meshIndex].triangleCount;
			return triangles.AsNativeArray<int>().Slice(firstTriangle, triangleCount);
		}
	}
}
