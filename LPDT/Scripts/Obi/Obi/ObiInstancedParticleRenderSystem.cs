using System.Collections.Generic;
using Unity.Profiling;
using UnityEngine;

namespace Obi
{
	public abstract class ObiInstancedParticleRenderSystem : RenderSystem<ObiInstancedParticleRenderer>, IRenderSystem
	{
		protected static ProfilerMarker m_SetupRenderMarker = new ProfilerMarker("SetupParticleRendering");

		protected static ProfilerMarker m_RenderMarker = new ProfilerMarker("ParticleRendering");

		protected ObiSolver m_Solver;

		protected List<InstancedRenderBatch> batchList = new List<InstancedRenderBatch>();

		protected ObiNativeList<int> activeParticles;

		protected ObiNativeList<int> rendererIndex;

		protected ObiNativeList<ParticleRendererData> rendererData;

		protected ObiNativeList<Matrix4x4> instanceTransforms;

		protected ObiNativeList<Matrix4x4> invInstanceTransforms;

		protected ObiNativeList<Vector4> instanceColors;

		public Oni.RenderingSystemType typeEnum => Oni.RenderingSystemType.InstancedParticles;

		public RendererSet<ObiInstancedParticleRenderer> renderers { get; } = new RendererSet<ObiInstancedParticleRenderer>();

		public bool isSetup => activeParticles != null;

		public ObiInstancedParticleRenderSystem(ObiSolver solver)
		{
			m_Solver = solver;
			activeParticles = new ObiNativeList<int>();
			rendererIndex = new ObiNativeList<int>();
			rendererData = new ObiNativeList<ParticleRendererData>();
			instanceTransforms = new ObiNativeList<Matrix4x4>();
			invInstanceTransforms = new ObiNativeList<Matrix4x4>();
			instanceColors = new ObiNativeList<Vector4>();
		}

		public virtual void Dispose()
		{
			for (int i = 0; i < batchList.Count; i++)
			{
				batchList[i].Dispose();
			}
			batchList.Clear();
			if (activeParticles != null)
			{
				activeParticles.Dispose();
			}
			if (rendererData != null)
			{
				rendererData.Dispose();
			}
			if (rendererIndex != null)
			{
				rendererIndex.Dispose();
			}
			if (instanceTransforms != null)
			{
				instanceTransforms.Dispose();
			}
			if (invInstanceTransforms != null)
			{
				invInstanceTransforms.Dispose();
			}
			if (instanceColors != null)
			{
				instanceColors.Dispose();
			}
		}

		protected virtual void Clear()
		{
			for (int i = 0; i < batchList.Count; i++)
			{
				batchList[i].Dispose();
			}
			batchList.Clear();
			activeParticles.Clear();
			rendererData.Clear();
			rendererIndex.Clear();
			instanceTransforms.Clear();
			invInstanceTransforms.Clear();
			instanceColors.Clear();
		}

		protected virtual void CreateBatches()
		{
			for (int i = 0; i < renderers.Count; i++)
			{
				renderers[i].renderParameters.layer = renderers[i].gameObject.layer;
				int num = 0;
				while (num < renderers[i].actor.particleCount)
				{
					InstancedRenderBatch instancedRenderBatch = new InstancedRenderBatch(i, renderers[i].mesh, renderers[i].material, renderers[i].renderParameters);
					instancedRenderBatch.firstInstance = num;
					instancedRenderBatch.instanceCount = Mathf.Min(renderers[i].actor.particleCount - num, 1023);
					num += instancedRenderBatch.instanceCount;
					batchList.Add(instancedRenderBatch);
				}
			}
			batchList.Sort();
			for (int j = 0; j < batchList.Count; j++)
			{
				InstancedRenderBatch instancedRenderBatch2 = batchList[j];
				ObiInstancedParticleRenderer obiInstancedParticleRenderer = renderers[instancedRenderBatch2.firstRenderer];
				int count = activeParticles.count;
				activeParticles.AddRange(obiInstancedParticleRenderer.actor.solverIndices, instancedRenderBatch2.firstInstance, instancedRenderBatch2.instanceCount);
				rendererIndex.AddReplicate(j, instancedRenderBatch2.instanceCount);
				rendererData.Add(new ParticleRendererData(obiInstancedParticleRenderer.instanceColor, obiInstancedParticleRenderer.instanceScale));
				instancedRenderBatch2.firstInstance = count;
			}
			instanceTransforms.ResizeUninitialized(activeParticles.count);
			invInstanceTransforms.ResizeUninitialized(activeParticles.count);
			instanceColors.ResizeUninitialized(activeParticles.count);
		}

		protected virtual void CloseBatches()
		{
			for (int i = 0; i < batchList.Count; i++)
			{
				batchList[i].Initialize();
			}
		}

		public virtual void Setup()
		{
			using (m_SetupRenderMarker.Auto())
			{
				Clear();
				CreateBatches();
				ObiUtils.MergeBatches(batchList);
				CloseBatches();
			}
		}

		public virtual void Step()
		{
		}

		public virtual void Render()
		{
		}
	}
}
