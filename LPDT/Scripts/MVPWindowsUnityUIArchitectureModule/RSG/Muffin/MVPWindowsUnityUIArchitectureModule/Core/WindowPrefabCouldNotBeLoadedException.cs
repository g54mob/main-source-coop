...
namespace RSG.Muffin.MVPWindowsUnityUIArchitectureModule.Core
{
	public class WindowPrefabCouldNotBeLoadedException : Exception
	{
		...
		public WindowPrefabCouldNotBeLoadedException(Type windowType, string key)
			: base(GenerateMessage(windowType, key, string.Empty))
		{
		}

		private static string GenerateMessage(Type windowType, string key, string loadSource)
		{
			return $"Window of type {windowType} coud not be loaded " + "from " + loadSource + ". Make sure you have been added window prefab to " + loadSource + " folder by key " + key + " and added MonoWindowInstance component to this prefab";
		}
	}
}
