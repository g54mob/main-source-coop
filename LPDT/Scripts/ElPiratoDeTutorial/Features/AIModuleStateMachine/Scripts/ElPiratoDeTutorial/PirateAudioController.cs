...
namespace Features.AIModuleStateMachine.Scripts.ElPiratoDeTutorial
{
	...
	public class PirateAudioController : NetworkBehaviour
	{
		...
		private void OnDisable()
		{
			...
			_breathInstance.stop(FMOD.Studio.STOP_MODE.IMMEDIATE);
		...
		private void Update()
		{
			...
			{
				_breathInstance.set3DAttributes(base.transform.position.To3DAttributes());
		...
		[Rpc(RpcSources.All, RpcTargets.All, Key = 71377618u)]
		private void PlaySafeZoneSoundRpc()
		{
			if (!NetworkBehaviourUtils.CheckInvokeRpc((NetworkBehaviour)this))
			{
				RpcInvokeInfo invokeInfo;
				using (Fusion.RpcBuilder rpcBuilder = base.Runner.CreateRpcBuilder(71377618u, 0, (NetworkBehaviour)this, PlayerRef.Invalid))
				{
					invokeInfo = rpcBuilder.Prepare(out var _);
					if (NetworkBehaviourUtils.ShouldNotifyRpcError(invokeInfo))
					{
						NetworkBehaviourUtils.NotifyRpcError((ILogSource)this, "System.Void Features.AIModuleStateMachine.Scripts.ElPiratoDeTutorial.PirateAudioController::PlaySafeZoneSoundRpc()", invokeInfo, PlayerRef.None);
					}
		...
		[Rpc(RpcSources.All, RpcTargets.All, Key = 2452993761u)]
		private void PlayGoAwaySoundRpc()
		{
			if (!NetworkBehaviourUtils.CheckInvokeRpc((NetworkBehaviour)this))
			{
				RpcInvokeInfo invokeInfo;
				using (Fusion.RpcBuilder rpcBuilder = base.Runner.CreateRpcBuilder(2452993761u, 0, (NetworkBehaviour)this, PlayerRef.Invalid))
				{
					invokeInfo = rpcBuilder.Prepare(out var _);
					if (NetworkBehaviourUtils.ShouldNotifyRpcError(invokeInfo))
					{
						NetworkBehaviourUtils.NotifyRpcError((ILogSource)this, "System.Void Features.AIModuleStateMachine.Scripts.ElPiratoDeTutorial.PirateAudioController::PlayGoAwaySoundRpc()", invokeInfo, PlayerRef.None);
					}
					if (invokeInfo.SendMessageResult == RpcSendMessageResult.Sent)
					{
						rpcBuilder.Send();
		...
		private void PlayOneShot(EventReference reference)
		{
			if (!reference.IsNull)
			{
				EventInstance soundImmediately = RuntimeManager.CreateInstance(reference);
				soundImmediately.set3DAttributes(base.transform.position.To3DAttributes());
				SetSoundImmediately(soundImmediately);
				soundImmediately.start();
				soundImmediately.release();
			}
		}
		...
		[NetworkRpcWeavedInvoker(71377618u)]
		[Preserve]
		[WeaverGenerated]
		protected static void PlaySafeZoneSoundRpc_0040Invoker71377618([In] ref RpcInvokeContext context)
		{
		...
		[NetworkRpcWeavedInvoker(2452993761u)]
		[Preserve]
		[WeaverGenerated]
		protected static void PlayGoAwaySoundRpc_0040Invoker2452993761([In] ref RpcInvokeContext context)
		{
			NetworkRunner.DebugRpcEvent?.Invoke(NetworkRunnerDebugRpcEvent.FromRemoteCall(in context, MethodBase.GetMethodFromHandle((RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/), RpcHostMode.SourceIsServer));
			NetworkBehaviourUtils.InvokeRpc = true;
