...
{
	public class EarPostAttackWanderingState : StateBase<EarStateId>
	{
		...
		public EarPostAttackWanderingState(EarEnemy enemy, EarEnemyContext context, EarEnemySettings settings)
			: base(false, false)
		{
			_enemy = enemy;
			_context = context;
		...
		{
			_enemy.SetCurrentStateId(EarStateId.PostAttackWandering);
		...
		{
			if (_context.CurrentStateTime >= _settings.PostAttackWanderDuration)
			{
				_enemy.TriggerEvent(EarEvent.OnPostAttackWanderFinished);
			}
		}
	}
}
