	...
	internal class PhotonTransportProtocol
	{
		...
		internal void onVoiceEvent(object content0, int channelId, int playerId, bool isLocalPlayer)
		{
			...
			if ((byte)array[0] == 0)
			{
				...
				case 2:
					...
					break;
				default:
					logger.Log(LogLevel.Error, "[PV] Unknown sevent subcode " + array[1]);
					break;
				}
				return;
			}
			byte voiceId = (byte)array[0];
			ushort num = (byte)array[1];
			...
			if (array.Length > 5)
			{
				num += (ushort)((byte)array[5] << 8);
			}
			FrameBuffer receivedBytes = new FrameBuffer(array2, flags, frameNum);
			voiceClient.onFrame(playerId, voiceId, num, ref receivedBytes, isLocalPlayer);
		...
		private void onVoiceInfo(int channelId, int playerId, object payload)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				dictionary.TryGetValue(13, out var value);
				VoiceInfo info = createVoiceInfoFromEventPayload(dictionary);
				voiceClient.onVoiceInfo(channelId, playerId, voiceId, (value != null) ? ((int)value) : 0, info);
			}
		}
