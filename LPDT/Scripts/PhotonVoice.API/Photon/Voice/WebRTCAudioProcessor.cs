...
namespace Photon.Voice
{
	public class WebRTCAudioProcessor : WebRTCAudioLib, IProcessor<short>, IDisposable
	{
		...
		private ArrayPool<short> reverseBufferFactory;
		...
		public bool Bypass
		{
			...
			set
			{
				if (bypass != value)
				{
					logger.Log(LogLevel.Info, "[PV] WebRTCAudioProcessor: setting bypass=" + value);
				}
				bypass = value;
			}
		}
		...
		public WebRTCAudioProcessor(ILogger logger, int frameSize, int samplingRate, int channels, int reverseSamplingRate, int reverseChannels)
		{
			...
			{
				logger.Log(LogLevel.Error, "[PV] WebRTCAudioProcessor: input frame size ({0} samples / {1} ms) must be equal to or N times more than webrtc processing frame size ({2} samples / 10 ms)", inFrameSize, 1000f * (float)inFrameSize / (float)samplingRate, processFrameSize);
				disposed = true;
				return;
			}
			...
			logger.Log(LogLevel.Info, "[PV] WebRTCAudioProcessor create sampling rate {0}, channels{1}, frame size {2}, frame samples {3}, reverseChannels {4}", samplingRate, channels, processFrameSize, inFrameSize / this.channels, this.reverseChannels);
		}
		...
		private void InitReverseStream()
		{
			lock (this)
			{
				...
				{
					int num = processFrameSize * reverseChannels;
					if (samplingRate != reverseSamplingRate)
					{
						reverseFramer = new FramerResampler<float>(num, reverseChannels, samplingRate, reverseSamplingRate, interpolate: true);
					}
					else
					{
						reverseFramer = new Framer<float>(num);
					}
					reverseBufferFactory = new ArrayPool<short>(50, "WebRTCAudioProcessor Reverse Buffers", num);
					logger.Log(LogLevel.Info, "[PV] WebRTCAudioProcessor Init reverse stream: frame size {0}, reverseSamplingRate {1}, reverseChannels {2}", num, reverseSamplingRate, reverseChannels);
		...
		public short[] Process(short[] buf)
		{
			...
			{
				logger.Log(LogLevel.Error, "[PV] WebRTCAudioProcessor Process: frame size expected: {0}, passed: {1}", inFrameSize, buf.Length);
				return buf;
			}
			bool flag = false;
			for (int i = 0; i < inFrameSize; i += processFrameSize)
			{
				...
				{
					lastProcessErr = num;
					logger.Log(LogLevel.Error, "[PV] WebRTCAudioProcessor Process: webrtc_audio_processor_process() error {0}", num);
					return buf;
				}
			}
			if (vad && !flag)
		...
		public void OnAudioOutFrameFloat(float[] data)
		{
			...
			foreach (float[] item in reverseFramer.Frame(data))
			{
				...
				lock (reverseStreamQueue)
				{
					...
					{
						logger.Log(LogLevel.Error, "[PV] WebRTCAudioProcessor Reverse stream queue overflow");
		...
		private void ReverseStreamThread()
		{
				...
				while (!disposed)
				{
					...
					while (true)
					{
						...
						if (lastProcessReverseErr != num)
						{
							...
							logger.Log(LogLevel.Error, "[PV] WebRTCAudioProcessor: OnAudioOutFrameFloat: webrtc_audio_processor_process_reverse() error {0}", num);
						}
					}
				}
			}
			...
			{
				logger.Log(LogLevel.Error, "[PV] WebRTCAudioProcessor: ReverseStreamThread Exceptions: " + ex);
			}
			finally
			{
		...
		private int setParam(Param param, int v)
		{
			...
			logger.Log(LogLevel.Info, "[PV] WebRTCAudioProcessor: setting param " + param.ToString() + "=" + v);
