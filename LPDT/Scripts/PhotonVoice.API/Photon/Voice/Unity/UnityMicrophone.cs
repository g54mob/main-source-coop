	...
	public static class UnityMicrophone
	{
		...
		public static string CheckDevice(ILogger logger, string logPref, string device, int suggestedFrequency, out int frequency)
		{
			if (Microphone.devices.Length < 1)
			{
				...
				logger.Log(LogLevel.Error, logPref + text);
				frequency = 0;
				return text;
			}
			if (!string.IsNullOrEmpty(device) && !Microphone.devices.Contains(device))
			{
				...
				logger.Log(LogLevel.Error, logPref + text2);
				frequency = 0;
				return text2;
			}
			...
			if (Application.platform == RuntimePlatform.PS4 || Application.platform == RuntimePlatform.PS5)
			{
				if (suggestedFrequency != minFreq && suggestedFrequency != maxFreq)
				{
					...
					logger.Log(LogLevel.Warning, logPref + "microphone does not support suggested frequency {0} (supported frequencies are: {1} and {2}). Setting to {3}", suggestedFrequency, minFreq, maxFreq, num);
					frequency = num;
				}
			}
			else if (suggestedFrequency < minFreq || (maxFreq != 0 && suggestedFrequency > maxFreq))
			{
				logger.Log(LogLevel.Warning, logPref + "microphone does not support suggested frequency {0} (min: {1}, max: {2}). Setting to {2}", suggestedFrequency, minFreq, maxFreq);
				frequency = maxFreq;
			}
			return null;
		}
