	...
	public class AndroidAudioInAEC : IAudioPusher<short>, IAudioDesc, IDisposable, IResettable
	{
		...
		public AndroidAudioInAEC(ILogger logger, bool enableAEC = false, bool enableAGC = false, bool enableNS = false)
		{
			...
			try
			{
				...
				logger.Log(LogLevel.Info, "[PV] AndroidAudioInAEC: AndroidJavaObject created: aec: {0}/{1}, agc: {2}/{3}, ns: {4}/{5} minBufSize: {6}", enableAEC, audioIn.Call<bool>("AECIsAvailable", Array.Empty<object>()), enableAGC, audioIn.Call<bool>("AGCIsAvailable", Array.Empty<object>()), enableNS, audioIn.Call<bool>("NSIsAvailable", Array.Empty<object>()), num);
				...
				if (flag)
				{
					...
					logger.Log(LogLevel.Info, "[PV] AndroidAudioInAEC: AndroidJavaObject started: {0}, sampling rate: {1}, channels: {2}, record buffer size: {3}", flag, SamplingRate, Channels, num * 4);
				}
				else
				{
