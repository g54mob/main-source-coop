...
namespace Photon.Voice
{
	public struct RemoteVoiceOptions
	{
		...
		{
			logger.Log(LogLevel.Info, logPrefix + ": Creating default decoder " + voiceInfo.Codec.ToString() + " for output FrameOut<" + typeof(T)?.ToString() + ">");
			if (voiceInfo.Codec == Codec.AudioOpus)
			{
				Decoder = new OpusCodec.Decoder<T>(output, logger);
				return;
			}
			logger.Log(LogLevel.Error, logPrefix + ": FrameOut<" + typeof(T)?.ToString() + "> output set for non-audio decoder " + voiceInfo.Codec);
		}
	}
}
