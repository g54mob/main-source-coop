		...
		public abstract class Encoder<T> : IEncoderDirect<T[]>, IEncoder, IDisposable
		{
			...
			protected Encoder(VoiceInfo i, ILogger logger)
			{
				try
				{
					...
					logger.Log(LogLevel.Info, "[PV] OpusCodec.Encoder created. Opus version " + version + ", " + voiceInfo.ToString());
				}
				catch (Exception ex)
				{
					Error = ex.ToString();
		...
		public class Decoder<T> : IDecoder, IDisposable
		{
			...
			public void Open(VoiceInfo i)
			{
				try
				{
					...
					string version = Version;
					VoiceInfo voiceInfo = i;
					obj.Log(LogLevel.Info, "[PV] OpusCodec.Decoder created. Opus version " + version + ", " + voiceInfo.ToString());
				}
				catch (Exception ex)
				{
					Error = ex.ToString();
