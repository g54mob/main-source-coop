...
namespace POpusCodec
{
	...
	{
		private const string prefix_sym = "";
		...
		public static int opus_decode(IntPtr st, FrameBuffer data, short[] pcm, int frame_size, int decode_fec)
		{
			...
			int num = opus_decode(st, data.Ptr, data.Length, pcm, frame_size, decode_fec);
			if (num == -4)
			{
				return 0;
			}
			if (num < 0)
			{
				HandleStatusCode((OpusStatusCode)num, "opus_decode/short", data.Length, pcm.Length, frame_size, decode_fec);
			}
			return num;
		}
		...
		public static int opus_decode(IntPtr st, FrameBuffer data, float[] pcm, int frame_size, int decode_fec)
		{
			...
			int num = opus_decode_float(st, data.Ptr, data.Length, pcm, frame_size, decode_fec);
			if (num == -4)
			{
				return 0;
			}
			if (num < 0)
			{
				HandleStatusCode((OpusStatusCode)num, "opus_decode/float", data.Length, pcm.Length, frame_size, decode_fec);
			}
			return num;
		}
