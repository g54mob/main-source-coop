	...
	public class SaveOutgoingStreamToFile : VoiceComponent
	{
		...
		private void PhotonVoiceCreated(PhotonVoiceCreatedParams photonVoiceCreatedParams)
		{
			...
			if (photonVoiceCreatedParams.Voice is LocalVoiceAudioFloat)
			{
				...
				base.Logger.Log(LogLevel.Info, "Outgoing 32 bit stream {0}, output file path: {1}", info, filePath);
			...
			else if (photonVoiceCreatedParams.Voice is LocalVoiceAudioShort)
			{
				...
				base.Logger.Log(LogLevel.Info, "Outgoing 16 bit stream {0}, output file path: {1}", info, filePath);
		...
		{
			wavWriter.Dispose();
			base.Logger.Log(LogLevel.Info, "Recording stopped: Saving wav file.");
		}
	}
}
