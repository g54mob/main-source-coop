...
namespace Photon.Voice.Unity
{
	...
	public class RecorderPreset : VoiceComponent
	{
		...
		protected override void Awake()
		{
			...
			if (component == null)
			{
				base.Logger.Log(LogLevel.Error, "Can't find Recorder component");
				return;
			}
			base.Logger.Log(LogLevel.Info, "Updating from preset for platform '{0}': Microphone Type = {1}, DSP Enabled = {2}", Application.platform, MicrophoneType, DSPEnabled);
			component.MicrophoneType = MicrophoneType;
			if (component2 == null)
			{
				base.Logger.Log(LogLevel.Error, "Can't find WebRtcAudioDsp component");
				return;
			}
			component2.enabled = DSPEnabled;
			if (DSPEnabled)
			{
				base.Logger.Log(LogLevel.Info, "Updating from preset for platform '{0}': DSP.AEC = {1}, DSP.VAD = {2}", Application.platform, DSPSettings.AEC, DSPSettings.VAD);
