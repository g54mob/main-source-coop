...
namespace Fusion
{
	public static class Log
	{
		...
		{
			public readonly CreateLogStreamDelegate StreamFactory;

			public readonly LogSettings Settings;

			public Factory(LogSettings settings, CreateLogStreamDelegate streamFactory)
			{
			...
			{
				DisposeAndNullify(ref stream);
				if (StreamFactory != null && (Settings.TraceChannels & channel) == channel)
				{
		...
		{
			Factory factory = new Factory(settings, (LogLevel type, LogFlags flags, TraceChannels channel) => new ConsoleLogStream(((flags & LogFlags.Debug) != 0) ? ConsoleColor.DarkGray : ConsoleColor.Gray, ((flags & LogFlags.Debug) != 0) ? "[DEBUG] " : ""));
			InitInternal(in factory);
		}

		public static void InitializeForConsole(LogLevel logLevel, TraceChannels traceChannels = (TraceChannels)0)
		{
			InitializeForConsole(new LogSettings(logLevel, traceChannels));
		}
		...
		private static void InitPartial(in Factory factory)
		{
			...
			factory.Init(ref InternalLogStreams.LogTraceAreaOfInterest, TraceChannels.AreaOfInterest);
			...
			factory.Init(ref InternalLogStreams.LogTraceSendRecv, TraceChannels.SendRecv);
			factory.Init(ref InternalLogStreams.LogTracePools, TraceChannels.Pools);
			factory.Init(ref InternalLogStreams.LogTraceParent, TraceChannels.Parent);
			factory.Init(ref InternalLogStreams.LogTraceForecast, TraceChannels.Forecast);
			factory.Init(ref InternalLogStreams.LogTraceAutomatedRun, TraceChannels.AutomatedRun);
			factory.Init(ref InternalLogStreams.LogTracePluginVersion, TraceChannels.PluginVersion);
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Conditional("FUSION_TRACE_REALTIME")]
		public static void TraceRealtime(string msg)
		{
			InternalLogStreams.LogTraceRealtime?.InfoStream.Log(msg);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Conditional("FUSION_TRACE_REALTIME")]
		public static void TraceRealtimeWarn(string msg)
		{
			InternalLogStreams.LogTraceRealtime?.WarnStream.Log(msg);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Conditional("FUSION_TRACE_REALTIME")]
		public static void TraceRealtimeError(string msg)
		{
			InternalLogStreams.LogTraceRealtime?.ErrorStream.Log(msg);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Conditional("FUSION_TRACE_REALTIME")]
		public static void TraceRealtime<T>(T source, string msg) where T : ILogSource
		{
			InternalLogStreams.LogTraceRealtime?.InfoStream.Log(source, msg);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Conditional("FUSION_TRACE_REALTIME")]
		public static void TraceRealtimeWarn<T>(T source, string msg) where T : ILogSource
		{
			InternalLogStreams.LogTraceRealtime?.WarnStream.Log(source, msg);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Conditional("FUSION_TRACE_REALTIME")]
		public static void TraceRealtimeError<T>(T source, string msg) where T : ILogSource
		{
			InternalLogStreams.LogTraceRealtime?.ErrorStream.Log(source, msg);
		}
	}
}
