...
namespace Features.AIModuleStateMachine.Scripts.Enemies.CrabEnemy.Systems
{
	...
	public class CrabVisionDetectingSystem : MonoSystem
	{
		[SerializeField]
		...
		[Inject]
		private void InjectDependencies(IDetectionContext detectionContext, SpawnedPlayersModel spawnedPlayersModel, PlayersGatesModelSynchronizedModel playersGatesModel, IEnemyPlayerAttackabilityService enemyPlayerAttackabilityService)
		{
		...
		private void Update()
		{
			...
			foreach (KeyValuePair<PlayerRef, PlayerDataHolder> player2 in _spawnedPlayersModel.Players)
			{
				...
				if (!_enemyPlayerAttackabilityService.CanEnemyTargetPlayer(key.PlayerId))
				{
					DropConfirmedTarget(key);
					continue;
				}
		...
		private void DropConfirmedTarget(PlayerRef player)
		{
			_confirmedTargets.Remove(player);
			_visibleTime[player] = 0f;
			_lostTime[player] = 0f;
