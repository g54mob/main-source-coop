...
{
	public class CrabHaulState : StateBase<CrabStateId>
	{
		private readonly CrabEnemy _enemy;

		private readonly CrabEnemyContext _context;

		public CrabHaulState(CrabEnemy enemy, CrabEnemyContext context)
			: base(false, false)
		{
			_enemy = enemy;
			_context = context;
		...
		{
			_enemy.SetCurrentStateId(CrabStateId.Haul);
			_context.SetVisualState(CrabVisualState.CarryingItem);
		...
		public override void OnLogic()
		{
			...
			else if (_context.AttackCooldown <= 0f && _context.CanEnemyTargetPriorityPlayer())
			{
			...
			else if (_context.TargetPositionCompleted)
			{
				...
				_enemy.TriggerEvent(CrabEvent.OnItemDropped);
			}
		}
	}
}
