...
namespace Features.AIModuleStateMachine.Scripts.Enemies.BartenderEnemy
{
	...
	public class BartenderLipSyncController : NetworkBehaviour
	{
		...
		public unsafe int CurrentOpenAmount
		{
			get
			{
				if (Ptr == null)
				{
					throw new InvalidOperationException("Error when accessing BartenderLipSyncController.CurrentOpenAmount. Networked properties can only be accessed when Spawned() has been called.");
				}
		...
		public unsafe NetworkBool IsMouthOpenWithTongue
		{
			get
			{
				if (Ptr == null)
				{
					throw new InvalidOperationException("Error when accessing BartenderLipSyncController.IsMouthOpenWithTongue. Networked properties can only be accessed when Spawned() has been called.");
				}
