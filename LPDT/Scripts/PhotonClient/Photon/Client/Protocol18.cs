...
namespace Photon.Client
{
	public class Protocol18 : Protocol
	{
		...
		public override EventData DeserializeEventData(StreamBuffer din, EventData target = null, DeserializationFlags flags = DeserializationFlags.None)
		{
			...
			for (uint num2 = 0u; num2 < num; num2++)
			{
				...
				if (b == eventData.SenderKey)
				{
					...
					default:
					{
						object arg = Read(din, b2, flags, eventData.Parameters);
						Debug.WriteLine($"Sender key has invalid (gp)type: {b2} value: {arg}");
						break;
					}
					}
				}
				...
				{
					object arg = ((!flag) ? Read(din, b2, flags, eventData.Parameters) : ((b2 != 67) ? Read(din, b2, flags, eventData.Parameters) : ReadNonAllocByteArray(din)));
					eventData.Parameters.Add(b, arg);
				}
			}
			return eventData;
		}
		...
		private void WriteCustomTypeBody(CustomType customType, StreamBuffer stream, object value)
		{
			...
			if (num2 != num)
			{
				Debug.WriteLine($"Serialization for Custom Type '{value.GetType()}' returns size {num} bytes but wrote {num2} bytes. Sending the latter as size.");
			}
