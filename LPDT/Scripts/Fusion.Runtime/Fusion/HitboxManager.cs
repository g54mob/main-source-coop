...
namespace Fusion
{
	...
	[HelpURL("https://doc.photonengine.com/fusion/v2/manual/advanced/lag-compensation")]
	public sealed class HitboxManager : SimulationBehaviour, IAfterTick, IPublicFacingInterface, IBeforeSimulation, ISpawned
	{
		...
		internal LagCompensationStatisticsSnapshot LagCompensationSnapshot => _lagCompStatManager.CompletedSnapshot;
		...
		{
			Assert.Check(hits != null, "hits != null");
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void GetPlayerTickAndAlpha(PlayerRef player, out int? tickFrom, out int? tickTo, out float? alpha)
		{
			...
			Assert.Check(tickFrom.HasValue, "tickFrom.HasValue");
		}
		...
		private int QueryInternal(Query query, List<LagCompensatedHit> hits, bool clearHits)
		{
			...
			Assert.Check(query.Tick.HasValue, "query.Tick.HasValue");
			...
			if (query.Options.Has(HitOptions.IncludePhysX) || query.Options.Has(HitOptions.IncludeBox2D))
			{
		...
		private void PositionRotationInternal(ref PositionRotationQueryParams param, out Vector3 position, out Quaternion rotation)
		{
			...
			else if (param.QueryParams.Options.HasNot(HitOptions.SubtickAccuracy) && param.QueryParams.Alpha.HasValue && param.QueryParams.Alpha.Value > 0.5f)
			{
		...
		private void RegisterHitboxSnapshot(int tick, int dataTick)
		{
			...
			Assert.Check(_hitboxBuffer != null, "_hitboxBuffer != null");
