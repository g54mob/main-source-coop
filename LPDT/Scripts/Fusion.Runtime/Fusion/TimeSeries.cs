...
namespace Fusion
{
	internal class TimeSeries
	{
		...
		private static readonly Comparison<double> DoubleComparison = (double a, double b) => a.CompareTo(b);
		...
		public double MedianAbsDev
		{
			get
			{
				...
				try
				{
					for (int i = 0; i < Count; i++)
					{
						_list.Add(Math.Abs(_samples[i] - median));
					}
					return 1.4826 * FindMedian(_list);
				}
				finally
				{
					_list.Clear();
		...
		{
			Assert.Check(!double.IsNaN(value), "!double.IsNaN(value)");
			Assert.Check(!double.IsInfinity(value), "!double.IsInfinity(value)");
			double mean = _mean;
			if (IsFull)
			{
