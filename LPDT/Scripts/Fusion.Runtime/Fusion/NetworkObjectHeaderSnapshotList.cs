...
namespace Fusion
{
	internal struct NetworkObjectHeaderSnapshotList
	{
		...
		public readonly NetworkObjectHeaderSnapshot Oldest => _tail;

		public readonly NetworkObjectHeaderSnapshot Latest => _head;
		...
		public void AddBefore(NetworkObjectHeaderSnapshot before, NetworkObjectHeaderSnapshot item)
		{
			...
			Assert.Check(IsInList(before), "IsInList(before)");
			Assert.Check(!IsInList(item), "IsInList(item) == false");
			if (before == _head)
			{
				AddFirst(item);
		...
		{
			Assert.Check(_count > 0, "_count > 0");
			Assert.Check(_tail != null, "_tail  != null");
			Assert.Check(IsInList(_tail), "IsInList(_tail)");
		...
		{
			Assert.Check(_count > 0, "_count > 0");
			Assert.Check(_head != null, "_head  != null");
			Assert.Check(IsInList(_head), "IsInList(_head)");
