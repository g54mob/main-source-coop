...
namespace Fusion
{
	...
	public struct NetworkButtons : INetworkStruct, IEquatable<NetworkButtons>
	{
		...
		internal const int BYTE_OF__BITS = 0;

		internal const int BYTE_COUNT_OF__BITS = 4;
		...
		{
			Assert.Check((uint)button < 32u, "(uint) button < 32u");
		...
		{
			Assert.Check(typeof(T).GetEnumUnderlyingType() == typeof(int), "typeof(T).GetEnumUnderlyingType() == typeof(int)");
		...
		public readonly bool Equals(NetworkButtons other)
		{
