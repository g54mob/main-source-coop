...
namespace Fusion
{
	...
	public class HitboxRoot : NetworkBehaviour
	{
		...
		{
			bool includeInactive = Config.Has(ConfigFlags.IncludeInactiveHitboxes);
			...
			{
				InternalLogStreams.LogWarn?.Log($"Hitbox count above limit per root, clamped to max {31}");
		...
		{
			if (Config.Has(ConfigFlags.ReinitializeHitboxesBeforeRegistration))
			{
				InitHitboxes();
			}
			if (Hitboxes == null)
