	...
	public sealed class NetworkRunnerUpdaterDefault : INetworkRunnerUpdater
	{
		...
		private class PlayerLoopSystemRegistration : IDisposable
		{
			...
			public PlayerLoopSystemRegistration(NetworkRunnerUpdaterDefaultInvokeSettings updateSettings, NetworkRunnerUpdaterDefaultInvokeSettings renderSettings)
			{
				...
				parentSystem.AddToPlayerLoop(UpdateSettings.ReferencePlayerLoopSystem, UpdateSettings.AddMode, typeof(NetworkRunnerUpdate), InvokeUpdate);
				parentSystem.AddToPlayerLoop(RenderSettings.ReferencePlayerLoopSystem, RenderSettings.AddMode, typeof(NetworkRunnerRender), InvokeRender);
			...
			public void Dispose()
			{
				...
				PlayerLoopSystem parentSystem = PlayerLoop.GetCurrentPlayerLoop();
				parentSystem.RemoveFromPlayerLoop(typeof(NetworkRunnerUpdate));
				parentSystem.RemoveFromPlayerLoop(typeof(NetworkRunnerRender));
				PlayerLoop.SetPlayerLoop(parentSystem);
			}
		}
		...
		private static void InvokeRender()
		{
			...
			Assert.Check(_instanceCount <= _instances.Count, "_instanceCount <= _instances.Count");
			bool flag = false;
			try
			{
				for (int i = 0; i < _instanceCount; i++)
