...
namespace Fusion
{
	public readonly ref struct NetworkBehaviourBufferInterpolator
	{
		...
		{
			Behaviour = nb;
			Valid = nb.TryGetSnapshotsBuffers(out From, out To, out Alpha);
		}
		...
		[Obsolete("Use NetworkBool instead. It is strongly recommended to use NetworkBool instead of bool. For interpolating a bool Select can be used if necessary.", false)]
		...
		public NetworkBool NetworkBool(NetworkBehaviour.PropertyReader<NetworkBool> property)
		{
			return Select(property);
		}

		public NetworkBool NetworkBool(string property)
		{
			return Select<NetworkBool>(property);
		}
