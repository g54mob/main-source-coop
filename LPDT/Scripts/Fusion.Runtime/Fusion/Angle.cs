...
namespace Fusion
{
	...
	public struct Angle : INetworkStruct, IEquatable<Angle>
	{
		...
		internal const int BYTE_OF__VALUE = 0;

		internal const int BYTE_COUNT_OF__VALUE = 4;

		private const uint __STATIC_ASSERT_ENSURE_PERFECT_FIT = 1u;

		public void Clamp(Angle min, Angle max)
		{
			Assert.Check(max._value >= min._value, "max._value >= min._value");
		...
		{
			Assert.Check(a._value >= 0 && a._value <= 3600000, "a._value >= 0 && a._value <= _360");
			Assert.Check(b._value >= 0 && b._value <= 3600000, "b._value >= 0 && b._value <= _360");
			a._value += b._value;
			if (a._value > 3600000)
			{
				a._value %= 3600000;
		...
		public static Angle operator -(Angle a, Angle b)
		{
			...
			if (a._value < 0)
			{
				Assert.Check(a._value >= -3600000, "a._value >= -_360");
