...
namespace Fusion
{
	...
	[HelpURL("https://doc.photonengine.com/fusion/v2/manual/sync-components/network-trsp")]
	[NetworkBehaviourWeaved(14)]
	public abstract class NetworkTRSP : NetworkBehaviour
	{
		...
		public bool IsMainTRSP
		{
			get
			{
				...
				if (BehaviourUtils.IsNull(base.Object))
				{
					return false;
				}
				return base.Object.Flags.Has(NetworkObjectFlags.HasMainNetworkTRSP);
			}
		}

		public NetworkTRSPData Data => base.StateBufferIsValid ? State : default(NetworkTRSPData);
		...
		protected ref NetworkTRSPData State
		{
			...
			get
			{
				return ref ReinterpretState<NetworkTRSPData>();
			}
		}
		...
		public virtual void SetAreaOfInterestOverride(NetworkObject obj)
		{
			if ((bool)obj)
			{
				Assert.Always(obj.Flags.Has(NetworkObjectFlags.HasMainNetworkTRSP), "area of interest proxy must have a main network trsp");
			}
		...
		{
			ref NetworkTRSPData state = ref behaviour.State;
		...
		protected static void ResolveAOIOverride(NetworkTRSP behaviour, Transform parent)
		{
			...
			{
				if (transform.TryGetComponent<NetworkObject>(out var component) && component.Flags.Has(NetworkObjectFlags.HasMainNetworkTRSP))
				{
		...
		{
			if (base.Object != null && base.Object.RuntimeFlags.Has(NetworkObjectRuntimeFlags.Spawned))
			{
		...
		{
			Render(behaviour, transform, syncScale, syncParent, local, checkForecastKinematic: false, ref initial);
		}

		internal static bool Render(NetworkTRSP behaviour, Transform transform, bool syncScale, bool syncParent, bool local, bool checkForecastKinematic, ref Tick initial)
		{
			if (behaviour.TryGetSnapshotsBuffers(out var from, out var to, out var alpha))
			{
				...
				if (checkForecastKinematic)
				{
					NetworkPhysicsData networkPhysicsData = from.ReinterpretState<NetworkPhysicsData>(14);
					NetworkPhysicsData networkPhysicsData2 = to.ReinterpretState<NetworkPhysicsData>(14);
					if (networkPhysicsData.Flags.HasNot(NetworkRigidbodyFlags.IsKinematic) || networkPhysicsData2.Flags.HasNot(NetworkRigidbodyFlags.IsKinematic))
					{
						return false;
					}
				}
