...
namespace Fusion
{
	internal class SimulationInputCollection
	{
		...
		private readonly SimulationInput[] _byIndex;

		private readonly Dictionary<PlayerRef, SimulationInput> _byPlayer;
		...
		{
			int num = _count++;
			Assert.Check(_byIndex[num] == null, "_byIndex[index] == null");
			Assert.Check(!_byPlayer.ContainsKey(input.Player), "_byPlayer.ContainsKey(input.Player) == false");
			_byIndex[num] = input;
