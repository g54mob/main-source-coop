...
namespace Fusion
{
	[HelpURL("https://doc.photonengine.com/fusion/v2/manual/sync-components/network-network-mecanim-animator")]
	...
	public sealed class NetworkMecanimAnimator : NetworkBehaviour, IAfterAllTicks, IPublicFacingInterface
	{
		internal readonly struct AnimatorData
		{
			...
			{
				Assert.Check(animator, "animator");
			...
			{
				bool flag = syncSettings.Has(AnimatorSyncSettings.ParameterFloats);
				bool flag2 = syncSettings.Has(AnimatorSyncSettings.ParameterInts);
				bool flag3 = syncSettings.Has(AnimatorSyncSettings.ParameterBools);
				bool flag4 = syncSettings.Has(AnimatorSyncSettings.ParameterTriggers);
				bool flag5 = syncSettings.Has(AnimatorSyncSettings.StateRoot);
				bool flag6 = syncSettings.Has(AnimatorSyncSettings.LayerWeights);
				bool flag7 = syncSettings.Has(AnimatorSyncSettings.StateLayers);
				param32Count = 0;
				paramBoolCount = 0;
				int i = 0;
		...
		private void CaptureAnimatorData()
		{
			...
			if (SyncSettings.Has(AnimatorSyncSettings.StateRoot))
			{
				CaptureStates(ref wordOffset);
			}
			if (SyncSettings.Has(AnimatorSyncSettings.LayerWeights))
			{
