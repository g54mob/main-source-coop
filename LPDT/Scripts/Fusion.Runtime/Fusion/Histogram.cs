...
namespace Fusion
{
	internal class Histogram
	{
		...
		{
			Assert.Check(error > 0.0, "error > 0.0");
		}

		public Histogram(double contrast, int resolution)
			: this(Capacity(contrast, resolution))
		{
			Assert.Check(resolution <= 20, "resolution <= MaxResolution");
		}
		...
		{
			Assert.Check(fraction >= 0.0 && fraction <= 1.0, "fraction >= 0.0 && fraction <= 1.0");
			try
			{
				double num;
				switch (estimator)
		...
		{
			Assert.Check(desiredMaxExp >= desiredMinExp, "desiredMaxExp >= desiredMinExp");
			int num = 0;
		...
		private static int Capacity(double contrast, int resolution)
		{
			...
			Assert.Check(num >= 1.0, "capacity >= 1.0");
			return (int)num;
		}
		...
		{
			Assert.Check(value != 0.0, "value != 0.0");
			double num = Math.Log(2.0);
			double num2 = 1.0 / num;
