...
namespace Fusion
{
	public struct TickAccumulator
	{
		...
		public readonly int Pending => _ticks;

		public readonly double Remainder => _time;

		public readonly bool Running => _running;
		...
		public double TimeScale
		{
			...
			set
			{
				Assert.Check(value > 0.0, "value > 0");
				_scale = value;
			}
		}
		...
		{
			Assert.Check(dt >= 0.0, "dt >= 0");
			Assert.Check(step > 0.0, "step > 0");
			if (!_running)
			{
				return;
			}
			Assert.Check(_scale > 0.0, "_scale > 0");
		...
		{
			Assert.Check(_ticks >= 0, "_ticks >= 0");
			if (_ticks > 0)
			{
				_ticks--;
				last = _ticks == 0;
