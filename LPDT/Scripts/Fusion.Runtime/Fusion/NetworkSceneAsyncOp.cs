...
namespace Fusion
{
	public readonly struct NetworkSceneAsyncOp : IEnumerator
	{
		public struct Awaiter : INotifyCompletion
		{
			...
			public readonly bool IsCompleted => _op.IsDone;
		...
		internal static NetworkSceneAsyncOp FromDeferred(SceneRef sceneRef, Task blockingTask, NetworkRunner runner, Func<NetworkRunner, SceneRef, NetworkSceneAsyncOp> op)
		{
			return FromTask(sceneRef, CreateDeferredOpTask(sceneRef, blockingTask, runner, op));
		}

		private static async Task CreateDeferredOpTask(SceneRef sceneRef, Task blockingTask, NetworkRunner runner, Func<NetworkRunner, SceneRef, NetworkSceneAsyncOp> op)
		{
			InternalLogStreams.LogTraceSceneManager?.Log(runner, $"Awaiting blocking task for {sceneRef}");
			await blockingTask;
			InternalLogStreams.LogTraceSceneManager?.Log(runner, $"Awaited blocking task for {sceneRef}, loading");
			await op(runner, sceneRef);
			InternalLogStreams.LogTraceSceneManager?.Log(runner, "Awaited loading");
		}
