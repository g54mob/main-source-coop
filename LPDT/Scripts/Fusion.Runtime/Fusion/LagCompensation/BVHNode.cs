...
namespace Fusion.LagCompensation
{
	internal struct BVHNode
	{
		...
		internal readonly int Index => _nodeIndex;
		...
		internal readonly bool IsRootNode
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get
			{
		...
		internal readonly bool HasParent
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get
			{
		...
		internal readonly bool HasLeft
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get
			{
		...
		internal readonly bool HasRight
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get
			{
		...
		internal readonly bool HasValidRoot
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get
			{
		...
		{
			Assert.Check(BehaviourUtils.IsAlive(_root), "IsAlive(_root)");
		...
		{
			Assert.Check(entries.Count > 0, "entries.Count > 0");
		...
		{
			Assert.Check(entries != null, "entries != null");
			Assert.Check(entries.Count > 1, "entries.Count > 1");
		...
		{
			Assert.Check(BehaviourUtils.IsAlive(entry), "IsAlive(entry)");
			if ((!startNode.HasLeft || !startNode.HasRight) && startNode.IsRootNode)
			{
				Assert.Check(!startNode.HasLeft && !startNode.HasRight, "startNode.HasLeft == false && startNode.HasRight == false");
				bvh.ReusableList.Clear();
				bvh.ReusableList.Add(entry);
				Assert.Check(startNode._isLeaf == BehaviourUtils.IsAlive(startNode._root), "startNode._isLeaf == (IsAlive(startNode._root))");
		...
		internal void Remove(BVH bvh, HitboxRoot entry)
		{
			...
			Assert.Check(BehaviourUtils.IsSame(_root, entry), "IsSame(_root, entry)");
			...
			Assert.Check(HasParent, "HasParent");
		...
		{
			Assert.Check(!IsLeaf, "IsLeaf == false");
			...
			Assert.Check(reference.IsLeaf, "removeLeaf.IsLeaf");
			if (IsRootNode)
			{
				this = reference2;
				_nodeIndex = 1;
			...
			else
			{
				...
				Assert.Check(Index == parent._leftIndex || Index == parent._rightIndex, "Index == parent._leftIndex || Index == parent._rightIndex");
				...
				Assert.Check(reference2.IsValid, "keepChild.IsValid");
				reference2._parentIndex = _parentIndex;
				reference2.SetDepth(bvh, reference2.Depth - 1);
				Assert.Check(reference2.HasParent, "keepChild.HasParent");
