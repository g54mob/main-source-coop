	...
	internal class HitboxBuffer
	{
		internal class HitboxSnapshot : IHitboxColliderContainer
		{
			...
			public void ReleaseCollider(int index)
			{
				...
				{
					throw new IndexOutOfRangeException($"Index {index} is out of valid range: (1, {CollidersCapacity})");
				}
				Assert.Check(_colliders[index].Used, index);
			...
			{
				bool flag = query.Options.Has(HitOptions.IgnoreInputAuthority) && query.Player.IsRealPlayer;
				bool flag2 = toContainer != null && query.Options.Has(HitOptions.SubtickAccuracy);
		...
		internal HitboxBuffer(List<HitboxRoot> initialObjects, int bufferSize, int hitboxCapacity, float expansionFactor)
		{
			...
			_head = 0;
			_advanced = 0;
			Assert.Check(Length > 0, "Length > 0");
		...
		internal void PositionQueryInternal(ref PositionRotationQueryParams param, out Vector3 position, out Quaternion rotation)
		{
			...
			if (param.QueryParams.Options.Has(HitOptions.SubtickAccuracy) && param.QueryParams.TickTo.HasValue && param.QueryParams.Alpha.HasValue)
			{
		...
		private void GetClosestSnapshotForTick(int tick, out HitboxSnapshot snapshot)
		{
			...
			int num3 = (_head + num + Length) % Length;
			snapshot = _buffer[num3];
			if (snapshot.Tick != tick)
			{
				int num4 = (num3 - 1) % Length;
				int num5 = (num3 + 1) % Length;
				InternalLogStreams.LogDebug?.Error($"The hitbox buffer seems to be missing the correct snapshot. Requested tick: {tick}, Snapshot found tick: {snapshot.Tick}," + $" Previous snapshot on buffer: {_buffer[num4].Tick}, Next snapshot on buffer: {_buffer[num5].Tick} Buffer current tick: {Tick}");
			}
			Assert.Check(snapshot.Tick == tick, "The hitbox buffer seems to be missing the correct snapshot.");
		}
		...
		private void QueryBroadphase(Query query, HashSet<int> processedColliderIndices, out IHitboxColliderContainer container)
		{
			...
			if (query.Options.Has(HitOptions.SubtickAccuracy) && query.TickTo.HasValue && query.Alpha.HasValue)
			{
		...
		{
			if (!query.TickTo.HasValue || query.Options.Has(HitOptions.SubtickAccuracy))
			{
