...
namespace QuickOutline.Scripts
{
	...
	public class Outline : MonoBehaviour
	{
		...
		[SerializeField]
		private bool _isAutomaticRenderersSearch = true;
		...
		[SerializeField]
		[Range(0f, 10f)]
		private int _outlineOverlapLayer = 5;
		...
		private void Awake()
		{
			if (_isAutomaticRenderersSearch)
			{
			...
			outlineFillMaterial.SetFloat("_OutlineFadeEnabled", _isFadeEnabled ? 1f : 0f);
			outlineFillMaterial.SetFloat("_OutlineFadeStartDistance", _startDistance);
			outlineFillMaterial.SetFloat("_OutlineFadeEndDistance", Mathf.Max(_endDistance, _startDistance));
			int num = 10 - _outlineOverlapLayer;
			int num2 = 3100 + num * 20;
			outlineMaskMaterial.renderQueue = num2;
			outlineFillMaterial.renderQueue = num2 + 10;
			LoadSmoothNormals();
			needsUpdate = true;
		}
		...
		private void Bake()
		{
			...
			{
				if (!(meshFilter.sharedMesh == null) && hashSet.Add(meshFilter.sharedMesh))
				{
