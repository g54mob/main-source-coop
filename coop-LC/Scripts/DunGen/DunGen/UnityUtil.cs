...
namespace DunGen
{
	public static class UnityUtil
	{
		...
		{
			Bounds result = CalculateObjectBounds(prefab, includeInactive: true, DunGenSettings.Instance.BoundsCalculationsIgnoreSprites);
		...
		public static Bounds CalculateObjectBounds(GameObject obj, bool includeInactive, bool ignoreSpriteRenderers, bool ignoreTriggerColliders = true)
		{
			...
			ProBuilderMesh[] componentsInChildren2 = obj.GetComponentsInChildren<ProBuilderMesh>();
			foreach (ProBuilderMesh proBuilderMesh in componentsInChildren2)
			{
				if (proBuilderMesh.meshSyncState != MeshSyncState.InSync)
				{
					proBuilderMesh.ToMesh();
					proBuilderMesh.Refresh();
				}
			}
			Renderer[] componentsInChildren3 = obj.GetComponentsInChildren<Renderer>(includeInactive);
			...
			Collider[] componentsInChildren4 = obj.GetComponentsInChildren<Collider>(includeInactive);
			...
			Terrain[] componentsInChildren5 = obj.GetComponentsInChildren<Terrain>(includeInactive);
			foreach (Terrain terrain in componentsInChildren5)
			{
