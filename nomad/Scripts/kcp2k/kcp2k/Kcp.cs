...
namespace kcp2k
{
	public class Kcp
	{
		...
		public int Input(byte[] data, int offset, int size)
		{
			...
			while (size >= 24)
			{
				offset += Utils.Decode32U(data, offset, out var value);
				if (value != conv)
				{
					return -1;
				}
				offset += Utils.Decode8u(data, offset, out var value2);
				offset += Utils.Decode8u(data, offset, out var value3);
				offset += Utils.Decode16U(data, offset, out var value4);
				offset += Utils.Decode32U(data, offset, out var value5);
				offset += Utils.Decode32U(data, offset, out var value6);
				offset += Utils.Decode32U(data, offset, out var value7);
				offset += Utils.Decode32U(data, offset, out var value8);
				size -= 24;
				if (size < value8 || (int)value8 < 0)
				{
					return -2;
				}
				if (value2 != 81 && value2 != 82 && value2 != 83 && value2 != 84)
				{
					return -3;
				}
				rmt_wnd = value4;
				...
				{
				case 82:
					if (Utils.TimeDiff(current, value5) >= 0)
					{
						UpdateAck(Utils.TimeDiff(current, value5));
					}
					ParseAck(value6);
					ShrinkBuf();
					if (num2 == 0)
					...
					else if (Utils.TimeDiff(value6, num) > 0)
					{
						num = value6;
						ts = value5;
					}
					break;
				case 81:
					if (Utils.TimeDiff(value6, rcv_nxt + rcv_wnd) >= 0)
					{
						break;
					}
					AckPush(value6, value5);
					if (Utils.TimeDiff(value6, rcv_nxt) >= 0)
					{
						Segment segment = SegmentNew();
						segment.conv = value;
						segment.cmd = value2;
						...
						if (value8 != 0)
						{
							segment.data.Write(data, offset, (int)value8);
						}
						ParseData(segment);
					}
					break;
		...
		public void Flush()
		{
			...
			uint num2 = ((fastresend > 0) ? ((uint)fastresend) : 4294967295u);
