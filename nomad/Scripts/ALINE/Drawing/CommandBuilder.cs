	...
	public struct CommandBuilder : IDisposable
	{
		...
		public struct PolylineWithSymbol
		{
			...
			public PolylineWithSymbol(SymbolDecoration symbol, float symbolSize, float symbolPadding, float symbolSpacing, bool reverseSymbols = false, float offset = 0f)
			{
				...
				connectingSegmentLength = math.max(0f, symbolSpacing - symbolPadding * 2f - symbolSize);
				symbolSpacing = symbolPadding * 2f + symbolSize + connectingSegmentLength;
				...
				this.offset = (connectingSegmentLength * 0.5f + offset) % symbolSpacing;
				if (this.offset > 0f)
				{
			...
			public void MoveTo(ref CommandBuilder draw, float3 next)
			{
				...
				{
					float6 = math.normalizesafe(math.cross(float5, math.cross(float5, up)));
				...
				while (true)
				{
					...
					{
						if (offset >= 0f && offset != num3)
						{
							num3 = math.max(0f, num3);
							float3 a = math.lerp(prev, next, num3 * num2);
					...
					else if (state == State.Symbol)
					{
						...
						if (offset >= 0f)
						{
							float3 center = math.lerp(prev, next, offset * num2);
							switch (symbol)
							{
						...
						num3 = offset;
						offset += 0f - symbolOffset + symbolSize + symbolPadding;
					}
					else
					{
						if (state != State.PostSymbolPadding)
						{
		...
		private class JobWireMesh
		{
			...
			[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
			internal unsafe delegate void WireMesh_0000010E_0024PostfixBurstDelegate(float3* verts, int* indices, int vertexCount, int indexCount, ref CommandBuilder draw);

			internal static class WireMesh_0000010E_0024BurstDirectCall
			{
				...
				private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
				{
					if (Pointer == (IntPtr)0)
					{
						Pointer = BurstCompiler.CompileFunctionPointer<WireMesh_0000010E_0024PostfixBurstDelegate>(WireMesh).Value;
					}
					P_0 = Pointer;
				}
			...
			[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
			internal delegate void Execute_0000010F_0024PostfixBurstDelegate(ref Mesh.MeshData rawMeshData, ref CommandBuilder draw);

			internal static class Execute_0000010F_0024BurstDirectCall
			{
				private static IntPtr Pointer;
				...
				private static void GetFunctionPointerDiscard(ref IntPtr P_0)
				{
					if (Pointer == (IntPtr)0)
					{
						Pointer = BurstCompiler.CompileFunctionPointer<Execute_0000010F_0024PostfixBurstDelegate>(Execute).Value;
					}
					P_0 = Pointer;
				}
			...
			[BurstCompile]
			[MonoPInvokeCallback(typeof(WireMesh_0000010E_0024PostfixBurstDelegate))]
			public unsafe static void WireMesh(float3* verts, int* indices, int vertexCount, int indexCount, ref CommandBuilder draw)
			{
				WireMesh_0000010E_0024BurstDirectCall.Invoke(verts, indices, vertexCount, indexCount, ref draw);
			}
			...
			{
				Execute_0000010F_0024BurstDirectCall.Invoke(ref rawMeshData, ref draw);
			}
		...
		internal static readonly float4x4 XZtoXYPlaneMatrix = float4x4.RotateX(-(float)Math.PI / 2f);

		internal static readonly float4x4 XZtoYZPlaneMatrix = float4x4.RotateZ((float)Math.PI / 2f);
		...
		internal unsafe CommandBuilder(DrawingData gizmos, DrawingData.Hasher hasher, RedrawScope frameRedrawScope, RedrawScope customRedrawScope, bool isGizmos, bool isBuiltInCommandBuilder)
		{
		...
		public void Arc(float3 center, float3 start, float3 end)
		{
			...
			if (math.any(float7 != 0f) && math.all(math.isfinite(float7)))
			{
				...
				float num = Vector3.SignedAngle(float5, float6, float7) * ((float)Math.PI / 180f);
				PushMatrix(matrix);
				CircleXZInternal(float3.zero, math.length(float5), (float)Math.PI / 2f, (float)Math.PI / 2f - num);
				PopMatrix();
			}
		}
		...
		public void SolidArc(float3 center, float3 start, float3 end)
		{
			...
			if (math.any(float7))
			{
				...
				float num = Vector3.SignedAngle(float5, float6, float7) * ((float)Math.PI / 180f);
				PushMatrix(matrix);
				SolidCircleXZInternal(float3.zero, math.length(float5), (float)Math.PI / 2f, (float)Math.PI / 2f - num);
				PopMatrix();
			}
		}
		...
		public void WireCapsule(float3 position, float3 direction, float length, float radius)
		{
			...
			PushMatrix(XZtoXYPlaneMatrix);
			CircleXZInternal(new float3(0f, 0f, radius), radius, (float)Math.PI);
			PopMatrix();
			PushMatrix(XZtoYZPlaneMatrix);
			CircleXZInternal(new float3(radius, 0f, 0f), radius, (float)Math.PI / 2f, 4.712389f);
			PopMatrix();
			if (length > 0f)
			{
				...
				PushMatrix(XZtoXYPlaneMatrix);
				CircleXZInternal(new float3(0f, 0f, num), radius, 0f, (float)Math.PI);
				PopMatrix();
				PushMatrix(XZtoYZPlaneMatrix);
				CircleXZInternal(new float3(num, 0f, 0f), radius, -(float)Math.PI / 2f, (float)Math.PI / 2f);
				PopMatrix();
		...
		public void ArrowheadArc(float3 origin, float3 direction, float offset, float width = 60f)
		{
			if (math.any(direction))
			{
				...
				if (offset != 0f)
				{
					...
					float num = (float)Math.PI / 2f - width * ((float)Math.PI / 360f);
					float num2 = (float)Math.PI / 2f + width * ((float)Math.PI / 360f);
		...
		public void WirePolygon(float3 center, int vertices, quaternion rotation, float radius)
		{
			...
			{
				float x = (float)Math.PI * 2f * ((float)i / (float)vertices);
		...
		public void WireRing(float3 center, float3 normal, float innerRadius, float outerRadius)
		{
			if (innerRadius < 0f)
			{
				throw new ArgumentOutOfRangeException("innerRadius", "Inner radius must be non-negative");
			}
			if (outerRadius < 0f)
			{
				throw new ArgumentOutOfRangeException("outerRadius", "Outer radius must be non-negative");
			}
			if (innerRadius > outerRadius)
			{
				throw new ArgumentException("Inner radius cannot be larger than outer radius");
			}
		...
		public void SolidRing(float3 center, float3 normal, float innerRadius, float outerRadius)
		{
			...
			{
				Quaternion quaternion2 = Quaternion.FromToRotation(new float3(0f, 1f, 0f), normal);
				PushMatrix(float4x4.TRS(center, quaternion2, Vector3.one));
				SolidRingXZInternal(float3.zero, innerRadius, outerRadius);
				PopMatrix();
			}
		}

		public void WireRing(float3 center, quaternion rotation, float innerRadius, float outerRadius, float startAngle = 0f, float endAngle = (float)Math.PI * 2f)
		{
			if (innerRadius < 0f)
			{
				throw new ArgumentOutOfRangeException("innerRadius", "Inner radius must be non-negative");
			...
			if (math.abs(endAngle - startAngle) < 6.2821856f)
			{
				math.sincos(startAngle, out var s, out var c);
				math.sincos(endAngle, out var s2, out var c2);
				float3 a = new float3(c * innerRadius, 0f, s * innerRadius);
				float3 b = new float3(c * outerRadius, 0f, s * outerRadius);
				float3 a2 = new float3(c2 * innerRadius, 0f, s2 * innerRadius);
				float3 b2 = new float3(c2 * outerRadius, 0f, s2 * outerRadius);
				Line(a, b);
				Line(a2, b2);
			}
			PopMatrix();
		...
		public void SolidRing(float3 center, quaternion rotation, float innerRadius, float outerRadius, float startAngle = 0f, float endAngle = (float)Math.PI * 2f)
		{
			if (innerRadius < 0f)
			{
				throw new ArgumentOutOfRangeException("innerRadius", "Inner radius must be non-negative");
		...
		internal void SolidRingXZInternal(float3 center, float innerRadius, float outerRadius, float startAngle = 0f, float endAngle = (float)Math.PI * 2f)
		{
			Reserve<RingData>();
			Add(Command.Ring);
			Add(new RingData
		...
		{
			CircleXZ(center, radius, 0f, (float)Math.PI * 2f, color);
		}
		...
		{
			CircleXY(center, radius, 0f, (float)Math.PI * 2f, color);
		}
		...
		{
			SolidCircleXZ(center, radius, 0f, (float)Math.PI * 2f, color);
		}
		...
		{
			SolidCircleXY(center, radius, 0f, (float)Math.PI * 2f, color);
		}
		...
		{
			WireRing(center, rotation, innerRadius, outerRadius, 0f, (float)Math.PI * 2f, color);
		}
		...
		{
			SolidRing(center, rotation, innerRadius, outerRadius, 0f, (float)Math.PI * 2f, color);
		}
