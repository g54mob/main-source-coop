...
namespace Drawing
{
	public struct CommandBuilder2D
	{
		...
		private static readonly quaternion XY_TO_XZ_ROTATION = quaternion.RotateX(-(float)Math.PI / 2f);
		...
		public void WirePill(float2 position, float2 direction, float length, float radius)
		{
			...
			Circle(new float2(0f, 0f), radius, (float)Math.PI / 2f, 4.712389f);
			Line(new float2(0f, 0f - radius), new float2(length, 0f - radius));
			Circle(new float2(length, 0f), radius, -(float)Math.PI / 2f, (float)Math.PI / 2f);
		...
		{
			draw.SolidPlane(xy ? new float3(rect.center.x, rect.center.y, 0f) : new float3(rect.center.x, 0f, rect.center.y), xy ? XY_TO_XZ_ROTATION : XZ_TO_XZ_ROTATION, new float2(rect.width, rect.height));
		}
		...
		public void WireRing(float2 center, float innerRadius, float outerRadius, float startAngle = 0f, float endAngle = (float)Math.PI * 2f)
		{
			WireRing(xy ? new float3(center, 0f) : new float3(center.x, 0f, center.y), innerRadius, outerRadius, startAngle, endAngle);
		}

		public void WireRing(float3 center, float innerRadius, float outerRadius, float startAngle = 0f, float endAngle = (float)Math.PI * 2f)
		{
			draw.WireRing(center, xy ? XY_TO_XZ_ROTATION : XZ_TO_XZ_ROTATION, innerRadius, outerRadius, startAngle, endAngle);
		}

		public void SolidRing(float2 center, float innerRadius, float outerRadius, float startAngle = 0f, float endAngle = (float)Math.PI * 2f)
		{
			SolidRing(xy ? new float3(center, 0f) : new float3(center.x, 0f, center.y), innerRadius, outerRadius, startAngle, endAngle);
		}

		public void SolidRing(float3 center, float innerRadius, float outerRadius, float startAngle = 0f, float endAngle = (float)Math.PI * 2f)
		{
			if (xy)
			{
				draw.PushMatrix(XZ_TO_XY_MATRIX);
		...
		public void WireRing(float2 center, quaternion rotation, float innerRadius, float outerRadius, float startAngle = 0f, float endAngle = (float)Math.PI * 2f)
		{
			WireRing(xy ? new float3(center, 0f) : new float3(center.x, 0f, center.y), rotation, innerRadius, outerRadius, startAngle, endAngle);
		}
		...
		public void SolidRing(float2 center, quaternion rotation, float innerRadius, float outerRadius, float startAngle = 0f, float endAngle = (float)Math.PI * 2f)
		{
			SolidRing(xy ? new float3(center, 0f) : new float3(center.x, 0f, center.y), rotation, innerRadius, outerRadius, startAngle, endAngle);
		}
		...
		{
			WireRing(xy ? new float3(center, 0f) : new float3(center.x, 0f, center.y), rotation, innerRadius, outerRadius, startAngle, endAngle, color);
		}
		...
		{
			SolidRing(xy ? new float3(center, 0f) : new float3(center.x, 0f, center.y), rotation, innerRadius, outerRadius, startAngle, endAngle, color);
		}
		...
		{
			Circle(center, radius, 0f, (float)Math.PI * 2f, color);
		}
		...
		{
			SolidCircle(center, radius, 0f, (float)Math.PI * 2f, color);
		}
		...
		{
			draw.SolidPlane(xy ? new float3(rect.center.x, rect.center.y, 0f) : new float3(rect.center.x, 0f, rect.center.y), xy ? XY_TO_XZ_ROTATION : XZ_TO_XZ_ROTATION, new float2(rect.width, rect.height), color);
		}
		...
		{
			WireRing(xy ? new float3(center, 0f) : new float3(center.x, 0f, center.y), innerRadius, outerRadius, startAngle, endAngle, color);
		}

		public void WireRing(float2 center, float innerRadius, float outerRadius, Color color)
		{
			WireRing(center, innerRadius, outerRadius, 0f, (float)Math.PI * 2f, color);
		}
		...
		{
			SolidRing(xy ? new float3(center, 0f) : new float3(center.x, 0f, center.y), innerRadius, outerRadius, startAngle, endAngle, color);
		}

		public void SolidRing(float2 center, float innerRadius, float outerRadius, Color color)
		{
			SolidRing(center, innerRadius, outerRadius, 0f, (float)Math.PI * 2f, color);
		}
