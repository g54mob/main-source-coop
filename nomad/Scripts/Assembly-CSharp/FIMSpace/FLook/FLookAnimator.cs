...
namespace FIMSpace.FLook
{
	...
	public class FLookAnimator : MonoBehaviour, IDropHandler, IEventSystemHandler, IFHierarchyIcon
	{
		...
		private float distanceOptimizeFade = 1f;
		...
		private void NoddingCalculations()
		{
			if (nodProgress < nodDuration)
			{
				...
				{
					nodValue = Mathf.Sin(value * (float)Math.PI);
				}
			}
		}
		...
		private void UpdateLookAnimatorAmountWeight()
		{
			...
			if (_stopLooking)
			{
				animatedMotionWeight = Mathf.SmoothDamp(animatedMotionWeight, 0f, ref _velo_animatedMotionWeight, Mathf.Lerp(0.5f, 0.25f, RotationSpeed / 2.5f), 1f / 0f, delta * num);
			}
			else
			{
				...
				animatedMotionWeight = Mathf.SmoothDamp(animatedMotionWeight, 1f, ref _velo_animatedMotionWeight, Mathf.Lerp(0.3f, 0.125f, RotationSpeed / 2.5f), 1f / 0f, delta * num);
			}
		...
		private void LookPositionUpdate()
		{
			...
			{
				smoothLookPosition = Vector3.SmoothDamp(smoothLookPosition, activeLookPosition, ref _velo_smoothLookPosition, LookAtPositionSmoother / 2f, 1f / 0f, delta);
			}
			else
			{
		...
		private void LookWhenAboveGoBackCalculations()
		{
			if (whenAboveGoBackDuration > 0f)
			{
				...
				{
					animatedLookWhenAbove = Mathf.SmoothDamp(animatedLookWhenAbove, 0f, ref _whenAboveGoBackVelo, whenAboveGoBackDuration, 1f / 0f, delta);
					...
					{
						animatedLookWhenAboveVertical = Mathf.SmoothDamp(animatedLookWhenAboveVertical, 0f, ref _whenAboveGoBackVerticalVelo, whenAboveGoBackDuration, 1f / 0f, delta);
					}
				}
				return;
			}
			...
			{
				animatedLookWhenAbove = Mathf.SmoothDamp(animatedLookWhenAbove, LookWhenAbove, ref _whenAboveGoBackVelo, whenAboveGoBackDuration, 1f / 0f, delta);
			}
			if (LookWhenAboveVertical <= 0f)
			{
				animatedLookWhenAboveVertical = animatedLookWhenAbove;
			...
			{
				animatedLookWhenAboveVertical = Mathf.SmoothDamp(animatedLookWhenAboveVertical, LookWhenAboveVertical, ref _whenAboveGoBackVerticalVelo, whenAboveGoBackDuration, 1f / 0f, delta);
			}
			if (WhenAboveGoBackAfter > 0f)
			{
		...
		public virtual void FindHeadBone()
		{
			...
			Animator componentInChildren = baseTransform.GetComponentInChildren<Animator>();
			Transform transform = null;
			if ((bool)componentInChildren && componentInChildren.isHuman)
			{
				transform = componentInChildren.GetBoneTransform(HumanBodyBones.Head);
			}
			List<SkinnedMeshRenderer> list = new List<SkinnedMeshRenderer>();
			Transform[] componentsInChildren = baseTransform.GetComponentsInChildren<Transform>();
			...
			{
				for (int k = 0; k < list[j].bones.Length; k++)
				{
					Transform transform5 = list[j].bones[k];
					if (transform5.name.ToLower().Contains("head") && !(transform5.parent == baseTransform))
					{
				...
				for (int l = 0; l < list[j].bones.Length; l++)
				{
					Transform transform5 = list[j].bones[l];
					if (transform5.name.ToLower().Contains("neck"))
					{
						transform3 = transform5;
						break;
					}
			...
			componentsInChildren = baseTransform.GetComponentsInChildren<Transform>();
			foreach (Transform transform6 in componentsInChildren)
			{
				if (transform6.name.ToLower().Contains("head"))
				{
					if (!transform6.GetComponent<SkinnedMeshRenderer>())
					{
						transform3 = transform6;
						break;
					}
					if (!(transform6.parent == baseTransform))
					{
			...
			{
				componentsInChildren = baseTransform.GetComponentsInChildren<Transform>();
				foreach (Transform transform7 in componentsInChildren)
				{
					if (transform7.name.ToLower().Contains("neck"))
					{
			...
			if (transform3 == null && transform != null)
			{
				transform3 = transform;
			}
			else if (transform3 != null && transform != null)
			{
				if (transform.name.ToLower().Contains("head"))
				{
					transform3 = transform;
				}
				else if (!transform3.name.ToLower().Contains("head"))
				{
					transform3 = transform;
				}
			}
			...
			else if ((bool)transform4)
			{
				LeadBone = transform4;
				Debug.LogWarning("[LOOK ANIMATOR] Found " + transform4?.ToString() + " but it's probably wrong transform");
			}
			else
			{
		...
		public virtual void FindCompensationBones()
		{
			...
			Transform leadBone = LeadBone;
			if ((bool)componentInChildren)
			{
				if (componentInChildren.isHuman)
				{
					Transform boneTransform = componentInChildren.GetBoneTransform(HumanBodyBones.LeftShoulder);
					if ((bool)boneTransform)
					{
						list.Add(boneTransform);
					}
					boneTransform = componentInChildren.GetBoneTransform(HumanBodyBones.RightShoulder);
					if ((bool)boneTransform)
					{
						list.Add(boneTransform);
					}
					boneTransform = componentInChildren.GetBoneTransform(HumanBodyBones.LeftUpperArm);
					if ((bool)boneTransform)
					{
						list.Add(boneTransform);
					}
					boneTransform = componentInChildren.GetBoneTransform(HumanBodyBones.RightUpperArm);
					if ((bool)boneTransform)
					{
						list.Add(boneTransform);
					}
					if (!leadBone)
					{
						componentInChildren.GetBoneTransform(HumanBodyBones.Head);
					}
				}
				else if ((bool)componentInChildren)
				{
					Transform[] componentsInChildren = componentInChildren.transform.GetComponentsInChildren<Transform>();
					foreach (Transform transform in componentsInChildren)
					{
						if (transform.name.ToLower().Contains("clav"))
						{
						...
						else if (transform.name.ToLower().Contains("shoulder"))
						{
							if (!list.Contains(transform))
							{
								list.Add(transform);
						...
						else if (transform.name.ToLower().Contains("uppera") && !list.Contains(transform))
						{
							list.Add(transform);
						}
					}
			...
			for (int j = 0; j < list.Count; j++)
			{
				...
				for (int k = 0; k < CompensationBones.Count; k++)
				{
					if (list[j] == CompensationBones[k].Transform)
					{
						flag = true;
						break;
					}
				...
				bool flag2 = false;
				for (int l = 0; l < CompensationBones.Count; l++)
				{
					if (CompensationBones[l].Transform == null)
					{
						CompensationBones[l] = new CompensationBone(list[j]);
						flag2 = true;
						break;
					}
				}
				if (!flag2)
				{
					CompensationBones.Add(new CompensationBone(list[j]));
				}
			}
			for (int num = CompensationBones.Count - 1; num >= 0; num--)
			{
				if (CompensationBones[num].Transform == null)
				{
					CompensationBones.RemoveAt(num);
				}
			}
		}
		...
		protected virtual void Start()
		{
			...
			if (LookBones == null || LookBones.Count == 0 || ContainsNullTransforms())
			{
				RefreshLookBones();
			}
			if (!StartAfterTPose)
		...
		private bool ContainsNullTransforms()
		{
			for (int i = 0; i < LookBones.Count; i++)
			{
		...
		private void Update()
		{
			...
			{
				if (CalculateDistanceToCameraForOptimization() > OptimizeAtDistance)
				{
		...
		{
			float result = -1f;
			if (customDistanceCamera != null)
			{
				result = Vector3.Distance(BaseTransform.position, customDistanceCamera.transform.position);
			}
			else if (Camera.main != null)
			{
				result = Vector3.Distance(BaseTransform.position, Camera.main.transform.position);
			}
			return result;
		}
