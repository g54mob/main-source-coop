...
namespace FIMSpace
{
	...
	{
		public static bool CanDrawLog(string id, int delayToNextCallInSeconds = 2147483647, int callLimitBeforeTimeMove = 1, int logSeparation = 0)
		{
			...
			if (PlayerPrefs.GetInt(id + "s", 0) != id2)
			{
				...
				if (delayToNextCallInSeconds == 2147483647)
				{
					return true;
				}
			}
			else if (delayToNextCallInSeconds == 2147483647)
			{
				return false;
			}
		...
		public static bool EditorCanDrawLog(string id, int delayToNextCallInSeconds = 2147483647, int callLimitBeforeTimeMove = 1, int logSeparation = 0)
		{
			return false;
		}
	}
