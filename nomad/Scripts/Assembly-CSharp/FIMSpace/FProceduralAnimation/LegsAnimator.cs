	...
	public class LegsAnimator : MonoBehaviour, IDropHandler, IEventSystemHandler, IFHierarchyIcon
	{
		...
		public class HipsReference
		{
			...
			public float CalculateBodyAdjust()
			{
				...
				if (Owner.IsGrounded)
				{
					Vector3 vector = new Vector3(3.4028235E+38f, 3.4028235E+38f, 3.4028235E+38f);
					Vector3 vector2 = new Vector3(3.4028235E+38f, 3.4028235E+38f, 3.4028235E+38f);
					for (int i = 0; i < ChildLegs.Count; i++)
					{
						Leg leg = ChildLegs[i];
					...
					bool flag = false;
					if (vector.x != 3.4028235E+38f && Owner.BodyStepDown > 0f && vector.y <= 0f)
					{
						AnimateStepAdjustTo(vector.y);
						flag = true;
					}
					if (!flag && Owner.MaxBodyStepUp > 0f && vector2.x != 3.4028235E+38f)
					{
		...
		public class Leg
		{
						...
						if (animationMoveType == EMoveType.FromLastAttachement)
						{
							...
							if (leg.LegAnimatingSettings.AllowSpeedups > 0f)
							{
								if (leg.hasOppositeleg)
								{
									...
									if (!oppositeLeg._UsingCustomRaycast && oppositeLeg.G_AttachementHandler.legMoveAnimation.attached && oppositeLeg.G_Attachement.IsValid)
									{
			...
			public void SetNextLeg(Leg next)
			{
				NextLeg = next;
			}

			public void RefreshPlaymodeIndex(int idx)
			{
				PlaymodeIndex = idx;
			}
			...
			public void RefreshLegAnkleToHeelAndFeet(Transform baseT)
			{
				if (!(BoneEnd == null))
				{
					...
					Vector3 position2 = baseT.InverseTransformPoint(position);
					position2.y = 0f;
					position = baseT.TransformPoint(position2);
		...
		public Vector3 Up
		{
			...
			set
			{
				_worldUpAxisVector = value.normalized;
			}
		}
		...
		private void RefreshMatrices()
		{
			...
			Vector3 vector = baseTransform.forward;
			if (Mathf.Abs(Vector3.Dot(vector, Up)) > 0.99f)
			{
				vector = baseTransform.right;
			}
			Quaternion q = Quaternion.LookRotation(Vector3.Cross(Vector3.Normalize(Vector3.Cross(Up, vector)), Up), Up);
		...
		{
			if (id < 0 || id >= Legs.Count)
			{
				return;
			}
			Leg leg = Legs[id];
			Leg leg2 = null;
			Dictionary<Leg, Leg> dictionary = new Dictionary<Leg, Leg>();
			for (int i = 0; i < Legs.Count; i++)
			{
				if (Legs[i].OppositeLegIndex == id)
				{
					Legs[i].OppositeLegIndex = -1;
				}
				else if (Legs[i].OppositeLegIndex != -1)
				{
					dictionary.Add(Legs[i], Legs[i].GetOppositeLegReference(this));
				}
				if (leg2 == null && Legs[i].NextLeg == leg)
				{
					leg2 = Legs[i];
				}
			}
			leg2.SetNextLeg(leg.NextLeg);
			Legs.RemoveAt(id);
			leg.ParentHub.ChildLegs.Remove(leg);
			for (int j = 0; j < Legs.Count; j++)
			{
				Legs[j].RefreshPlaymodeIndex(j);
			}
			foreach (KeyValuePair<Leg, Leg> item in dictionary)
			{
				item.Key.OppositeLegIndex = item.Value.PlaymodeIndex;
			}
			for (int k = 0; k < Legs.Count; k++)
			{
				Legs[k].RefreshHasOppositeLeg();
			}
		}
