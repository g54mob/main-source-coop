...
namespace Technie.PhysicsCreator.Rigid
{
	...
	public class Hull : IHull
	{
		...
		private void GenerateConvexHull(Hull hull, Vector3[] meshVertices, int[] meshIndices, Mesh destMesh)
		{
			...
			{
				Mesh mesh = new Mesh();
				mesh.vertices = hullVertices;
				mesh.triangles = hullIndices;
				Mesh mesh2 = new HullSimplifier().Simplify(mesh, hull.maxPlanes, HullSimplifier.PlaneSelection.DisparateAngle, HullSimplifier.HoleFillMethod.SortVertices);
				hullVertices = mesh2.vertices;
				hullIndices = mesh2.triangles;
