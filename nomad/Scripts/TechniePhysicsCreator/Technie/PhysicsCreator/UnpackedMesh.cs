...
namespace Technie.PhysicsCreator
{
	public class UnpackedMesh
	{
		...
		public UnpackedMesh(SkinnedMeshRenderer skinnedRenderer)
		{
			...
			Transform outputLocalSpace = ((rootBone != null) ? rootBone.parent : skinnedRenderer.transform.parent);
			...
			for (int i = 0; i < vertices.Length; i++)
			{
				if (rootBone != null)
				{
					modelSpaceVertices[i] = ApplyBindPoseWeighted(vertices[i], weights[i], bindposes, bones, outputLocalSpace);
				}
				else
				{
