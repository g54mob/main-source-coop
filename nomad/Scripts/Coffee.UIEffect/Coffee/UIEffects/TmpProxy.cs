	...
	internal sealed class TmpProxy : GraphicProxy
	{
		...
		private static void ModifyMesh(TextMeshProUGUI textMeshProUGUI)
		{
			...
			for (int i = 0; i < textMeshProUGUI.textInfo.meshInfo.Length; i++)
			{
				...
				TMP_SubMeshUI subMeshUI2 = GetSubMeshUI(toRelease, tMP_MeshInfo.material, i - 1);
				if (!subMeshUI2)
				{
					break;
				}
				...
				{
					component2.target = uIEffect;
					component2.useTargetTransform = true;
					component2.customRoot = null;
				}
				else if (component is UIEffectReplica { isActiveAndEnabled: not false } uIEffectReplica)
				{
					component2.target = uIEffectReplica.target;
					component2.preset = uIEffectReplica.preset;
					component2.useTargetTransform = uIEffectReplica.useTargetTransform;
					component2.customRoot = uIEffectReplica.customRoot;
				}
				else
				{
