...
namespace DigitalOpus.MB.Core
{
	public class MB3_TextureCombinerPipeline
	{
		...
		public static List<string> shaderTexPropertiesToExclude = new List<string>(new string[2] { "_DetailAlbedoMap", "_DetailNormalMap" });
		...
		internal virtual IEnumerator __Step1_CollectDistinctMatTexturesAndUsedObjects(ProgressUpdateDelegate progressInfo, MB3_TextureCombiner.CombineTexturesIntoAtlasesCoroutineResult result, TexturePipelineData data, MB3_TextureCombiner combiner, MB2_EditorMethodsInterface textureEditorMethods, List<GameObject> usedObjsToMesh, MB2_LogLevel LOG_LEVEL)
		{
				...
				if (!dictionary.TryGetValue(mesh.GetInstanceID(), out var value))
				{
					...
					for (int j = 0; j < mesh.subMeshCount; j++)
					{
						...
						if (data._fixOutOfBoundsUVs && !value[j].hasUVs)
						{
							...
							Debug.LogWarning("Mesh for object " + gameObject?.ToString() + " has no UV channel but 'consider UVs' is enabled. Assuming UVs will be generated filling 0,0,1,1 rectangle.");
						}
					}
						...
						if (material.HasProperty(data.texPropertyNames[l].name))
						{
							...
							if (textureConsideringStandardShaderKeywords != null)
							{
								...
								if ((format != TextureFormat.ARGB32 && format != TextureFormat.RGBA32 && format != TextureFormat.BGRA32 && format != TextureFormat.RGB24 && format != TextureFormat.Alpha8) || flag2)
								{
									...
									{
										Debug.LogError("Object " + gameObject.name + " in the list of objects to mesh uses Texture " + texture.name + " uses format " + format.ToString() + " that is not in: ARGB32, RGBA32, BGRA32, RGB24, Alpha8 or DXT. These textures cannot be resized at runtime. Try changing texture format. If format says 'compressed' try changing it to 'truecolor'");
										result.success = false;
										yield break;
									}
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				Debug.Log($"Step1_CollectDistinctTextures collected {data.distinctMaterialTextures.Count} sets of textures fixOutOfBoundsUV={data._fixOutOfBoundsUVs} considerNonTextureProperties={data._considerNonTextureProperties}");
			}
			if (data.distinctMaterialTextures.Count == 0)
			{
				...
				Debug.LogError("None of the materials on the objects to combine matched any of the allowed materials for submesh with result material: " + data.resultMaterial?.ToString() + " allowedMaterials: " + text + ". Do any of the source objects use the allowed materials?");
				result.success = false;
				yield break;
			}
		...
		private static CreateAtlasForProperty[] CalculateAllTexturesAreNullAndSameColor(TexturePipelineData data, MB2_LogLevel LOG_LEVEL)
		{
			...
			for (int i = 0; i < data.texPropertyNames.Count; i++)
			{
				...
				{
					Debug.Log($"AllTexturesAreNullAndSameColor prop: {data.texPropertyNames[i].name} createAtlas:{_ShouldWeCreateAtlasForThisProperty(i, data._considerNonTextureProperties, array)}  val:{array[i]}");
				}
			}
			return array;
		}
		...
		internal virtual IEnumerator CalculateIdealSizesForTexturesInAtlasAndPadding(ProgressUpdateDelegate progressInfo, MB3_TextureCombiner.CombineTexturesIntoAtlasesCoroutineResult result, TexturePipelineData data, MB3_TextureCombiner combiner, MB2_EditorMethodsInterface textureEditorMethods, MB2_LogLevel LOG_LEVEL)
		{
			...
			for (int l = 0; l < data.distinctMaterialTextures.Count; l++)
			{
				if (LOG_LEVEL >= MB2_LogLevel.debug)
				{
					Debug.Log("Calculating ideal sizes for texSet TexSet " + l + " of " + data.distinctMaterialTextures.Count);
				}
				...
				for (int m = 0; m < data.texPropertyNames.Count; m++)
				{
					...
					if (LOG_LEVEL >= MB2_LogLevel.trace)
					{
						Debug.Log($"Calculating ideal size for texSet {l} property {data.texPropertyNames[m].name}");
					}
					...
					{
						Debug.LogWarning("Texture " + meshBakerMaterialTexture.GetTexName() + "is tiled by " + meshBakerMaterialTexture.matTilingRect.size.ToString() + " tiling will be baked into a texture with maxSize:" + data._maxTilingBakeSize);
					}
					...
					{
						Debug.LogWarning("Texture " + meshBakerMaterialTexture.GetTexName() + " has out of bounds UVs that effectively tile by " + mB_TexSet.obUVscale.ToString() + " tiling will be baked into a texture with maxSize:" + data._maxTilingBakeSize);
					}
					if (meshBakerMaterialTexture.isNull)
					{
						...
						if (LOG_LEVEL >= MB2_LogLevel.trace)
						{
							Debug.Log($"No source texture creating a 16x16 texture for {data.texPropertyNames[m].name} texSet {l} srcMat {mB_TexSet.matsAndGOs.mats[0].GetMaterialName()}");
						}
					}
					if (meshBakerMaterialTexture.isNull)
					{
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				Debug.Log("Total time Step2 Calculate Ideal Sizes part1: " + stopwatch.Elapsed);
			}
			yield break;
		}
		...
		internal virtual IEnumerator __Step3_BuildAndSaveAtlasesAndStoreResults(MB3_TextureCombiner.CombineTexturesIntoAtlasesCoroutineResult result, ProgressUpdateDelegate progressInfo, TexturePipelineData data, MB3_TextureCombiner combiner, MB_ITextureCombinerPacker packer, AtlasPackingResult atlasPackingResult, MB2_EditorMethodsInterface textureEditorMethods, MB_AtlasesAndRects resultAtlasesAndRects, StringBuilder report, MB2_LogLevel LOG_LEVEL)
		{
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				Debug.Log("time Step 3 Create And Save Atlases part 1 " + sw.Elapsed);
			}
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				Debug.Log("Total time Step 3 Create And Save Atlases " + sw.Elapsed);
			}
		}
