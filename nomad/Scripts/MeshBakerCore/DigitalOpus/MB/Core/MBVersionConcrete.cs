...
namespace DigitalOpus.MB.Core
{
	public class MBVersionConcrete : MBVersionInterface
	{
		...
		{
			GraphicsFormatUsage usage = GraphicsFormatUsage.Linear;
			if (!SystemInfo.IsFormatSupported(GraphicsFormat.RGBA_DXT5_SRGB, usage) && !SystemInfo.IsFormatSupported(GraphicsFormat.RGBA_DXT5_UNorm, usage) && !SystemInfo.IsFormatSupported(GraphicsFormat.RGB_DXT1_SRGB, usage))
			{
				return SystemInfo.IsFormatSupported(GraphicsFormat.RGB_DXT1_UNorm, usage);
			}
			return true;
		}
		...
		{
			if (DetectPipeline() != MBVersion.PipelineType.HDRP)
			{
				Debug.LogError("The 'PlayerSettings -> Other Settings -> Scripting Define Symbols' included the symbol 'MB_USING_HDRP' which should only be used if the GraphicsSettings -> Render Pipeline Asset is HDRenderPipelineAsset. The Render Pipeline Asset is NOT HDRenderPipelineAsset. Please  remove the symbol MB_USING_HDRP from PlayerSettings -> Other Settings -> Scripting Define Symbols");
			}
			HDAdditionalCameraData hDAdditionalCameraData = cameraGameObject.GetComponent<HDAdditionalCameraData>();
			if (hDAdditionalCameraData == null)
			{
				hDAdditionalCameraData = cameraGameObject.AddComponent<HDAdditionalCameraData>();
			}
			hDAdditionalCameraData.volumeLayerMask = LayerMask.GetMask("UI");
		}
