...
namespace DigitalOpus.MB.Core
{
	public class MB3_AtlasPackerRenderTextureUsingMesh
	{
		...
		public class MeshAtlas
		{
			internal static void BuildAtlas(AtlasPackingResult packedAtlasRects, List<MB_TexSet> distinctMaterialTextures, int propIdx, int atlasSizeX, int atlasSizeY, Mesh m, List<Material> generatedMats, ShaderTextureProperty property, MB3_TextureCombinerPipeline.TexturePipelineData data, MB3_TextureCombiner combiner, MB2_EditorMethodsInterface textureEditorMethods, MB2_LogLevel LOG_LEVEL)
			{
				...
				for (int j = 0; j < distinctMaterialTextures.Count; j++)
				{
					...
					{
						Debug.Log($"Adding texture {meshBakerMaterialTexture.GetTexName()} to atlas {property.name} for texSet {j} srcMat {mB_TexSet.matsAndGOs.mats[0].GetMaterialName()}");
					}
					Rect rect = rects[j];
					...
					{
						Rect rect2 = rect;
						Debug.LogError("Image in atlas has no height or width " + rect2.ToString());
					}
		...
		internal Texture2D DoRenderAtlas(GameObject go, int width, int height, bool isNormalMap, ShaderTextureProperty propertyName)
		{
			...
			{
				Debug.Log(string.Format("Begin Camera.Render destTex w={0} h={1} camPos={2} camSize={3} camAspect={4}", width, height, go.transform.localPosition, component.orthographicSize, component.aspect.ToString("f5")));
			}
			component.Render();
			...
			if (LOG_LEVEL >= MB2_LogLevel.trace)
			{
				Debug.Log("Finished rendering atlas " + propertyName.name + "  db_time_DoRenderAtlas:" + (float)stopwatch.ElapsedMilliseconds * 0.001f + "  db_ConvertRenderTextureToTexture2D:" + (float)stopwatch2.ElapsedMilliseconds * 0.001f);
			}
