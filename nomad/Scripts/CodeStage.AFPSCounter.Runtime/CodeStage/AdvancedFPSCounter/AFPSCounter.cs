...
namespace CodeStage.AdvancedFPSCounter
{
	...
	public class AFPSCounter : MonoBehaviour
	{
		...
		private static AFPSCounter GetOrCreateInstance(bool keepAlive)
		{
			...
			AFPSCounter aFPSCounter = UnityEngine.Object.FindFirstObjectByType<AFPSCounter>(FindObjectsInactive.Include);
			if (aFPSCounter != null)
			{
		...
		{
			if (hotKey != KeyCode.None && AFPSInputProxy.GetHotKeyDown(hotKey))
			{
				bool flag = true;
				if (hotKeyCtrl)
				{
					flag &= AFPSInputProxy.GetControlKey();
				}
				if (hotKeyAlt)
				{
					flag &= AFPSInputProxy.GetAltKey();
				}
				if (hotKeyShift)
				{
					flag &= AFPSInputProxy.GetShiftKey();
				}
				if (flag)
				{
					SwitchCounter();
		...
		private bool CircleGestureMade()
		{
			...
			if (AFPSInputProxy.GetMouseButton(0))
			{
				...
				if (num == 0 || (mousePosition - gesturePoints[num - 1]).magnitude > 10f)
				{
			...
			{
				Vector2 vector = gesturePoints[i + 1] - gesturePoints[i];
				zero += vector;
