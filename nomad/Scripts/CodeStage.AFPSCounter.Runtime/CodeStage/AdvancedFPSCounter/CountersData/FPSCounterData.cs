	...
	public class FPSCounterData : UpdatableCounterData
	{
		...
		internal override void UpdateValue(bool force)
		{
			...
			if (average && LastValue > -1)
			{
				...
				{
					currentAverageRaw += ((float)LastValue - currentAverageRaw) / (float)(averageSamples + 1);
				}
				else
				{
			...
			if (realtimeFPS)
			{
				string value = ((LastValue >= warningLevelValue || LastValue == -1) ? colorCached : ((LastValue > criticalLevelValue) ? colorWarningCached : colorCriticalCached));
				text.Append(value).AppendLookup(LastValue).Append("</color>");
				if (milliseconds)
				{
					value = ((LastValue >= warningLevelValue || LastValue == -1) ? colorCachedMs : ((LastValue > criticalLevelValue) ? colorWarningCachedMs : colorCriticalCachedMs));
			...
			if (average)
			{
				...
				string value = ((num4 >= warningLevelValue || num4 == -1) ? colorCachedAvg : ((num4 > criticalLevelValue) ? colorWarningCachedAvg : colorCriticalCachedAvg));
			...
			if (minMax)
			{
				...
				string value = ((LastMinimumValue >= warningLevelValue || LastMinimumValue == -1) ? colorCachedMin : ((LastMinimumValue > criticalLevelValue) ? colorWarningCachedMin : colorCriticalCachedMin));
				...
				value = ((LastMaximumValue >= warningLevelValue || LastMaximumValue == -1) ? colorCachedMax : ((LastMaximumValue > criticalLevelValue) ? colorWarningCachedMax : colorCriticalCachedMax));
		...
		protected override void PerformActivationActions()
		{
			...
			LastMaximumValue = -1;
			LastAverageValue = -1;
			LastAverageMillisecondsValue = -1f;
			if (render)
			{
				previousFrameCount = Time.frameCount;
				TryToAddRenderRecorder();
