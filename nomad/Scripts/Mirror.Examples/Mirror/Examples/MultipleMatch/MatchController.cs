...
namespace Mirror.Examples.MultipleMatch
{
	...
	public class MatchController : NetworkBehaviour
	{
		...
		private sealed class _003CServerEndMatch_003Ed__32 : IEnumerator<object>, IEnumerator, IDisposable
		{
			...
			private bool MoveNext()
			{
				...
				case 0:
					...
					matchController.canvasController.OnPlayerDisconnect -= matchController.OnPlayerDisconnect;
				...
				case 1:
					...
					if (!disconnected)
					{
						NetworkServer.RemovePlayerForConnection(matchController.player1.connectionToClient, RemovePlayerOptions.Destroy);
						CanvasController.waitingConnections.Add(matchController.player1.connectionToClient);
						NetworkServer.RemovePlayerForConnection(matchController.player2.connectionToClient, RemovePlayerOptions.Destroy);
		...
		[ReadOnly]
		[SerializeField]
		internal NetworkIdentity startingPlayer;
		...
		public Action<NetworkIdentity, NetworkIdentity> _Mirror_SyncVarHookDelegate_currentPlayer;
		...
		public NetworkIdentity NetworkcurrentPlayer
		{
			...
			set
			{
				GeneratedSyncVarSetter_NetworkIdentity(value, ref currentPlayer, 1uL, _Mirror_SyncVarHookDelegate_currentPlayer, ref ___currentPlayerNetId);
			}
		}

		private void Awake()
		{
			canvasController = UnityEngine.Object.FindAnyObjectByType<CanvasController>();
		}
		...
		public void CmdMakePlay(CellValue cellValue, NetworkConnectionToClient sender = null)
		{
			...
			SendCommandInternal("System.Void Mirror.Examples.MultipleMatch.MatchController::CmdMakePlay(Mirror.Examples.MultipleMatch.CellValue,Mirror.NetworkConnectionToClient)", 890435284, writer, 0, requiresAuthority: false);
		...
		public void RpcUpdateCell(CellValue cellValue, NetworkIdentity player)
		{
			...
			SendRPCInternal("System.Void Mirror.Examples.MultipleMatch.MatchController::RpcUpdateCell(Mirror.Examples.MultipleMatch.CellValue,Mirror.NetworkIdentity)", -754276294, writer, 0, includeOwner: true);
			NetworkWriterPool.Return(writer);
		}
		...
		public void RpcShowWinner(NetworkIdentity winner)
		{
			...
			SendRPCInternal("System.Void Mirror.Examples.MultipleMatch.MatchController::RpcShowWinner(Mirror.NetworkIdentity)", 1068113577, writer, 0, includeOwner: true);
			NetworkWriterPool.Return(writer);
		}
		...
		{
			NetworkWriterPooled writer = NetworkWriterPool.Get();
			SendCommandInternal("System.Void Mirror.Examples.MultipleMatch.MatchController::CmdPlayAgain(Mirror.NetworkConnectionToClient)", -268672418, writer, 0, requiresAuthority: false);
			NetworkWriterPool.Return(writer);
		}
		...
		{
			NetworkWriterPooled writer = NetworkWriterPool.Get();
			SendRPCInternal("System.Void Mirror.Examples.MultipleMatch.MatchController::RpcRestartGame()", 369943583, writer, 0, includeOwner: true);
			NetworkWriterPool.Return(writer);
		}
		...
		public void RequestExitGame()
		{
			if (!NetworkClient.active)
			{
				Debug.LogWarning("[Client] function 'System.Void Mirror.Examples.MultipleMatch.MatchController::RequestExitGame()' called when client was not active");
				return;
			}
			exitButton.gameObject.SetActive(value: false);
			playAgainButton.gameObject.SetActive(value: false);
		...
		{
			NetworkWriterPooled writer = NetworkWriterPool.Get();
			SendCommandInternal("System.Void Mirror.Examples.MultipleMatch.MatchController::CmdRequestExitGame(Mirror.NetworkConnectionToClient)", 1301607703, writer, 0, requiresAuthority: false);
			NetworkWriterPool.Return(writer);
		}
		...
		{
			NetworkWriterPooled writer = NetworkWriterPool.Get();
			SendRPCInternal("System.Void Mirror.Examples.MultipleMatch.MatchController::RpcExitGame()", 822947962, writer, 0, includeOwner: true);
			NetworkWriterPool.Return(writer);
		}
		...
		public MatchController()
		{
			...
			_Mirror_SyncVarHookDelegate_currentPlayer = UpdateGameUI;
		}
		...
		protected static void InvokeUserCode_CmdMakePlay__CellValue__NetworkConnectionToClient(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			if (!NetworkServer.active)
			{
				Debug.LogError("Command CmdMakePlay called on client.");
			}
			else
			{
		...
		protected static void InvokeUserCode_RpcUpdateCell__CellValue__NetworkIdentity(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			if (!NetworkClient.active)
			{
				Debug.LogError("RPC RpcUpdateCell called on server.");
			}
			else
			{
		...
		protected static void InvokeUserCode_RpcShowWinner__NetworkIdentity(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			if (!NetworkClient.active)
			{
				Debug.LogError("RPC RpcShowWinner called on server.");
			}
			else
			{
		...
		protected static void InvokeUserCode_CmdPlayAgain__NetworkConnectionToClient(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			if (!NetworkServer.active)
			{
				Debug.LogError("Command CmdPlayAgain called on client.");
			}
			else
			{
		...
		protected static void InvokeUserCode_RpcRestartGame(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			if (!NetworkClient.active)
			{
				Debug.LogError("RPC RpcRestartGame called on server.");
			}
			else
			{
		...
		protected static void InvokeUserCode_CmdRequestExitGame__NetworkConnectionToClient(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			if (!NetworkServer.active)
			{
				Debug.LogError("Command CmdRequestExitGame called on client.");
			}
			else
			{
		...
		protected static void InvokeUserCode_RpcExitGame(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			if (!NetworkClient.active)
			{
				Debug.LogError("RPC RpcExitGame called on server.");
			}
			else
			{
		...
		public override void DeserializeSyncVars(NetworkReader reader, bool initialState)
		{
			...
			if (initialState)
			{
				GeneratedSyncVarDeserialize_NetworkIdentity(ref currentPlayer, _Mirror_SyncVarHookDelegate_currentPlayer, reader, ref ___currentPlayerNetId);
				return;
			}
