...
namespace NWH.Common.Demo
{
	public class DragObject : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			_mouseRay = _cam.ScreenPointToRay(_mousePosition);
			if (_draggingButtonWasPressed && !_dragging && Physics.Raycast(_mouseRay, out _hit, 600f))
			{
		...
		private void FixedUpdate()
		{
			if (_dragging)
			{
				...
				_resultantForce = new Vector3(_forceXz * base.transform.right.x, _force.y, _forceXz * base.transform.right.z);
