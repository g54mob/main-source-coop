...
namespace NWH.VehiclePhysics2.Demo.VehicleOverview
{
	public class WheelUI : MonoBehaviour
	{
		...
		public void Start()
		{
			...
			_maxTorque = 9600f * _vc.powertrain.engine.maxPower / _vc.powertrain.engine.revLimiterRPM * _vc.powertrain.transmission.GetGearRatio(1) * _vc.powertrain.transmission.finalGearRatio;
		...
		private IEnumerator UpdateCoroutine()
		{
			...
			{
				float num = 0.033f;
				_smoothTorque = Mathf.Lerp(_smoothTorque, Mathf.Abs(wheelUAPI.MotorTorque), num * 40f);
				_smoothLoad = Mathf.Lerp(_smoothLoad, wheelUAPI.Load, num * 40f);
				_smoothLatSlip = Mathf.Lerp(_smoothLatSlip, Mathf.Abs(wheelUAPI.LateralSlip), num * 40f);
				_smoothLngSlip = Mathf.Lerp(_smoothLngSlip, Mathf.Abs(wheelUAPI.LongitudinalSlip), num * 40f);
