...
namespace Rewired
{
	...
	{
		internal class ICvEqTkajGQZLXkOyPTomcjaeCWs
		{
			public bool ZcZgINApiFeKWNrRfDQvzTSWOTHDA;

			public bool hjLbPwdNqHwzoMYJYbkxWjYbBRDG;

			public VKBFHptpOwZTIxZpGslzThtgRcTO[] JQsALtKzYdonJoCbINPGcPTlfsekA;

			public ICvEqTkajGQZLXkOyPTomcjaeCWs(bool P_0, bool P_1, VKBFHptpOwZTIxZpGslzThtgRcTO[] P_2)
			{
				ZcZgINApiFeKWNrRfDQvzTSWOTHDA = P_0;
				hjLbPwdNqHwzoMYJYbkxWjYbBRDG = P_1;
				JQsALtKzYdonJoCbINPGcPTlfsekA = P_2;
			}
		}
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			public ControllerSetSelector controllerSetSelector
			{
				...
				set
				{
					...
					if (!value.QVWXuasRsUAvognxOkisyYrwXInSA)
					{
		...
		private bool gAfnBHsGAOrvyfKmVKtDVyKgsQNU;

		private bool oovAdNulWfcfwyqtqcWFGVwjtcBlA = true;

		private Player rFYSdlZJcONijOfYQYXxoYHPDGhN;

		private ICvEqTkajGQZLXkOyPTomcjaeCWs nfCuhHTjxFoXJCnVEsXTaUBLpgHx;

		private readonly int gxTIyUzjVGBqQNwluTnufjOqOBFo;

		private List<RuleSet> uLrXYajgBlKYIABNTRgfFbgGEePz;

		private Action tefvdcueYLnHPmZIuuSysGoTBWNh;

		public bool enabled
		{
			get
			{
				return gAfnBHsGAOrvyfKmVKtDVyKgsQNU;
			}
			set
			{
				gAfnBHsGAOrvyfKmVKtDVyKgsQNU = value;
				if (value)
				{
					Apply();
				}
		...
		public bool loadFromUserDataStore
		{
			get
			{
				return oovAdNulWfcfwyqtqcWFGVwjtcBlA;
			}
			set
			{
				oovAdNulWfcfwyqtqcWFGVwjtcBlA = value;
			}
		}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return uLrXYajgBlKYIABNTRgfFbgGEePz;
			}
			set
			{
				if (value == null)
				...
				uLrXYajgBlKYIABNTRgfFbgGEePz = value;
			}
		}

		internal event Action cwMWXWydqGuwHwMYdesaMfzxTQMK
		{
			add
			{
				tefvdcueYLnHPmZIuuSysGoTBWNh = (Action)Delegate.Combine(tefvdcueYLnHPmZIuuSysGoTBWNh, b);
			}
			remove
			{
				tefvdcueYLnHPmZIuuSysGoTBWNh = (Action)Delegate.Remove(tefvdcueYLnHPmZIuuSysGoTBWNh, value2);
			}
		}

		internal ControllerMapLayoutManager(Player P_0, ICvEqTkajGQZLXkOyPTomcjaeCWs P_1)
		{
			if (P_0 == null)
			{
			...
			gxTIyUzjVGBqQNwluTnufjOqOBFo = ReInput.id;
			rFYSdlZJcONijOfYQYXxoYHPDGhN = P_0;
			nfCuhHTjxFoXJCnVEsXTaUBLpgHx = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != gxTIyUzjVGBqQNwluTnufjOqOBFo)
			{
				ReInput.CheckInitialized(gxTIyUzjVGBqQNwluTnufjOqOBFo);
				return;
			}
			tefvdcueYLnHPmZIuuSysGoTBWNh?.Invoke();
			if (!gAfnBHsGAOrvyfKmVKtDVyKgsQNU || uLrXYajgBlKYIABNTRgfFbgGEePz == null)
			{
				return;
			}
			int count = uLrXYajgBlKYIABNTRgfFbgGEePz.Count;
			if (count == 0)
			{
				return;
			}
			...
			rFYSdlZJcONijOfYQYXxoYHPDGhN.controllers.maps.GetAllMaps(list);
			list2.AddRange(rFYSdlZJcONijOfYQYXxoYHPDGhN.controllers.Controllers);
			IControllerMapStore controllerMapStore = ReInput.userDataStore as IControllerMapStore;
			for (int i = 0; i < count; i++)
			{
				RuleSet ruleSet = uLrXYajgBlKYIABNTRgfFbgGEePz[i];
				...
				for (int j = 0; j < num; j++)
				{
					...
					for (int num2 = list.Count - 1; num2 >= 0; num2--)
					{
						...
						{
							list.RemoveAt(num2);
							rFYSdlZJcONijOfYQYXxoYHPDGhN.controllers.maps.RemoveMap(controllerMap.controllerType, controllerMap.controllerId, controllerMap.id);
						}
					}
					foreach (Controller controller in rFYSdlZJcONijOfYQYXxoYHPDGhN.controllers.Controllers)
					{
						...
						{
							ControllerMap controllerMap2 = rFYSdlZJcONijOfYQYXxoYHPDGhN.controllers.maps.GetMap(controller, categoryIds[k], rule.layoutId);
							if (controllerMap2 != null)
							{
								continue;
							}
							if (oovAdNulWfcfwyqtqcWFGVwjtcBlA && controllerMapStore != null)
							{
								try
								{
									controllerMap2 = controllerMapStore.LoadControllerMap(rFYSdlZJcONijOfYQYXxoYHPDGhN.id, controller.identifier, categoryIds[k], rule.layoutId);
								}
								catch (Exception exception)
								{
								...
								if (controllerMap2 != null)
								{
									rFYSdlZJcONijOfYQYXxoYHPDGhN.controllers.maps.AddMap(controller, controllerMap2);
									continue;
								}
							}
							rFYSdlZJcONijOfYQYXxoYHPDGhN.controllers.maps.LoadMap(controller.type, controller.id, categoryIds[k], rule.layoutId, startEnabled: true);
						}
					}
				}
			}
		...
		public void LoadDefaults()
		{
			...
			int num = ((nfCuhHTjxFoXJCnVEsXTaUBLpgHx != null && nfCuhHTjxFoXJCnVEsXTaUBLpgHx.JQsALtKzYdonJoCbINPGcPTlfsekA != null) ? nfCuhHTjxFoXJCnVEsXTaUBLpgHx.JQsALtKzYdonJoCbINPGcPTlfsekA.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapLayoutManagerRuleSetInstance = ReInput.mapping.GetControllerMapLayoutManagerRuleSetInstance(nfCuhHTjxFoXJCnVEsXTaUBLpgHx.JQsALtKzYdonJoCbINPGcPTlfsekA[i].ZfJoYctgPpVSnpftahQTlMmpPLQm);
				...
				controllerMapLayoutManagerRuleSetInstance.enabled = nfCuhHTjxFoXJCnVEsXTaUBLpgHx.JQsALtKzYdonJoCbINPGcPTlfsekA[i].dcQXOoEkwgQXbaVwOAumDixTNHlg;
				list.Add(controllerMapLayoutManagerRuleSetInstance);
			}
			if (nfCuhHTjxFoXJCnVEsXTaUBLpgHx != null)
			{
				gAfnBHsGAOrvyfKmVKtDVyKgsQNU = nfCuhHTjxFoXJCnVEsXTaUBLpgHx.ZcZgINApiFeKWNrRfDQvzTSWOTHDA;
				oovAdNulWfcfwyqtqcWFGVwjtcBlA = nfCuhHTjxFoXJCnVEsXTaUBLpgHx.hjLbPwdNqHwzoMYJYbkxWjYbBRDG;
			}
			uLrXYajgBlKYIABNTRgfFbgGEePz = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return FpOCAOFASSPKMfOpKSFdanbZxPceb().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return FpOCAOFASSPKMfOpKSFdanbZxPceb().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				RuvfhtFWEIvhNWJquoaUGlEAGLCdb(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				RuvfhtFWEIvhNWJquoaUGlEAGLCdb(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject FpOCAOFASSPKMfOpKSFdanbZxPceb()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			xuCxfRkKSTBzFoooJkOZrITHzuDx(serializedObject);
			return serializedObject;
		}

		private void xuCxfRkKSTBzFoooJkOZrITHzuDx(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", gAfnBHsGAOrvyfKmVKtDVyKgsQNU);
			P_0.Add("loadFromUserDataStore", oovAdNulWfcfwyqtqcWFGVwjtcBlA);
			P_0.Add("ruleSets", uLrXYajgBlKYIABNTRgfFbgGEePz);
		}

		private bool RuvfhtFWEIvhNWJquoaUGlEAGLCdb(SerializedObject P_0)
		{
			gAfnBHsGAOrvyfKmVKtDVyKgsQNU = false;
			uLrXYajgBlKYIABNTRgfFbgGEePz = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref gAfnBHsGAOrvyfKmVKtDVyKgsQNU);
			P_0.TryGetDeserializedValueByRef("loadFromUserDataStore", ref oovAdNulWfcfwyqtqcWFGVwjtcBlA);
