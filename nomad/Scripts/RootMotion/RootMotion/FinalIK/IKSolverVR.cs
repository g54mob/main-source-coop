...
namespace RootMotion.FinalIK
{
	...
	public class IKSolverVR : IKSolver
	{
		...
		public class Arm : BodyPart
		{
			...
			[ShowIf("shoulderRotationWeight", 0f, 1f / 0f, false, ShowIfMode.Hidden)]
			public ShoulderRotationMode shoulderRotationMode;

			[Tooltip("The weight of twisting the shoulders backwards when arms are lifted up.")]
			[ShowRangeIf(0f, 1f, "shoulderRotationWeight", 0f, 1f / 0f, false, ShowIfMode.Hidden)]
			...
			public void Solve(bool isLeft)
			{
				...
				if (hasShoulder && shoulderRotationWeight > 0f && LOD < 1)
				{
					...
					case ShoulderRotationMode.YawPitch:
					{
						...
						Quaternion quaternion4 = Quaternion.AngleAxis(0f - num7, quaternion2 * Vector3.right) * quaternion3;
						if (shoulderBankOffset != 0f)
						{
							quaternion4 = Quaternion.AngleAxis(shoulderBankOffset, quaternion2 * Vector3.forward) * quaternion4;
						}
						if (shoulderRotationWeight * positionWeight < 1f)
						{
							quaternion4 = Quaternion.Lerp(Quaternion.identity, quaternion4, shoulderRotationWeight * positionWeight);
						}
						VirtualBone.RotateBy(bones, quaternion4);
						Stretching();
				...
				if (LOD < 1 && positionWeight > 0f)
				{
					...
					Quaternion quaternion5 = upperArm.solverRotation * forearmRelToUpperArm;
					Quaternion quaternion6 = Quaternion.FromToRotation(quaternion5 * forearm.axis, hand.solverPosition - forearm.solverPosition);
					RotateTo(forearm, quaternion6 * quaternion5, positionWeight);
				}
		...
		public class Locomotion
		{
			...
			public void Solve_Animated(IKSolverVR solver, float scale, float deltaTime)
			{
				...
				else
				{
					...
					velocityLocal = Vector3.SmoothDamp(velocityLocal, target, ref velocityLocalV, currentAnimationSmoothTime, 1f / 0f, deltaTime);
					...
					solver.spine.maxRootAngle = Mathf.SmoothDamp(solver.spine.maxRootAngle, target2, ref maxRootAngleV, 0.2f, 1f / 0f, deltaTime);
					...
					animSpeed = Mathf.SmoothDamp(animSpeed, value, ref animSpeedV, 0.05f, 1f / 0f, deltaTime);
		...
		public class Spine : BodyPart
		{
			...
			[ShowIf("useAnimatedHeadHeightWeight", 0f, 1f / 0f, false, ShowIfMode.Hidden)]
		...
		public class VirtualBone
		{
			...
			private static Vector3 GetDirectionToBendPoint(Vector3 direction, float directionMag, Vector3 bendDirection, float sqrMag1, float sqrMag2)
			{
				...
				float y = (float)Math.Sqrt(Mathf.Clamp(sqrMag1 - num * num, 0f, 1f / 0f));
